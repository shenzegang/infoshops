...
<div class="cms-content mt10">
	<form id="add_form" method="post"
		action="index.php?act=publish&op=publish_picture_save">
		<input id="save_type" name="save_type" type="hidden" value="draft" />
		<input id="picture_id" name="picture_id" type="hidden"
			value="<?php if(!empty($output['picture_detail'])) echo $output['picture_detail']['picture_id'];?>" />
		<div class="cms-publish-content">
			<table class="cms-publish-table cms-publish-table-picture">
                ...
                <option value="<?php echo $value['class_id'];?>"
									<?php if($value['class_id'] == $output['picture_detail']['picture_class_id']) echo 'selected';?>><?php echo $value['class_name'];?></option>
                <?php } ?>
                <?php } ?>
              </select></td>
						<td rowspan="3" class="w200"><input id="picture_image"
							name="picture_image" type="hidden" value="" />
							<div class="edit-cover-pic">
								<div id="div_picture_image" class="edit-cover-pic">
                <?php if(!empty($output['picture_detail']['picture_image'])) { ?>
                 <img alt=""
										src="<?php echo getCMSArticleImageUrl($output['picture_detail']['picture_attachment_path'],$output['picture_detail']['picture_image'], 'list');?>">
                 <?php } ?>
             </div>
								<p class="edit-cover-text"><?php echo $lang['cms_cover'];?></p></td>
					</tr>
					<tr>
						<th><?php echo $lang['cms_title'];?><?php echo $lang['nc_colon'];?></th>
						<td><input id="picture_title" class="text w300"
							name="picture_title" type="text"
							value="<?php if(!empty($output['picture_detail'])) echo $output['picture_detail']['picture_title'];?>" />
					...
					<tr>
						...
						<td><input id="article_title_short" class="text w300"
							name="picture_title_short" type="text"
							value="<?php if(!empty($output['picture_detail'])) echo $output['picture_detail']['picture_title_short'];?>" />
					...
					<tr>
						...
						<td colspan="2"><div>
								<textarea name="picture_abstract" rows="3" class="textarea"
									style="width: 650px" id="picture_abstract"><?php if(!empty($output['picture_detail'])) echo $output['picture_detail']['picture_abstract'];?>
</textarea>
					...
					<tr>
						...
						<td colspan="2">
							<div id="pic-upload-scrollbar">
								<ul class="picture-image-list">
                ...
                <li>
											...
											<dt class="picture-img">
												<div class="upload-thumb">
													<a href="Javascript: void(0);"><img
														nctype="picture_image_item" alt=""
														src="<?php echo getCMSArticleImageUrl(empty($value['image_path'])?$output['picture_detail']['picture_attachment_path']:$value['image_path'], $value['image_name'], 'list');?>"></a>
												</div>
												<div class="handle">
													<a image_name="<?php echo $value['image_name'];?>"
														nctype="btn_set_picture_image"><?php echo $lang['cms_cover'];?></a><a
														image_name="<?php echo $value['image_name'];?>"
														nctype="btn_drop_picture_image"><?php echo $lang['nc_delete'];?></a>
												</div>
											</dt>
											<input type="hidden"
												value="<?php echo $value['image_name'].','.$value['image_path'];?>"
												name="picture_image_all[]">
											<dd class="picture-abstract">
												<h4><?php echo $lang['cms_image_abstract'];?></h4>
												<textarea name="picture_image_abstract[]" rows="2"
													class="textarea"><?php echo $value['image_abstract'];?></textarea>
											</dd>
											<dd class="picture-add-goods">
												<input class="btn-type-s" nctype="btn_add_image_goods"
													image_name="<?php echo $value['image_name'];?>"
													type="button" value="<?php echo $lang['cms_goods_add'];?>" />
											</dd>
										</dl>
                    ...
                    <dl class="taobao-item">
												<dt class="taobao-item-title">
													<a href="<?php echo $image_value['url'];?>" target="_blank"><?php echo $image_value['title'];?></a>
												</dt>
												<dd class="taobao-item-img">
													<a href="<?php echo $image_value['link'];?>"
														target="_blank"> <img
														src="<?php echo $image_value['image'];?>"
														alt="<?php echo $image_value['title'];?>"
														title="<?php echo $image_value['title'];?>" />
													</a>
												</dd>
												<dd class="taobao-item-price"><?php echo $lang['currency'];?><em><?php echo $image_value['price'];?></em>
												</dd>
												<dd class="taobao-item-delete"
													nctype="btn_image_goods_delete"
													title="<?php echo $lang['nc_delete'];?>"><?php echo $lang['nc_delete'];?></dd>
												<input
													name="image_goods_url[<?php echo $value['image_name'];?>][]"
													type="hidden" value="<?php echo $image_value['url'];?>" />
												<input
													name="image_goods_image[<?php echo $value['image_name'];?>][]"
													type="hidden" value="<?php echo $image_value['image'];?>" />
												<input
													name="image_goods_price[<?php echo $value['image_name'];?>][]"
													type="hidden" value="<?php echo $image_value['price'];?>" />
												<input
													name="image_goods_title[<?php echo $value['image_name'];?>][]"
													type="hidden" value="<?php echo $image_value['title'];?>" />
											</dl>
                    <?php } ?>
                    <?php } ?>
                  </div>
                ...
                <?php } ?>
							...
							<div class="upload-btn">
								<a href="javascript:void(0);"><span> <input type="file"
										name="article_image_upload" id="picture_image_upload"
										multiple="" file_id="0"
					...
					<tr>
						...
						<td colspan="2"><input id="picture_author" class="text w100"
							name="picture_author" type="text"
							value="<?php echo empty($output['picture_detail']['picture_author'])?$output['publisher_info']['name']:$output['picture_detail']['picture_author'];?>" />
							<div class="hint"><?php echo $lang['cms_publisher_explain'];?></div></td>
					</tr>
					<tr>
						<th><?php echo $lang['cms_publish_time'];?></th>
						<td colspan="2"><input id="picture_publish_time" class="text w100"
							name="picture_publish_time" type="text"
							value="<?php echo empty($output['picture_detail'])?date('Y-m-d',time()):date('Y-m-d',$output['picture_detail']['picture_publish_time']);?>" />
              ...
              <?php } ?>
              <input id="picture_tag" name="picture_tag" type="hidden"
							value="<?php if(!empty($output['picture_detail'])) echo $output['picture_detail']['picture_tag'];?>" /></td>
					</tr>
					<tr>
						<th><?php echo $lang['cms_keyword'];?><?php echo $lang['nc_colon'];?></th>
						<td colspan="2"><input id="picture_keyword" class="text w380"
							name="picture_keyword" type="text"
							value="<?php if(!empty($output['picture_detail'])) echo $output['picture_detail']['picture_keyword'];?>" />
						<div class="hint"><?php echo $lang['cms_keyword_expalin'];?></div></td>
					</tr>
				</tbody>
				<tfoot>
					<tr>
						<td colspan="21"><a id="btn_draft" class="btn-draft"
