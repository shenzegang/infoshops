...
<dl>
	<dt>
		<span class="thumb"><a
			href="<?php echo SHOP_SITE_URL;?>/index.php?act=member_snshome&mid=<?php echo $value['comment_member_id'];?>"
			target="_blank"> <img
				src="<?php echo getMemberAvatar($value['member_avatar']);?>"
				alt="<?php echo $value['member_name'];?>" /></a></span>
	</dt>
	<dd>
		<p class="comment-user">
			<a
				href="<?php echo SHOP_SITE_URL;?>/index.php?act=member_snshome&mid=<?php echo $value['comment_member_id'];?>"
				target="_blank"> <?php echo $value['member_name'];?></a><em>发表于<?php echo date('Y-m-d H:i:s',$value['comment_time']);?></em>
		</p>
    ...
    <?php if(isset($output['comment_quote_list'][$comment_quote_id])) { ?>
		...
		<p>
			...
			<span class="quote"><a nctype="comment_quote"
				comment_id="<?php echo $output['comment_quote_list'][$comment_quote_id]['comment_id'];?>"
				href="javascript:void(0)">回复</a></span>
		</p>
    <?php } else { ?>
    <p>
			<span class="comment-quote-member"></span> <span
				class="comment-quote-message">已删除</span>
		</p>
    <?php } ?>
    </div>
    <?php $i++;?>
    ...
    <?php } ?>
		...
		<p class="commnet-date">
			<a nctype="comment_up"
				comment_id="<?php echo $value['comment_id'];?>"
				href="javascript:void(0)" class="up">顶<span>[<em><?php echo $value['comment_up'];?></em>]
			</span></a> <a nctype="comment_quote"
				comment_id="<?php echo $value['comment_id'];?>"
				href="javascript:void(0)" class="quote">回复</a>
    <?php if($value['comment_member_id'] == $_SESSION['member_id']) { ?>
    <a nctype="comment_drop"
				comment_id="<?php echo $value['comment_id'];?>"
				href="javascript:void(0)" class="del"><?php echo $lang['nc_delete'];?></a>
    <?php } ?>
    </p>
	</dd>
</dl>
