...
<div id="div_sns_share" style="display: none;" class="cms-share">
	<form
		action="<?php echo CMS_SITE_URL;?>/index.php?act=share&op=share_save&type=<?php echo $_GET['act'];?>"
		method="post" id="share_form" class="feededitor">
		<input type="hidden" value="<?php echo $output['detail_object_id'];?>"
			name="share_id"> </input> <input type="hidden"
			value="<?php echo $output[$_GET['act'].'_detail'][$_GET['act'].'_title'];?>"
			name="share_title"> </input> <input type="hidden"
			value="<?php echo getCMSArticleImageUrl($output[$_GET['act'].'_detail'][$_GET['act'].'_attachment_path'], $output[$_GET['act'].'_detail'][$_GET['act'].'_image']);?>"
			name="share_image"> </input>
		<dl class="share-target">
			<dt id="_share_title" class="title"></dt>
			<dd class="cover">
				<a href="JavaScript: void(0);"><img id="_share_image" src="" alt="" /></a>
			</dd>
			<dd class="sub">
				<span id="_share_publisher"></span><span id="_share_origin"></span>
				<!--<span id="_share_publish_time"></span>-->
			</dd>
			<dd class="abstract">
				<span id="_share_abstract"></span>
			</dd>
		</dl>
		<div class="share-content">
			<textarea name="commend_message" id="commend_message"
				placeholder="<?php echo $lang['cms_sns_share_title'];?>"></textarea>
		</div>
		<div class="handle">
			<input id="btn_share_publish" type="button"
				value="<?php echo $lang['cms_text_share'];?>" class="button" />
			<!-- 站外分享 -->
