...
class articleControl extends CMSHomeControl
{
    ...
    public function article_comment_detailOp()
    {
        ...
        $article_hot_comment = $model_article->getList(array(
            'article_state' => self::ARTICLE_STATE_PUBLISHED
        ), null, 'article_comment_count desc', '*', 10);
    ...
    public function article_searchOp()
    {
        ...
        if (C('fullindexer.open')) {
            $condition['article_id'] = array(
                'in',
                $this->full_search($_GET['keyword'], 'index_cms_article')
            );
        } else {
