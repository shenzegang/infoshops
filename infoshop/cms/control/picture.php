...
class pictureControl extends CMSHomeControl
{
    ...
    public function picture_detailOp()
    {
        ...
        $picture_image_list = $model_picture_image->getList(array(
            'image_picture_id' => $picture_id
        ), NULL);
        Tpl::output('picture_image_list', $picture_image_list);
        
        $conition = array();
        $conition['picture_state'] = self::ARTICLE_STATE_PUBLISHED;
        $conition['picture_id'] = array(
            'lt',
            $picture_id
        );
    ...
    public function picture_comment_detailOp()
    {
        ...
        $picture_hot_comment = $model_picture->getList(array(
            'picture_state' => self::ARTICLE_STATE_PUBLISHED
        ), null, 'picture_comment_count desc', '*', 10);
    ...
    public function picture_searchOp()
    {
        ...
        if (C('fullindexer.open')) {
            $condition['picture_id'] = array(
                'in',
                $this->full_search($_GET['keyword'], 'index_cms_picture')
            );
        } else {
