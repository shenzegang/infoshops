...
class CMSMemberControl extends CMSControl
{
    ...
    public function __construct()
    {
        ...
        // 发布人信息
        Tpl::output('publisher_info', array(
    ...
    protected function check_article_auth($article_id)
    {
        if ($article_id > 0) {
            $model_article = Model('cms_article');
            $article_detail = $model_article->getOne(array(
                'article_id' => $article_id
            ));
            if (! empty($article_detail)) {
                if ($this->publisher_type == self::ARTICLE_TYPE_ADMIN || ($article_detail['article_type'] != self::ARTICLE_TYPE_ADMIN && $article_detail['article_publisher_id'] == $this->publisher_id)) {
    ...
    protected function check_picture_auth($picture_id)
    {
        if ($picture_id > 0) {
            ...
            $picture_detail = $model_picture->getOne(array(
                'picture_id' => $picture_id
            ));
            if (! empty($picture_detail)) {
                if ($this->publisher_type == self::ARTICLE_TYPE_ADMIN || ($picture_detail['picture_type'] != self::ARTICLE_TYPE_ADMIN && $picture_detail['picture_publisher_id'] == $this->publisher_id)) {
