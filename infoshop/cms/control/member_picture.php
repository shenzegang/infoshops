...
class member_pictureControl extends CMSMemberControl
{
    ...
    public function picture_listOp()
    {
        ...
        if (! empty($_GET['picture_state'])) {
            ...
            $condition['picture_state'] = array(
                'in',
                array(
                    self::ARTICLE_STATE_PUBLISHED,
                    self::ARTICLE_STATE_VERIFY
                )
            );
        }
    ...
    private function get_picture_list($condition = array())
    {
        ...
        $picture_image_array = $model_picture_image->getList(array(
            'image_picture_id' => array(
                'in',
                $picture_ids
            )
        ));
    ...
    public function picture_dropOp()
    {
        ...
        if ($picture_auth) {
            $model_picture = Model('cms_picture');
            $result = $model_picture->drop(array(
    ...
    private function picture_state_change($picture_state_new)
    {
        ...
        if ($picture_auth) {
            $model_picture = Model('cms_picture');
            $result = $model_picture->modify(array(
                'picture_state' => $picture_state_new
            ), array(
                'picture_id' => $picture_id
            ));
