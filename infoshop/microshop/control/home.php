...
class homeControl extends MircroShopControl
{
    ...
    {
        self::get_goods_list(array(
            'commend_member_id' => $this->member_info['member_id']
        ));
    ...
    private function publish_drop($key)
    {
        ...
        $info = $model->getOne(array(
            $key => $_GET['item_id']
        ));
        $result = FALSE;
        if ($info['commend_member_id'] == $_SESSION['member_id']) {
            $result = $model->drop(array(
                $key => $_GET['item_id']
            ));
            // 删除个人秀图片
        ...
        switch ($type) {
            ...
            case 'store':
                ...
                if (! empty($like_store_list)) {
                    ...
                    $store_list = $model_microshop_store->getListWithStoreInfo(array(
                        'microshop_store_id' => array(
                            'in',
                            $store_id
                        )
