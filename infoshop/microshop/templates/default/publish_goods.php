...
<div class="all-layout-box">
  ...
  <div class="publish-goods-list">
      ...
      <li
				class="<?php echo in_array($val['goods_id'],$output['commend_goods_array'])?'selected':'select' ?>">
				<div class="picture">
					<span class="thumb size210"><i></i><a
						href="<?php echo urlShop('goods', 'index',array('goods_id'=>$val['goods_id']));?>"
						target="_blank"> <img src="<?php echo thumb($val, 240);?>"
							onload="javascript:DrawImage(this,210,210);"
							alt="<?php echo $val['goods_name'];?>"
          ...
          <?php } else { ?>
          <a href="javascript:void(0)" class="btn_commend_dialog"
						goods_id="<?php echo $val['goods_id'];?>"
						goods_name="<?php echo $val['goods_name'];?>"
						goods_price="<?php echo $lang['currency'].$val['goods_price'];?>"
						goods_image="<?php echo thumb($val, 240);?>"><?php echo $lang['microshop_goods_commend_want'];?></a>
          <?php } ?>
        </div>
			</li>
      <?php } ?>
...
<div id="div_commend_goods" style="display: none;">
	<form
		action="<?php echo MICROSHOP_SITE_URL;?>/index.php?act=publish&op=goods_save"
		method="post" id="add_form" class="feededitor">
		<input type="hidden" value="" name="commend_goods_id"
			id="commend_goods_id"> </input>
		<div class="command-goods">
			<div class="pic">
				<span class="thumb size100"> <i></i><img alt=""
					onload="javascript:DrawImage(this,100,100);" src=""
					id="commend_dialog_goods_image"></span>
			</div>
			<dl class="intro">
