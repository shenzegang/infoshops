...
<div class="commend-goods-list">
  ...
  <ul id="pinterest">
		<li id="btn_personal_publish" class="item" style="display: none;">
			<div class="pinterest-button">
				<a id="btn_show_publish_div" href="javascript:void(0)"
					class="pngFix"><?php echo $lang['microshop_personal_publish'];?></a>
			</div>
		</li>
    ...
    <li class="item">
      ...
      <!-- 喜欢删除按钮 -->
			<div class="del">
				<a nc_type="like_drop" like_id="<?php echo $value['like_id'];?>"
					href="javascript:void(0)"><?php echo $lang['nc_delete'];?></a>
			</div>
      <?php } else { ?>
      <!-- 个人主页删除按钮 -->
			<div class="del">
				<a nc_type="home_drop" type="personal"
					item_id="<?php echo $value['personal_id'];?>"
					href="javascript:void(0)"><?php echo $lang['nc_delete'];?></a>
			</div>
      <?php } ?>
      <?php } ?>
      <div class="picture">
				<a
					href="<?php echo MICROSHOP_SITE_URL;?>/index.php?act=personal&op=detail&personal_id=<?php echo $value['personal_id'];?>"
					target="_blank">
        ...
        <img class="lazy" height="<?php echo $size['height'];?>"
					width="<?php echo $size['width'];?>"
					src="<?php echo MICROSHOP_TEMPLATES_URL;?>/images/loading.gif"
					data-src="<?php echo $personal_image_array[0];?>" />
				</a>
			</div>
			<div class="handle">
				<span class="like-btn"><a nc_type="microshop_like"
					like_id="<?php echo $value['personal_id'];?>"
					href="javascript:void(0)"><i class="pngFix"></i><span><?php echo $lang['microshop_text_like'];?></span><em><?php echo $value['like_count']<=999?$value['like_count']:'999+';?></em></a></span>
				<span class="comment"><a
					href="<?php echo MICROSHOP_SITE_URL;?>/index.php?act=personal&op=detail&personal_id=<?php echo $value['personal_id'];?>"
					target="_blank"><i
						title="<?php echo $lang['microshop_text_comment'];?>"
						class="pngFix">&nbsp;</i><em><?php echo $value['comment_count']<=999?$value['comment_count']:'999+';?></em></a></span>
			</div>
			<dl class="commentary">
				<dt>
					<span class="thumb size30"><i></i><a
				...
				<dd>
					...
					<p>
						<a
							href="<?php echo MICROSHOP_SITE_URL;?>/index.php?act=personal&op=detail&personal_id=<?php echo $value['personal_id'];?>"
							target="_blank"> <?php echo $value['commend_message'];?> </a>
					</p>
				</dd>
			</dl>
		</li>
  ...
  <!-- 占位用勿删 -->
    ...
    <p><?php echo $lang['microshop_personal_list_none_publish'];?></p>
		<a id="link_personal_publish" href="javascript:void(0)"><?php echo $lang['microshop_personal_publish'];?></a>
	</div>
  <?php } else { ?>
...
<script type="text/javascript"
	src="<?php echo MICROSHOP_RESOURCE_SITE_URL;?>/js/fileupload/jquery.iframe-transport.js"
	charset="utf-8"></script>
<script type="text/javascript"
	src="<?php echo MICROSHOP_RESOURCE_SITE_URL;?>/js/fileupload/jquery.ui.widget.js"
	charset="utf-8"></script>
<script type="text/javascript"
	src="<?php echo MICROSHOP_RESOURCE_SITE_URL;?>/js/fileupload/jquery.fileupload.js"
	charset="utf-8"></script>
<!-- 发布个人秀 -->
<script type="text/javascript">
$(document).ready(function(){
...
<div id="_personal_image_list" style="display: none;">
	<div class="personal_image_content">
		<a nc_type="btn_personal_image_delete" image_name=""
			href="javascript:void(0);" class="personal_image_delete"
			style="display: none;"><?php echo $lang['nc_delete'];?></a> <span
			class=" thumb size100"><i></i><img src=""
			onload="javascript:DrawImage(this,100,100);" /></span>
	</div>
</div>
</div>
<!-- 发布按钮 -->
...
<div id="publish_div" style="display: none;">
	<form
		action="<?php echo MICROSHOP_SITE_URL;?>/index.php?act=publish&op=personal_save"
		method="post" id="add_form" class="publish-div">
				...
				<div class="upload-btn">
					<a href="javascript:void(0);"> <span
						style="width: 80px; height: 30px; position: absolute; left: 0; top: 0; z-index: 999; cursor: pointer;">
							<input type="file" name="personal_image_ajax" id="fileupload"
							file_id="0"
							style="width: 80px; height: 30px; cursor: pointer; opacity: 0; filter: alpha(opacity = 0)"
							size="1" hidefocus="true" maxlength="0" />
					</span>
						<div class="upload-button">&nbsp;</div> <input id="submit_button"
						style="display: none" type="button"
						value="<?php echo $lang['microshop_text_upload_image'];?>"
						onClick="submit_form($(this))" />
					</a>
				</div>
				<div id="personal_image_info" class="personal_image_info"
					style="display: none;"
					title="<?php echo $lang['microshop_text_uploading'];?>">&nbsp;</div>
			</div>
		</div>
		<div class="express">
			<dl class="type">
          ...
          <span class="mr5"> <input
						id="personal_class_list<?php echo $value['class_id'];?>"
						name="personal_class" type="radio"
						value="<?php echo $value['class_id'];?>"
						<?php if($count == 1) echo "checked='checked'";?> /> <label
						for="personal_class_list<?php echo $value['class_id'];?>"><?php echo $value['class_name'];?></label>
					</span>
          <?php $count++;?>
          <?php } ?>
          <?php } ?>
			...
			<dl class="intro">
				...
				<dd>
					<textarea cols="55" rows="3" name="commend_message"
						id="commend_message"></textarea>
				</dd>
			</dl>
			<dl class="url">
          ...
          <?php } ?>
				...
				<dd>
					...
					<div id="div_personal_image_link" style="display: none;">
						...
						<a id="btn_personal_link_add" href="javascript:void(0);"
							class="add-link"><?php echo $lang['nc_add'];?></a>
					</div>
					<div id="personal_image_link">
						<a id="btn_personal_link" href="javascript:void(0);"
							class="add-btn"><?php echo $lang['microshop_personal_add_link'];?></a>
					</div>
				</dd>
			</dl>
		</div>
		<div class="handle">
			<div id="personal_image_notice" class="personal_image_notice"
				style="display: none;"></div>
			<input id="btn_publish" type="button"
				value="<?php echo $lang['microshop_text_commend'];?>" />
			<!-- 站外分享 -->
