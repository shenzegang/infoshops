...
<div id="widgetcommenttitle" class="widget-comment-title">
	...
	<em>(<span id="comment_count"><?php echo $output['detail']['comment_count']<=999?$output['detail']['comment_count']:'999+';?></span>)
	</em>
</div>
<?php if(!empty($_SESSION['member_id'])) { ?>
<dl class="widget-comment">
	<dt class="head-portrait">
		<span class="thumb size30"><i></i><img
	...
	<dd>
		<form id="add_form">
			<input id="comment_id" name="comment_id" type="hidden"
				value="<?php echo $output['comment_id'];?>" />
			<textarea id="comment_message" name="comment_message"
				class="comment-message"></textarea>
			<div>
				<a id="btn_show_smilies" href="javascript:void(0)" class="smile"><?php echo $lang['microshop_comment_smiles'];?></a>
				<!-- 站外分享 -->
				<input id="btn_publish_comment" class="input-button" type="button"
					value="<?php echo $lang['microshop_text_publish'];?>" />
			</div>
		</form>
	</dd>
	<div class="clear">&nbsp;</div>
...
<?php } else { ?>
<div class="login"> <?php echo $lang['no_login'];?> <a
		href="<?php echo SHOP_SITE_URL.'/index.php?act=login&ref_url='.getRefUrl();?>"><?php echo $lang['nc_login'];?></a>
</div>
<?php } ?>
...
<div id="_comment_list_template" style="display: none;">
	<dl>
		<dt class="head-portrait">
			<span class="thumb"><i></i> <a class="comment_member_info" href=""
				target="_blank"> <img src="" alt="" />
			</a> </span>
		</dt>
		<dd>
			<a class="comment_member_info" href="" target="_blank"></a> <span></span>
			<p>
				<em></em> <a nc_type="comment_drop" comment_id=""
					href="javascript:void(0)" class="del pngFix"><?php echo $lang['nc_delete'];?></a>
			</p>
		</dd>
	</dl>
</div>
