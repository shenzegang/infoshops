...
class cms_pictureControl extends SystemControl
{
    ...
    private function get_cms_picture_list($condition, $menu_key)
    {
        if (! empty($_GET['picture_title'])) {
            $condition['picture_title'] = array(
                'like',
                '%' . $_GET['picture_title'] . '%'
            );
        }
        if (! empty($_GET['picture_publisher_name'])) {
            $condition['picture_publisher_name'] = array(
                'like',
                '%' . $_GET['picture_publisher_name'] . '%'
            );
        }
    ...
    public function cms_picture_verifyOp()
    {
        if (intval($_POST['verify_state']) === 1) {
            ...
            $this->cms_picture_state_modify(self::ARTICLE_STATE_DRAFT, array(
                'picture_verify_reason' => $_POST['verify_reason']
            ));
        }
    }
    ...
    private function cms_picture_state_modify($new_state, $param = array())
    {
        ...
        $model_picture->modify($param, array(
            'picture_id' => array(
                'in',
                $picture_id
            )
    ...
    public function update_picture_sortOp()
    {
        ...
        if ($new_sort > 255) {
            echo json_encode(array(
                'result' => FALSE,
                'message' => Language::get('picture_sort_error')
            ));
            die();
        } else {
            $model_class = Model("cms_picture");
            $result = $model_class->modify(array(
                'picture_sort' => $new_sort
            ), array(
                'class_id' => $_GET['id']
            ));
            if ($result) {
    ...
    {
        $array = array();
        $array[self::ARTICLE_STATE_DRAFT] = array(
            'text' => Language::get('cms_text_draft')
        );
        $array[self::ARTICLE_STATE_VERIFY] = array(
            'text' => Language::get('cms_text_verify')
        );
        $array[self::ARTICLE_STATE_PUBLISHED] = array(
            'text' => Language::get('cms_text_published')
        );
        $array[self::ARTICLE_STATE_RECYCLE] = array(
            'text' => Language::get('cms_text_recycle')
        );
        return $array;
    }
