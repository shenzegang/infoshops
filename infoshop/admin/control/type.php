...
class typeControl extends SystemControl
{
    ...
    public function type_addOp()
    {
        ...
        $brand_list = $model_brand->getBrandList(array(
            'brand_apply' => '1',
            'order' => 'brand_class asc,brand_sort asc'
        ));
        $b_list = array();
        ...
        $spec_list = $model_spec->specList(array(
            'order' => 'sp_sort asc'
        ), '', 'sp_id,sp_name,class_id,class_name');
        $s_list = array();
    ...
    public function attr_editOp()
    {
                ...
                if (! empty($attr_value) && is_array($attr_value)) {
                    foreach ($attr_value as $key => $val) {
                        if (isset($val['form_submit']) && $val['form_submit'] == 'ok' && ! in_array(intval($key), $del_array)) { // 属性已修改
                            ...
                            if (isset($val['form_submit']) && $val['form_submit'] == '' && ! in_array(intval($key), $del_array)) { // 属性未修改
        ...
        $attr_info = $model->table('attribute')
            ->where('attr_id=' . $attr_id)
            ->find();
        Tpl::output('attr_info', $attr_info);
        
        $attr_value_list = $model->table('attribute_value')
            ->where('attr_id=' . $attr_id)
            ->order('attr_value_sort asc, attr_value_id asc')
            ->select();
