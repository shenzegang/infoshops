...
class microshopControl extends SystemControl
{
    ...
    private function class_save($type)
    {
        ...
        if (isset($_POST['class_id']) && intval($_POST['class_id']) > 0) {
            $result = $model_class->modify($param, array(
    ...
    private function update_class_sort($type)
    {
        ...
        if ($new_sort > 255) {
            ...
            $model_class = Model("micro_{$type}_class");
            $result = $model_class->modify(array(
                'class_sort' => $new_sort
            ), array(
                'class_id' => $_GET['id']
            ));
            if ($result) {
                echo json_encode(array(
                    'result' => TRUE,
                    'message' => 'class_add_success'
                ));
                die();
            } else {
                echo json_encode(array(
                    'result' => FALSE,
                    'message' => Language::get('class_add_fail')
                ));
                die();
            }
        }
    ...
    public function goodsclass_dropOp()
    {
        ...
        $model_microshop_goods_class = Model('micro_goods_class');
        $condition = array();
        $condition['class_parent_id'] = array(
            'in',
            $class_id
        );
    ...
    public function goodsclass_bindingOp()
    {
        ...
        $class_binding_list = $model_goods_relation->getList(array(
            'class_id' => $class_id
        ));
    ...
    public function goodsclass_binding_saveOp()
    {
        ...
        $model_goods_relation = Model('micro_goods_relation');
        $model_goods_relation->drop(array(
            'class_id' => $class_id
        ));
    ...
    public function goodsclass_defaultOp()
    {
        ...
        $model_goods_class->modify(array(
            'class_default' => 0
        ), TRUE);
        $result = $model_goods_class->modify(array(
            'class_default' => 1
        ), array(
            'class_id' => $class_id
        ));
    ...
    public function goods_manageOp()
    {
        ...
        if (! empty($_GET['member_name'])) {
            $condition['member_name'] = array(
                'like',
                '%' . trim($_GET['member_name']) . '%'
            );
        }
        if (! empty($_GET['commend_goods_name'])) {
            $condition['commend_goods_name'] = array(
                'like',
                '%' . trim($_GET['commend_goods_name']) . '%'
            );
        }
        if (! empty($_GET['commend_time_from']) && ! empty($_GET['commend_time_to'])) {
            $condition['commend_time'] = array(
                'between',
                strtotime($_GET['commend_time_from']) . ',' . strtotime($_GET['commend_time_to'])
            );
        }
    ...
    public function goods_dropOp()
    {
        ...
        $condition = array();
        $condition['commend_id'] = array(
            'in',
            trim($_POST['commend_id'])
        );
        // 删除随心看图片
    ...
    private function update_microshop_sort($model_name, $key_name)
    {
        ...
        if ($new_sort > 255) {
            echo json_encode(array(
                'result' => FALSE,
                'message' => Language::get('microshop_sort_error')
            ));
            die();
        } else {
            $model_class = Model($model_name);
            $result = $model_class->modify(array(
                'microshop_sort' => $new_sort
            ), array(
                $key_name => $_GET['id']
            ));
            if ($result) {
                echo json_encode(array(
                    'result' => TRUE,
                    'message' => 'nc_common_op_succ'
                ));
                die();
            } else {
                echo json_encode(array(
                    'result' => FALSE,
                    'message' => Language::get('nc_common_op_fail')
                ));
                die();
            }
        }
    ...
    public function comment_manageOp()
    {
        ...
        if (! empty($_GET['comment_message'])) {
            $condition['comment_message'] = array(
                'like',
                '%' . trim($_GET['comment_message']) . '%'
            );
        }
    ...
    public function comment_dropOp()
    {
        ...
        $condition = array();
        $condition['comment_id'] = array(
            'in',
            trim($_POST['comment_id'])
        );
        $result = $model->drop($condition);
        if ($result) {
            showMessage(Language::get('nc_common_del_succ'), '');
    ...
    public function adv_manageOp()
    {
        ...
        if (! empty($_GET['adv_type'])) {
            $condition['adv_type'] = array(
                'like',
                '%' . trim($_GET['adv_type']) . '%'
            );
        }
        if (! empty($_GET['adv_name'])) {
            $condition['adv_name'] = array(
                'like',
                '%' . trim($_GET['adv_name']) . '%'
            );
        }
    ...
    public function adv_saveOp()
    {
        ...
        if (isset($_POST['adv_id']) && intval($_POST['adv_id']) > 0) {
            $result = $model_adv->modify($param, array(
                'adv_id' => $_POST['adv_id']
            ));
        } else {
    ...
    {
        $channel_array = array();
        $channel_array[self::GOODS_FLAG] = array(
            'name' => Language::get('nc_microshop_goods'),
            'key' => 'goods'
        );
        $channel_array[self::PERSONAL_FLAG] = array(
            'name' => Language::get('nc_microshop_personal'),
            'key' => 'personal'
        );
        $channel_array[self::STORE_FLAG] = array(
            'name' => Language::get('nc_microshop_store'),
            'key' => 'store'
        );
    ...
    private function show_menu($menu_key)
    {
        $menu_array = array(
            "{$menu_key}" => array(
                ...
                'menu_url' => 'index.php?act=microshop&op=' . $menu_key
            )
        );
    ...
    private function show_menu_goods_class($menu_key)
    {
        $menu_array = array(
            'goods_class_list' => array(
                'menu_type' => 'link',
                'menu_name' => Language::get('nc_manage'),
                'menu_url' => 'index.php?act=microshop&op=goodsclass_list'
            ),
            'goods_class_add' => array(
                'menu_type' => 'link',
                'menu_name' => Language::get('nc_new'),
                'menu_url' => 'index.php?act=microshop&op=goodsclass_add'
            )
        );
        if ($menu_key == 'goods_class_edit') {
            $menu_array['goods_class_edit'] = array(
                'menu_type' => 'link',
        ...
        if ($menu_key == 'goods_class_binding') {
            $menu_array['goods_class_binding'] = array(
                'menu_type' => 'link',
    ...
    {
        $menu_array = array(
            'personal_class_list' => array(
                'menu_type' => 'link',
                'menu_name' => Language::get('nc_manage'),
                'menu_url' => 'index.php?act=microshop&op=personalclass_list'
            ),
            'personal_class_add' => array(
                'menu_type' => 'link',
                'menu_name' => Language::get('nc_new'),
                'menu_url' => 'index.php?act=microshop&op=personalclass_add'
            )
        );
        if ($menu_key == 'personal_class_edit') {
            $menu_array['personal_class_edit'] = array(
                'menu_type' => 'link',
                'menu_name' => Language::get('nc_edit'),
                'menu_url' => '###'
            );
    ...
    private function show_menu_store($menu_key)
    {
        $menu_array = array(
            'store_manage' => array(
                'menu_type' => 'link',
                'menu_name' => Language::get('nc_manage'),
                'menu_url' => 'index.php?act=microshop&op=store_manage'
            ),
            'store_add' => array(
                'menu_type' => 'link',
                'menu_name' => Language::get('nc_new'),
                'menu_url' => 'index.php?act=microshop&op=store_add'
            )
        );
        $menu_array[$menu_key]['menu_type'] = 'text';
        Tpl::output('menu', $menu_array);
    ...
    private function show_menu_adv($menu_key)
    {
        $menu_array = array(
            'adv_manage' => array(
                'menu_type' => 'link',
                'menu_name' => Language::get('nc_manage'),
                'menu_url' => 'index.php?act=microshop&op=adv_manage'
            ),
            'adv_add' => array(
                'menu_type' => 'link',
                'menu_name' => Language::get('nc_new'),
                'menu_url' => 'index.php?act=microshop&op=adv_add'
            )
        );
        
        if ($menu_key == 'adv_edit') {
            $menu_array['adv_edit'] = array(
                'menu_type' => 'link',
                'menu_name' => Language::get('nc_edit'),
                'menu_url' => '###'
            );
