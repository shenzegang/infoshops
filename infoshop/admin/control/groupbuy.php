...
class groupbuyControl extends SystemControl
{
    ...
    public function groupbuy_delOp()
    {
        ...
        $model_groupbuy = Model('groupbuy');
        $result = $model_groupbuy->delGroupbuy(array(
    ...
    public function class_saveOp()
    {
        ...
        if (empty($class_id)) {
            ...
            // 编辑
            if ($model_groupbuy_class->update($param, array(
                'class_id' => $class_id
            ))) {
                H('groupbuy', null);
    ...
    public function area_saveOp()
    {
        ...
        if (empty($area_id)) {
            ...
            // 编辑
            if ($model_groupbuy_area->update($param, array(
                'area_id' => $area_id
            ))) {
                H('groupbuy', null);
    ...
    private function show_menu($menu_key)
    {
        $menu_array = array(
            ...
            'groupbuy_setting' => array(
                'menu_type' => 'link',
                'menu_name' => '设置',
                'menu_url' => urlAdmin('groupbuy', 'groupbuy_setting')
            )
        );
        switch ($menu_key) {
            case 'class_add':
