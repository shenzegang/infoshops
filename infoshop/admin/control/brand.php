...
class brandControl extends SystemControl
{
    ...
    public function brandOp()
    {
            ...
            if (! empty($_POST['del_brand_id'])) {
                ...
                if (is_array($_POST['del_brand_id'])) {
                    foreach ($_POST['del_brand_id'] as $k => $v) {
                        $v = intval($v);
                        $brand_array = $model_brand->table('brand')
                            ->where(array(
                            'brand_id' => $v
                        ))
                            ->find();
        ...
        if (! empty($_GET['search_brand_name'])) {
            $condition['brand_name'] = array(
                'like',
                "%{$_GET['search_brand_name']}%"
            );
        }
        if (! empty($_GET['search_brand_class'])) {
            $condition['brand_class'] = array(
                'like',
                "%{$_GET['search_brand_class']}%"
            );
        }
        $condition['brand_apply'] = '1';
        
        $brand_list = $model_brand->table('brand')
            ->where($condition)
            ->order('brand_id desc')
            ->page(20)
            ->select();
    ...
    private function createExcel($data = array())
    {
        ...
        $excel_data[0][] = array(
            'styleid' => 's_title',
            'data' => L('exp_brand_cate')
        );
        $excel_data[0][] = array(
            'styleid' => 's_title',
            'data' => L('exp_brand_img')
        );
