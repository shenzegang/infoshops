...
class voucherControl extends SystemControl
{
    ...
    public function pricelistOp()
    {
        // 获得代金券金额列表
        $model = Model();
        $voucherprice_list = $model->table('voucher_price')
    ...
    public function priceaddOp()
    {
        if (chksubmit()) {
            ...
            $model = Model();
            $voucherprice_info = $model->table('voucher_price')
                ->where(array(
    ...
    public function quotalistOp()
    {
        ...
        $list = $model->table('voucher_quota')
            ->where($param)
            ->order('quota_id desc')
            ->page(10)
            ->select();
    ...
    public function templatelistOp()
    {
        ...
        $list = $model->table('voucher_template')
            ->where($param)
            ->order('voucher_t_state asc,voucher_t_id desc')
            ->page(10)
            ->select();
        Tpl::output('show_page', $model->showpage(2));
