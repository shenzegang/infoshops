...
class goods_albumControl extends SystemControl
{
    ...
    public function listOp()
    {
        ...
        $model->table('album_class,store')
            ->where($condiiton)
            ->join('inner')
            ->on('album_class.store_id=store.store_id');
        $list = $model->field('album_class.*,store.store_name')
            ->page(10)
            ->select();
        ...
        $model->cls()
            ->table('album_pic')
            ->field('aclass_id,count(*) as pcount')
            ->group('aclass_id');
    ...
    private function del_file($filename)
    {
        ...
        if (preg_match('/^(\d+_)/', $filename)) {
            ...
            $store_id = Model()->cls()
                ->table('album_pic')
                ->getfby_apic_cover($filename, 'store_id');
        }
