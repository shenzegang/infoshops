...
class circle_manageControl extends SystemControl
{
    ...
    public function circle_listOp()
    {
        ...
        if (trim($_GET['searchname']) != '') {
            $where['circle_name'] = array(
                'like',
                '%' . trim($_GET['searchname']) . '%'
            );
        }
        if (trim($_GET['searchmaster']) != '') {
            $where['circle_mastername'] = array(
                'like',
                '%' . trim($_GET['searchmaster']) . '%'
            );
        }
    ...
    public function circle_editOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($error != '') {
                ...
                if ($rs) {
                    // 更新圈子会员表 圈主信息。
                    $cminfo = $model->table('circle_member')
                        ->where(array(
    ...
    public function search_memberOp()
    {
        ...
        $count_array = $model->table('circle')
            ->field('circle_masterid,count(*) as count')
            ->where($where)
            ->group('circle_masterid')
            ->select();
        ...
        $count_array = $model->table('circle_member')
            ->field('member_id,count(*) as count')
            ->where($where)
            ->group('member_id')
            ->select();
        if (! empty($count_array)) {
