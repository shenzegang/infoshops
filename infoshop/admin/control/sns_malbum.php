...
class sns_malbumControl extends SystemControl
{
    ...
    public function class_listOp()
    {
        ...
        $ac_list = $model->table('sns_albumclass,member')
            ->field('sns_albumclass.*,member.member_name')
            ->on('sns_albumclass.member_id = member.member_id')
            ->join('left')
            ->where($where)
            ->page('10')
            ->select();
    ...
    public function pic_listOp()
    {
        ...
        if (chksubmit()) {
            ...
            $ap_list = $model->table('sns_albumpic')
                ->where($where)
                ->select();
            if (empty($ap_list)) {
