...
class sns_straceControl extends SystemControl
{
    ...
    public function stracelistOp()
    {
        ...
        if ($_POST['search_sname'] != '') {
            $where['strace_storename'] = array(
                'like',
                '%' . trim($_POST['search_sname']) . '%'
            );
        }
        if ($_POST['search_scontent'] != '') {
            $where['strace_content'] = array(
                'like',
                '%' . trim($_POST['strace_content']) . '%'
            );
        }
        ...
        if ($_POST['search_stime'] != '') {
            $where['strace_time'] = array(
                'egt',
                strtotime($_POST['search_stime'])
            );
        }
        if ($_POST['search_etime'] != '') {
            $where['strace_time'] = array(
                'elt',
                strtotime($_POST['search_etime'])
            );
        }
        // 实例化模型
    ...
    public function strace_delOp()
    {
        ...
        $model = Model();
        // 删除动态
        $rs = Model('store_sns_tracelog')->delStoreSnsTracelog(array(
            'strace_id' => array(
                'in',
                $st_id
            )
        ...
        if ($rs) {
            // 删除评论
            Model('store_sns_comment')->delStoreSnsComment(array(
                'strace_id' => array(
                    'in',
                    $st_id
                )
    ...
    public function scomm_listOp()
    {
        ...
        if ($_POST['search_uname'] != '') {
            $where['scomm_membername'] = array(
                'like',
                '%' . trim($_POST['search_uname']) . '%'
            );
        }
        if ($_POST['search_content'] != '') {
            $where['scomm_content'] = array(
                'like',
                '%' . trim($_POST['search_content']) . '%'
            );
        }
    ...
    public function scomm_delOp()
    {
        ...
        // 实例化模型
        $rs = Model('store_sns_comment')->delStoreSnsComment(array(
            'scomm_id' => array(
                'in',
                $sc_id
            )
    ...
    public function scomm_editOp()
    {
        ...
        // 实例化模型
        $rs = Model('store_sns_comment')->editStoreSnsComment(array(
            'scomm_state' => $scomm_state
        ), array(
            'scomm_id' => array(
                'in',
                $sc_id
