...
class promotion_mansongControl extends SystemControl
{
    ...
    public function mansong_listOp()
    {
        ...
        if (! empty($_GET['mansong_name'])) {
            $param['mansong_name'] = array(
                'like',
                '%' . $_GET['mansong_name'] . '%'
            );
        }
        if (! empty($_GET['store_name'])) {
            $param['store_name'] = array(
                'like',
                '%' . $_GET['store_name'] . '%'
            );
        }
    ...
    public function mansong_cancelOp()
    {
        ...
        $model_mansong = Model('p_mansong');
        $result = $model_mansong->cancelMansong(array(
    ...
    public function mansong_delOp()
    {
        $mansong_id = intval($_POST['mansong_id']);
        $model_mansong = Model('p_mansong');
        $result = $model_mansong->delMansong(array(
            'mansong_id' => $mansong_id
        ));
        if ($result) {
    ...
    private function show_menu($menu_key)
    {
        $menu_array = array(
            'mansong_list' => array(
                ...
                'menu_url' => urlAdmin('promotion_mansong', 'mansong_list')
            ),
            'mansong_quota' => array(
                'menu_type' => 'link',
                'menu_name' => Language::get('mansong_quota'),
                'menu_url' => urlAdmin('promotion_mansong', 'mansong_quota')
            ),
            'mansong_detail' => array(
                'menu_type' => 'link',
                'menu_name' => Language::get('mansong_detail'),
                'menu_url' => urlAdmin('promotion_mansong', 'mansong_detail')
            ),
            'mansong_setting' => array(
                'menu_type' => 'link',
                'menu_name' => Language::get('mansong_setting'),
                'menu_url' => urlAdmin('promotion_mansong', 'mansong_setting')
            )
        );
