...
class memberControl extends SystemControl
{
    ...
    public function memberOp()
    {
        ...
        if ($_GET['idcard_chk'] != 3) {
            switch ($_GET['idcard_chk']) {
                case 'idcard_chk_no':
                    $condition['idcard_chk'] = '0';
                    break;
                case 'idcard_chk_wait':
                    $condition['idcard_chk'] = '1';
                    break;
                case 'idcard_chk_yes':
                    $condition['idcard_chk'] = '2';
                    break;
            }
        }
        ...
        if (is_array($member_list)) {
            foreach ($member_list as $k => $v) {
            	...
            	$amounts = Model('UserService') -> getAmount($v['member_id']);
            	$member_list[$k]['available_predeposit'] = ncPriceFormat(floatval($amounts[0]) + floatval($amounts[1]));
            	$member_list[$k]['freeze_predeposit'] = $amounts[1];
            }
        }
    ...
    public function chkIdcardOp(){
    	$id = $_POST['id'];
    	Model()->table('member') -> where(array('member_id' => $id)) -> update(array('idcard_chk' => 1));
    	echo 1;
    }
    /**
     * 会员修改
    ...
    public function member_editOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($error != '') {
                ...
                $update_array['member_tel'] = trim($_POST['member_tel']);
                ...
                $update_array['idcard_chk'] = trim($_POST['idcard_chk']);
    ...
    public function member_addOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($error != '') {
                ...
                $insert_array['member_tel'] = trim($_POST['member_tel']);
    ...
    public function ajaxOp()
    {
        switch ($_GET['branch']) {
            ...
            case 'check_tel':
                $model_member = Model('member');
                $condition['member_tel'] = trim($_GET['member_tel']);
                $condition['no_member_id'] = intval($_GET['member_id']);
                $list = $model_member->infoMember($condition);
                if (empty($list)) {
                    echo 'true';
