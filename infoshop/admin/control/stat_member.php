...
class stat_memberControl extends SystemControl
{
    ...
    private $links = array(
        array(
            'url' => 'act=stat_member&op=newmember',
            'lang' => 'stat_newmember'
        ),
        array(
            'url' => 'act=stat_member&op=analyze',
            'lang' => 'stat_memberanalyze'
        ),
        array(
            'url' => 'act=stat_member&op=scale',
            'lang' => 'stat_scaleanalyze'
        ),
        array(
            'url' => 'act=stat_member&op=area',
            'lang' => 'stat_areaanalyze'
        )
    );
 ...
 // 处理后的参数
    ...
    public function newmemberOp()
    {
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            $excel_data[count($statlist['data']) + 1][] = array(
                'data' => '总计'
            );
            $excel_data[count($statlist['data']) + 1][] = array(
                'format' => 'Number',
    ...
    public function analyzeOp()
    {
        ...
        $where = array();
        $where['statm_time'] = array(
            'between',
            $searchtime_arr
        );
        // 下单量
        $where['statm_ordernum'] = array(
            'gt',
            0
        );
    ...
    public function analyzeinfoOp()
    {
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            $memberlist = $model->statByStatmember($where, $field, array(
                10,
                $countnum
            ), 0, "{$_GET['type']} desc,statm_memberid desc", 'statm_memberid');
        }
    ...
    public function showmemberOp()
    {
        ...
        if ($this->search_arr['exporttype'] == 'excel') {
            ...
            $excel_data[0][] = array(
                'styleid' => 's_title',
                'data' => L('member_index_login_time')
            );
            $excel_data[0][] = array(
                'styleid' => 's_title',
                'data' => L('member_index_last_login')
            );
            $excel_data[0][] = array(
                'styleid' => 's_title',
                'data' => L('member_index_points')
            );
            $excel_data[0][] = array(
                'styleid' => 's_title',
                'data' => L('member_index_prestore')
            );
            // data
            foreach ($member_list as $k => $v) {
                $excel_data[$k + 1][] = array(
                    'data' => $v['member_name'] . '(' . L('member_index_true_name,nc_colon') . $v['member_truename'] . ')'
                );
                $excel_data[$k + 1][] = array(
                    'data' => $v['member_time']
                );
                $excel_data[$k + 1][] = array(
                    'format' => 'Number',
                    'data' => $v['member_login_num']
                );
                $excel_data[$k + 1][] = array(
                    'data' => $v['member_login_time'] . '(IP:' . $v['member_login_ip'] . ')'
                );
                $excel_data[$k + 1][] = array(
                    'data' => $v['member_points']
                );
                $excel_data[$k + 1][] = array(
                    'data' => L('member_index_available,nc_colon') . $v['available_predeposit'] . L('currency_zh') . '(' . L('member_index_frozen,nc_colon') . $v['freeze_predeposit'] . L('currency_zh') . ')'
                );
            }
            $excel_data = $excel_obj->charset($excel_data, CHARSET);
            $excel_obj->addArray($excel_data);
        ...
        Tpl::output('show_page', $model->showpage(2));
        $this->links[] = array(
            'url' => 'act=stat_member&op=showmember',
            'lang' => 'stat_memberlist'
        );
    ...
    public function scaleOp()
    {
        ...
        if (trim($this->search_arr['membername'])) {
            $where['statm_membername'] = array(
                'like',
                "%" . trim($this->search_arr['membername']) . "%"
            );
        }
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            $statlist = $model->statByStatmember($where, $field, array(
                10,
                $countnum
            ), 0, $orderby, 'statm_memberid');
        }
        // 导出Excel
        if ($this->search_arr['exporttype'] == 'excel') {
            // 导出Excel
    ...
    public function area_listOp()
    {
        ...
        if ($this->search_arr['exporttype'] == 'excel') {
            ...
            $statlist_tmp = $model->statByOrderCommon($where, $field, array(
                10,
                $countnum
            ), 0, $orderby, 'order_common.reciver_province_id');
        }
        // 地区
