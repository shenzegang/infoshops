...
class cms_indexControl extends SystemControl
{
    ...
    public function drop_page_moduleOp()
    {
        ...
        if (empty($module_id)) {
            ...
            $model_index_module = Model('cms_index_module');
            $result = $model_index_module->drop(array(
                'module_id' => $module_id
            ));
            if (! $result) {
                $data['result'] = FALSE;
    ...
    public function update_page_module_titleOp()
    {
        ...
        if ($error != '') {
            echo json_encode(array(
                'result' => FALSE,
                'message' => '标题名称不能为空且必须小于10个字'
            ));
            die();
        } else {
            $update = array(
                'module_title' => $new_title
            );
    ...
    {
        $update = array(
            'module_style' => $_POST['module_style']
        );
        $result = $this->update_page_module($update, $_POST['module_id']);
        echo $result;
    }
    ...
    private function update_page_module($update, $module_id)
    {
        ...
        if (empty($module_id)) {
            ...
            $model_index_module = Model('cms_index_module');
            $result = $model_index_module->modify($update, array(
                'module_id' => $module_id
            ));
            if (! $result) {
                $data['result'] = FALSE;
    ...
    public function update_page_module_indexOp()
    {
        ...
        if (! empty($page_module_id_string)) {
            ...
            foreach ($page_module_id_array as $module_id) {
                $result = $model_index_module->modify(array(
                    'module_sort' => $index
                ), array(
                    'module_id' => $module_id
                ));
    ...
    {
        $model_index_module = Model('cms_index_module');
        $module_list = $model_index_module->getList(array(
            'module_state' => 1
        ), null, 'module_sort asc');
    ...
    public function save_moduleOp()
    {
        ...
        // 生成模板
        $model_module_frame = Model('cms_module_frame');
        $module_frame = $model_module_frame->getOne(array(
    ...
    public function drop_moduleOp()
    {
        ...
        $module_detail = $model_cms_module->getOne(array(
            'module_id' => $module_id
        ));
        ...
        // 删除模块数据库记录
        $model_cms_module->drop(array(
            'module_id' => $module_id
        ));
        $model_cms_index_module = Model('cms_index_module');
        $model_cms_index_module->drop(array(
    ...
    public function module_editOp()
    {
        ...
        $model_index_module = Model('cms_index_module');
        $module_detail = $model_index_module->getOne(array(
            'module_id' => $module_id
        ));
    ...
    public function save_page_moduleOp()
    {
        ...
        $model_cms_index_module = Model('cms_index_module');
        $result = $model_cms_index_module->modify(array(
            'module_content' => base64_encode(serialize($module_content))
        ), array(
