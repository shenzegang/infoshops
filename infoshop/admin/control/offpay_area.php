...
class offpay_areaControl extends SystemControl
{
    ...
    public function indexOp()
    {
        ...
        if (chksubmit()) {
            ...
            // 内置自营店ID
            $area_info = $model_parea->getAreaInfo(array(
                'store_id' => $store_id
            ));
            $data = array();
            ...
            if (! $area_info) {
                ...
                $result = $model_parea->updateArea(array(
                    'store_id' => $store_id
                ), $data);
            }
            if ($result) {
        ...
        // 取出支持货到付款的县ID
        $parea_info = $model_parea->getAreaInfo(array(
            'store_id' => $store_id
        ));
        ...
        $county_array = $model_area->getAreaList(array(
            'area_deep' => 3,
            'area_id' => array(
                'in',
                $parea_ids
        ...
        $list = $model_area->getAreaList(array(
            'area_deep' => 3
        ), 'area_parent_id,count(area_id) as child_count', 'area_parent_id');
        foreach ($list as $k => $v) {
