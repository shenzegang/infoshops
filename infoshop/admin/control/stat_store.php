...
class stat_storeControl extends SystemControl
{
    ...
    private $links = array(
        array(
            'url' => 'act=stat_store&op=newstore',
            'lang' => 'stat_newstore'
        ),
        array(
            'url' => 'act=stat_store&op=rank',
            'lang' => 'stat_storerank'
        ),
        array(
            'url' => 'act=stat_store&op=degree',
            'lang' => 'stat_storedegree'
        )
    );
    ...
    public function rankOp()
    {
        ...
        $where['payment_code'] = array(
            'exp',
            "!(payment_code='offline' and order_state <> '" . ORDER_STATE_SUCCESS . "')"
        ); // 货到付款订单，订单成功之后才计入统计
                                                                                                                        // 得到统计图数据
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            for ($i = 0; $i < 15; $i ++) {
                $stat_arr['series'][0]['data'][] = array(
                    'name' => strval($store_list[$i]['store_name']),
                    'y' => floatval($store_list[$i]['allnum'])
                );
            }
            // 构造横轴数据
            for ($i = 1; $i <= 15; $i ++) {
    ...
    public function showstoreOp()
    {
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            foreach ($store_list as $k => $v) {
                ...
                $excel_data[$k + 1][] = array(
                    'data' => $search_grade_list[$v['grade_id']]
                );
                $excel_data[$k + 1][] = array(
                    'data' => $v['store_end_time'] ? date('Y-m-d', $v['store_end_time']) : '无限制'
                );
                $excel_data[$k + 1][] = array(
                    'data' => date('Y-m-d', $v['store_time'])
                );
            }
            $excel_data = $excel_obj->charset($excel_data, CHARSET);
            $excel_obj->addArray($excel_data);
        ...
        $this->links[] = array(
            'url' => 'act=stat_store&op=showstore',
            'lang' => 'stat_storelist'
        );
