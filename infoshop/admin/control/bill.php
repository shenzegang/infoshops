...
class billControl extends SystemControl
{
    ...
    public function show_statisOp()
    {
        ...
        //sj 20150916 系统默认30天对账
        $settlement_period = C("settlement_period") == null ? 30:C("settlement_period");
        $bill_list = $model_bill->getOrderBillList($condition, '*', $settlement_period, 'ob_no desc');
        Tpl::output('bill_list', $bill_list);
        Tpl::output('show_page', $model_bill->showpage());
    ...
    public function bill_checkOp()
    {
        ...
        $update = $model_bill->editOrderBill(array(
            'ob_state' => BILL_STATE_SYSTEM_CHECK
        ), $condition);
        if ($update) {
    ...
    public function bill_payOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($update) {
                ...
                $model_store_cost->editStoreCost(array(
                    'cost_state' => 1
                ), $cost_condition);
