...
class stat_marketingControl extends SystemControl
{
    ...
    private $links = array(
        array(
            'url' => 'act=stat_marketing&op=promotion',
            'lang' => 'stat_promotion'
        ),
        array(
            'url' => 'act=stat_marketing&op=group',
            'lang' => 'stat_group'
        )
    );
 ...
 // 处理后的参数
    public function __construct()
    {
        ...
        // 处理搜索时间
        if (in_array($this->search_arr['op'], array(
            'promotion',
            'group'
        ))) {
    ...
    public function promotionOp()
    {
        ...
        if ($statlist_tmp) {
            ...
            $stat_json['ordernum'] = getStatData_Pie(array(
                'title' => '下单量',
                'name' => '下单量(%)',
                'label_show' => false,
                'series' => $stat_arr['ordernum']
            ));
            $stat_json['goodsnum'] = getStatData_Pie(array(
                'title' => '下单商品数',
                'name' => '下商品数(%)',
                'label_show' => false,
                'series' => $stat_arr['goodsnum']
            ));
            $stat_json['orderamount'] = getStatData_Pie(array(
                'title' => '下单金额',
                'name' => '下单金额(%)',
                'label_show' => false,
    ...
    public function grouplistOp()
    {
        ...
        $where['start_time'] = array(
            'exp',
            "!(end_time < {$searchtime_arr[0]} or start_time > {$searchtime_arr[1]})"
        );
        $where['state'] = array(
            'in',
            array(
    ...
    public function groupgoodsOp()
    {
        ...
        if ($this->search_arr['exporttype'] == 'excel') {
            ...
            $statlist_tmp = $model->statByOrderGoods($where, $field, array(
                10,
                $countnum
            ), 0, $orderby, 'goods_id');
        }
        ...
        foreach ((array) $statlist_tmp as $k => $v) {
            ...
            foreach ($statheader as $h_k => $h_v) {
                ...
                if ($h_v['key'] == 'goods_name') {
                    $tmp[$h_v['key']] = '<a href="' . urlShop('goods', 'index', array(
                        'goods_id' => $v['goods_id']
