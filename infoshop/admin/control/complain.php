...
class complainControl extends SystemControl
{
    ...
    const STATE_BUYER_WIN = 50;

    const STATE_SELLER_WIN = 60;
    ...
    * 买家胜的投诉列表
    */
    public function complain_buyer_win_listOp()
    {
        $this->get_complain_list(self::STATE_BUYER_WIN, 'complain_buyer_win_list');
    }

    /*
    * 卖家胜的投诉列表
    */
    public function complain_seller_win_listOp()
    {
        $this->get_complain_list(self::STATE_SELLER_WIN, 'complain_seller_win_list');
    }
    ...
    //sj 20150924 支持卖家
    public function complain_support_sellerOp()
    {
        //获取订单id 买家 卖家信息
        $complain_id = intval($_POST['complain_id']);
        ...
        $result = array();
        $result['result'] = true;
        $result['complain_id'] = $complain_id;
        $result['order_id'] = $order_id;
        $model_complain = Model('complain');
        $update_array = array();
        $where_array = array();
        //支持买家
        $update_array['complain_state'] = self::STATE_SELLER_WIN;
        ...
        $where_array['complain_id'] = $complain_id;
        //修改cor_complain状态为60
        $return = $model_complain->updateComplain($update_array, $where_array);
    ...
    //sj 20150924 支持买家
    public function complain_support_buyerOp()
    {
        // 获取输入的数据
        $complain_id = intval($_POST['complain_id']);
        $final_handle_message = trim($_POST['final_handle_message']);
        ...
        if ($current_state < 50) {
            $model_complain = Model('complain');
            $update_array = array();
            $update_array['complain_state'] = self::STATE_BUYER_WIN;
            $update_array['final_handle_message'] = $final_handle_message;
            $update_array['final_handle_datetime'] = time();
            $update_array['final_handle_member_id'] = $this->get_admin_id();
            $where_array = array();
            ...
            if ($model_complain->updateComplain($update_array, $where_array)) {
                if (intval($complain_info['complain_subject_id']) == 1) { // 退款信息
                    ...
                    if (!empty($refund_goods) && is_array($refund_goods)) {//是空
                        ...
                        foreach ($refund_goods as $key => $value) {
                            ...
                            if (!empty($checked_goods) && array_key_exists($goods_id, $checked_goods)) { // 验证提交的商品属于订单
                                $refund_array = array();
                                $refund_array['refund_type'] = $complain_info['refund_type']; // 类型:1为退款,2为退货
                                $refund_array['return_type'] = $complain_info['return_type'];// 类型:1为弃货(不需要买家退货),2为需要买家退货
                                ...
                                $refund_array['buyer_message'] = '平台仲裁结果：支持买家';
                                $refund_array['seller_message'] = '平台仲裁结果：支持买家';
                ...
                $order_id = $complain_info['order_id'];
                //sj 20150923 如果订单状态为已完成 则投诉过后 订单状态不变，否则 订单状态变为已取消
                if ($order['order_state'] == '40') {
                    ...
                    $data['order_state'] = '0';
                    $data['refund_state'] = '2';
                    $data['refund_amount'] = $order['pd_amount'];
                    $condition['order_id'] = $order_id;
                    $model_order->editOrder($data, $condition);
                }
                $this->log(L('complain_close_success') . '[' . $complain_id . ']', 1);
                showMessage(Language::get('complain_close_success'), $this->get_complain_state_link($current_state));
            } else {
                    ...
                    if (!empty($refund_goods) && is_array($refund_goods)) {
                        ...
                        foreach ($refund_goods as $key => $value) {
                            ...
                            if (!empty($checked_goods) && array_key_exists($goods_id, $checked_goods)) { // 验证提交的商品属于订单
                                ...
                                $refund_array['add_time'] = time();
                                $refund_array['seller_time'] = time();
                                $refund_array['is_force'] = '2';//平台生成的退款退货记录
                                $model_refund->addRefundReturn($refund_array, $order, $value);
                            }
                        }
                    }
    ...
    private function get_complain_state_text($complain_state)
    {
        switch (intval($complain_state)) {
            ...
            case self::STATE_HANDLE:
                ...
                break;
            case self::STATE_BUYER_WIN:
                return Language::get('complain_state_buyer_win');
                break;
            case self::STATE_SELLER_WIN:
                return Language::get('complain_state_seller_win');
                break;
    ...
    private function get_complain_state_link($complain_state)
    {
        switch (intval($complain_state)) {
            ...
            case self::STATE_HANDLE:
                ...
                break;
            case self::STATE_BUYER_WIN:
                return 'index.php?act=complain&op=complain_state_buyer_win';
                break;
            case self::STATE_SELLER_WIN:
                return 'index.php?act=complain&op=complain_state_seller_win';
                break;
            case self::STATE_FINISH:
    ...
    private function show_menu($menu_key)
    {
        $menu_array = array(
            ...
            //sj 20150918 新增买家胜 卖家胜菜单
            'complain_buyer_win_list' => array(
                'menu_type' => 'link',
                'menu_name' => Language::get('complain_buyer_win_list'),
                'menu_url' => 'index.php?act=complain&op=complain_buyer_win_list'
            ),
            'complain_seller_win_list' => array(
                'menu_type' => 'link',
                'menu_name' => Language::get('complain_seller_win_list'),
                'menu_url' => 'index.php?act=complain&op=complain_seller_win_list'
            ),
