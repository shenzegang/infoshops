...
class stat_tradeControl extends SystemControl
{
    ...
    private $links = array(
        array(
            'url' => 'act=stat_trade&op=goods',
            'lang' => 'stat_goods_ranking'
        ),
        array(
            'url' => 'act=stat_trade&op=income',
            'lang' => 'stat_sale_income'
        ),
        array(
            'url' => 'act=stat_trade&op=predeposit',
            'lang' => 'stat_predeposit'
        ),
        array(
            'url' => 'act=stat_trade&op=goods_sale',
            'lang' => 'stat_goods_sale'
        ),
        array(
            'url' => 'act=stat_trade&op=class_sale',
            'lang' => 'stat_class_sale'
        ),
        array(
            'url' => 'act=stat_trade&op=sale',
            'lang' => 'stat_sale'
        )
    );
    ...
    public function goodsOp()
    {
        ...
        $model = Model('stat');
        // 获取上架商品数量
        Tpl::output('goods_on_allnum', $model->getGoodsNum(array(
            'goods_state' => 1
        )));
        // 下单商品数
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            // header
            $excel_data[0][0] = array(
                'styleid' => 's_title',
                'data' => '排名'
            );
            $excel_data[0][1] = array(
                'styleid' => 's_title',
                'data' => '商品名称'
            );
            $excel_data[0][2] = array(
                'styleid' => 's_title',
                'data' => trim($_GET['rank_type']) == 'trade_amount' ? '销售额' : '销量'
            );
            // data
            foreach ($goods_list as $k => $v) {
                $excel_data[$k + 1][0] = array(
                    'data' => $k + 1
                );
                $excel_data[$k + 1][1] = array(
                    'data' => $v['goods_name']
                );
                $excel_data[$k + 1][2] = array(
                    'data' => $v['allnum']
                );
            }
            ...
            for ($i = 0; $i < 15; $i ++) {
                $stat_arr['series'][0]['data'][] = array(
                    'name' => strval($goods_list[$i]['goods_name']),
                    'y' => floatval($goods_list[$i]['allnum'])
                );
            }
    ...
    public function incomeOp()
    {
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            $excel_data[0][3] = array(
                'styleid' => 's_title',
                'data' => '收取佣金'
            );
            $excel_data[0][4] = array(
                'styleid' => 's_title',
                'data' => '退单金额'
            );
            $excel_data[0][5] = array(
                'styleid' => 's_title',
                'data' => '退回佣金'
            );
            $excel_data[0][6] = array(
                'styleid' => 's_title',
                'data' => '店铺费用'
            );
            $excel_data[0][7] = array(
                'styleid' => 's_title',
                'data' => '结算金额'
            );
            // data
            foreach ($bill_list as $k => $v) {
                $excel_data[$k + 1][0] = array(
                    'data' => $v['ob_store_name']
                );
                $excel_data[$k + 1][1] = array(
                    'data' => $v['member_name']
                );
                $excel_data[$k + 1][2] = array(
                    'data' => $v['ob_order_totals']
                );
                $excel_data[$k + 1][3] = array(
                    'data' => $v['ob_commis_totals']
                );
                $excel_data[$k + 1][4] = array(
                    'data' => $v['ob_order_return_totals']
                );
                $excel_data[$k + 1][5] = array(
                    'data' => $v['ob_commis_return_totals']
                );
                $excel_data[$k + 1][6] = array(
                    'data' => $v['ob_store_cost_totals']
                );
                $excel_data[$k + 1][7] = array(
                    'data' => $v['ob_result_totals']
                );
            }
            $excel_data = $excel_obj->charset($excel_data, CHARSET);
            $excel_obj->addArray($excel_data);
    ...
    public function predepositOp()
    {
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            foreach ($log_list as $k => $v) {
                $excel_data[$k + 1][] = array(
                    'data' => $v['lg_member_name']
                );
                $excel_data[$k + 1][] = array(
                    'data' => date('Y-m-d H:i:s', $v['lg_add_time'])
                );
                $excel_data[$k + 1][] = array(
                    'data' => $v['lg_av_amount']
                );
                $excel_data[$k + 1][] = array(
                    'data' => $v['lg_freeze_amount']
                );
                $excel_data[$k + 1][] = array(
                    'data' => $v['lg_admin_name']
                );
                switch ($v['lg_type']) {
                    case 'recharge':
                        $excel_data[$k + 1][] = array(
            ...
            Tpl::output('show_page', $model->showpage());
            // 总数统计部分
            $recharge_amount = $model->getPredepositInfo(array(
                'lg_type' => 'recharge',
                'lg_add_time' => array(
                    'between',
                    array(
            ...
            $order_amount = $model->getPredepositInfo(array(
                'lg_type' => 'order_pay',
                'lg_add_time' => array(
                    'between',
                    array(
            ...
            $cash_amount = $model->getPredepositInfo(array(
                'lg_type' => 'cash_pay',
                'lg_add_time' => array(
                    'between',
                    array(
            ...
            Tpl::output('stat_array', array(
                'recharge_amount' => $recharge_amount[0]['allnum'],
                'order_amount' => abs($order_amount[0]['allnum']),
                'cash_amount' => abs($cash_amount[0]['allnum'])
            ));
    ...
    public function goods_saleOp()
    {
        ...
        $where['order.order_state'] = array(
            'neq',
            ORDER_STATE_NEW
        ); // 去除未支付订单
        $where['order.refund_state'] = array(
            'exp',
            "!(order_state = '" . ORDER_STATE_CANCEL . "' and refund_state = 0)"
        ); // 没有参与退款的取消订单，不记录到统计中
        $where['order.payment_code'] = array(
            'exp',
            "!(order.payment_code='offline' and order.order_state <> '" . ORDER_STATE_SUCCESS . "')"
        ); // 货到付款订单，订单成功之后才计入统计
        if (trim($_GET['store_name'])) {
            $where['goods.store_name'] = array(
                'like',
                '%' . trim($_GET['store_name']) . '%'
            );
        }
        if (trim($_GET['goods_name'])) {
            $where['goods.goods_name'] = array(
                'like',
                '%' . trim($_GET['goods_name']) . '%'
            );
        }
        if (trim($_GET['goods_serial'])) {
            $where['goods.goods_serial'] = array(
                'like',
                '%' . trim($_GET['goods_serial']) . '%'
            );
        }
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            foreach ($goods_list as $k => $v) {
                ...
                $excel_data[$k + 1][] = array(
                    'data' => $v['goods_selltime'] == 0 ? date('Y-m-d H:i:s', $v['goods_addtime']) : date('Y-m-d H:i:s', $v['goods_selltime'])
                );
                $excel_data[$k + 1][] = array(
                    'data' => $v['gnum']
                );
            ...
            Tpl::output('goods_list', $goods_list);
            Tpl::output('show_page', $model->showpage());
            Tpl::output('brand_list', Model('brand')->getBrandList(array(
                'brand_apply' => 1
            )));
    ...
    public function class_saleOp()
    {
        ...
        // 获取销售排名数据Top10
        $condition = array();
        $condition['order.add_time'] = array(
            'between',
            array(
                $stime,
                $etime
        ...
        $condition['order.order_state'] = array(
            'neq',
            ORDER_STATE_NEW
        ); // 去除未支付订单
        $condition['order.refund_state'] = array(
            'exp',
            "!(order_state = '" . ORDER_STATE_CANCEL . "' and refund_state = 0)"
        ); // 没有参与退款的取消订单，不记录到统计中
        $condition['order.payment_code'] = array(
            'exp',
            "!(order.payment_code='offline' and order.order_state <> '" . ORDER_STATE_SUCCESS . "')"
        ); // 货到付款订单，订单成功之后才计入统计
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            for ($i = 0; $i < 15; $i ++) {
                $stat_arr['series'][0]['data'][] = array(
                    'name' => strval($data_list[$i][trim($_GET['class_type']) == 'store_class' ? 'store_name' : 'goods_name']),
                    'y' => floatval($data_list[$i]['pnum'])
                );
            }
            // 构造横轴数据
            for ($i = 1; $i <= 15; $i ++) {
    ...
    public function saleOp()
    {
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            foreach ($order_all_list as $k => $v) {
                ...
                $excel_data[$k + 1][] = array(
                    'data' => date('Y-m-d H:i:s', $v['add_time'])
                );
                $excel_data[$k + 1][] = array(
                    'data' => number_format(($v['order_amount']), 2)
                );
