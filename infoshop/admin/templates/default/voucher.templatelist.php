...
<div class="page">
	...
	<form method="get" name="formSearch">
		<input type="hidden" name="act" value="voucher"> <input type="hidden"
			name="op" value="templatelist">
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td><input type="text" id="sdate" name="sdate" class="txt date"
						value="<?php echo $_GET['sdate'];?>">~<input type="text"
						id="edate" name="edate" class="txt date"
						value="<?php echo $_GET['edate'];?>"></td>
					<th><label><?php echo $lang['nc_state'];?></label></th>
					<td><select name="state">
							<option value="0"
								<?php if(0 === intval($_GET['state'])) echo 'selected';?>><?php echo $lang['nc_status'];?></option>
          		<?php if(is_array($output['templatestate_arr'])) { ?>
          		<?php foreach($output['templatestate_arr'] as $key=>$val) { ?>
          			<option value="<?php echo $val[0];?>"
								<?php if(intval($val[0]) === intval($_GET['state'])) echo 'selected';?>><?php echo $val[1];?></option>
          		<?php } ?>
          		<?php } ?>
            </select></td>
	...
	<form id="list_form" method="post">
        ...
        <tr class="hover">
					...
					<td class="align-left"><a
						href="<?php echo urlShop('show_store','index', array('store_id'=>$val['voucher_t_store_id']));?>"
						target="_blank"><span><?php echo $val['voucher_t_storename'];?></span></a></td>
...
foreach ($output['templatestate_arr'] as $k => $v) {
                ...
                ?>
					...
					<td class="nowrap align-center"><a
						href="index.php?act=voucher&op=templateedit&tid=<?php echo $val['voucher_t_id'];?>"><?php echo $lang['nc_edit'];?></a></td>
				</tr>
        <?php } ?>
        <?php }else { ?>
        <tr class="no_data">
