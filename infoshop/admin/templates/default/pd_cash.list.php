...
<div class="page">
	...
	<form method="get" action="index.php" name="formSearch" id="formSearch">
		<input type="hidden" name="act" value="predeposit"> <input
			type="hidden" name="op" value="pd_cash_list">
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td colspan="2"><input type="text" id="stime" name="stime"
						class="txt date" value="<?php echo $_GET['stime'];?>"> <label>~</label>
						<input type="text" id="etime" name="etime" class="txt date"
						value="<?php echo $_GET['etime'];?>"></td>
				</tr>
				<tr>
					<th><?php echo $lang['admin_predeposit_cash_shoukuanname'];?></th>
					<td><input type="text" name="pdc_bank_user" class="txt"
						value='<?php echo $_GET['pdc_bank_user'];?>'></td>
					<th><?php echo $lang['admin_predeposit_paystate']?></th>
					<td><select id="paystate_search" name="paystate_search">
							<option value=""><?php echo $lang['nc_please_choose']; ?></option>
							<option value="0" <?php if($_GET['paystate_search'] == '0' ) { ?>
								selected="selected" <?php } ?>>未支付</option>
							<option value="1" <?php if($_GET['paystate_search'] == '1' ) { ?>
								selected="selected" <?php } ?>>已支付</option>
	...
	<div style="text-align: right;">
		<a class="btns" target="_blank"
			href="index.php?<?php echo $_SERVER['QUERY_STRING'];?>&op=export_cash_step1"><span><?php echo $lang['nc_export'];?>Excel</span></a>
	</div>
	<table class="table tb-type2 nobdb">
      ...
      <tr class="hover">
          ...
          <a href="javascript:void(0)"
					onclick="if(confirm('<?php echo $lang['nc_ensure_del']; ?>')){window.location='index.php?act=predeposit&op=pd_cash_del&id=<?php echo $v['pdc_id']; ?>';}else{return false;}"><?php echo $lang['nc_del']; ?></a> 
          <?php } ?><a
					href="index.php?act=predeposit&op=pd_cash_view&id=<?php echo $v['pdc_id']; ?>"
