	...
	<div class="fixed-bar">
		<div class="item-title">
			...
			<ul class="tab-base">
				...
				<li><a
					href="index.php?act=adv&op=adv_add&ap_id=<?php echo $_GET['ap_id'];?>"><span><?php echo $lang['adv_add'];?></span></a></li>
			</ul>
		</div>
	</div>
	...
	<form method="post" id="store_form">
        ...
        <tr class="hover">
					<td class="w24"><input type="checkbox" class="checkitem"
						name="del_id[]" value="<?php echo $v['adv_id']; ?>" /></td>
          ...
          <td class="align-center nowrap"><?php echo date('Y-m-d',$v['adv_start_date']); ?></td>
					...
					<td class="w72 align-center"><a
						href="index.php?act=adv&op=adv_edit&adv_id=<?php echo $v['adv_id'];?>"><?php echo $lang['adv_edit'];?></a>&nbsp;|&nbsp;<a
						href="javascript:if(confirm('<?php echo $lang['nc_ensure_del'];?>'))window.location = 'index.php?act=adv&op=adv_del&adv_id=<?php echo $v['adv_id'];?>';"><?php echo $lang['nc_del'];?></a></td>
				</tr>
        <?php } ?>
        <?php }else { ?>
        <tr class="no_data">
			...
			<tfoot>
				<tr class="tfoot">
					...
					<td id="batchAction" colspan="15"><span class="all_checkbox"> <label
						...
						onclick="if(confirm('<?php echo $lang['adv_del_sure'];?>')){$('#store_form').submit();}"><span><?php echo $lang['nc_del'];?></span></a>
