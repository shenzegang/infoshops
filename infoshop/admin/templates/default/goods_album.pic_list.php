...
<div class="page">
	...
	<form method="get" name="formSearch" action="index.php">
		<input type="hidden" name="act" value="goods_album"> <input
			type="hidden" name="op" value="pic_list">
	...
	<form method='post' action="index.php" name="picForm" id="picForm">
		<input type="hidden" name="act" value="goods_album" /> <input
			type="hidden" name="op" value="del_more_pic" />
        ...
        <?php foreach($output['list'] as $k => $v){ ?>
          <li class="picture">
				...
				<?php if($v['apic_cover'] != ''){ ?>
				<a nctype="nyroModal"
										href="<?php echo cthumb($v['apic_cover'], 1280, $v['store_id']);?>"
										rel="gal"><img
											title="<?php echo date('Y-m-d',$v['upload_time']);?><br/>
											<?php echo $v['apic_spec'];?> px<br /><?php echo number_format($v['apic_size']/1024,2);?>k" width="64" height="64"  class="tip" src="<?php echo cthumb($v['apic_cover'], 60, $v['store_id']);?>"></a>
				<?php }else{?>
				<img height="64"
				...
				<?php }?>
								...
								<p>
									<span><input class="checkitem" type="checkbox" name="delbox[]"
										value="<?php echo $v['apic_id'];?>"></span><span><a
										href="javascript:void(0);" nc_type="delete"
										nc_key="<?php echo $v['apic_id'].'|'.$v['apic_cover'];?>"><?php echo $lang['nc_del'];?></a></span>
								</p>
							</li>
          <?php } ?>
      	</ul></td>
        ...
        <?php } ?>
			...
			<tfoot>
				<tr class="tfoot">
					<td class="w48"><input id="checkallBottom" class="checkall"
						type="checkbox" /></td>
