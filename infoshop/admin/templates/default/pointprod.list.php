...
<div class="page">
	...
	<form method="get" name="formSearch">
		<input type="hidden" name="act" value="pointprod"> <input
			type="hidden" name="op" value="pointprod">
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					<th><label for="pg_name"><?php echo $lang['admin_pointprod_goods_name']; ?></label></th>
					<td><input type="text" name="pg_name" id="pg_name" class="txt"
						value='<?php echo $_GET['pg_name'];?>'></td>
					<td><select name="pg_state">
							<option value=""
								<?php if (!$_GET['pg_state']){echo 'selected=selected';}?>><?php echo $lang['admin_pointprod_state']; ?></option>
							<option value="show"
								<?php if ($_GET['pg_state'] == 'show'){echo 'selected=selected';}?>><?php echo $lang['admin_pointprod_show_up']; ?></option>
							<option value="nshow"
								<?php if ($_GET['pg_state'] == 'nshow'){echo 'selected=selected';}?>><?php echo $lang['admin_pointprod_show_down']; ?></option>
							<option value="commend"
								<?php if ($_GET['pg_state'] == 'commend'){echo 'selected=selected';}?>><?php echo $lang['admin_pointprod_commend']; ?></option>
					</select></td>
	...
	<form method='post' id="form_prod" action="index.php">
		<input type="hidden" name="act" value="pointprod"> <input
			type="hidden" id="list_op" name="op" value="prod_dropall">
        ...
        <tr class="hover">
					<td class="w24"><input type="checkbox" name="pg_id[]"
						value="<?php echo $v['pgoods_id'];?>" class="checkitem"></td>
					<td class="w48 picture"><div class="size-44x44">
							<span class="thumb size-44x44"><i></i><img height="44" width="44"
								src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_POINTPROD.DS.str_ireplace('.', '_small.', $v['pgoods_image']); ?>"
								onload="javascript:DrawImage(this,44,44);" /></span>
						</div></td>
					<td><a
						href="<?php echo urlShop('pointprod', 'pinfo', array('id' => $v['pgoods_id']));?>"
						target="_blank"><?php echo $v['pgoods_name'];?></a></td>
            ...
            <a href="JavaScript:void(0);" class=" disabled"
						ajax_branch='pgoods_show' nc_type="inline_edit"
						fieldname="pgoods_show" fieldid="<?php echo $v['pgoods_id']?>"
						fieldvalue="0" title="<?php echo $lang['nc_editable'];?>"><img
							src="<?php echo ADMIN_TEMPLATES_URL;?>/images/transparent.gif"></a>
            <?php }else { ?>
            <a href="JavaScript:void(0);" class=" enabled"
						ajax_branch='pgoods_show' nc_type="inline_edit"
						fieldname="pgoods_show" fieldid="<?php echo $v['pgoods_id']?>"
						fieldvalue="1" title="<?php echo $lang['nc_editable'];?>"><img
							src="<?php echo ADMIN_TEMPLATES_URL;?>/images/transparent.gif"></a>
            <?php } ?></td>
					<td class="align-center yes-onoff"><?php if($v['pgoods_commend'] == '0'){ ?>
            <a href="JavaScript:void(0);" class=" disabled"
						ajax_branch='pgoods_commend' nc_type="inline_edit"
						fieldname="pgoods_commend" fieldid="<?php echo $v['pgoods_id']?>"
						fieldvalue="0" title="<?php echo $lang['nc_editable'];?>"><img
							src="<?php echo ADMIN_TEMPLATES_URL;?>/images/transparent.gif"></a>
            <?php }else { ?>
            <a href="JavaScript:void(0);" class=" enabled"
						...
						fieldvalue="1" title="<?php echo $lang['nc_editable'];?>"><img
							src="<?php echo ADMIN_TEMPLATES_URL;?>/images/transparent.gif"></a>
            <?php } ?></td>
					<td class="w72 align-center"><a
						href="index.php?act=pointprod&op=prod_edit&pg_id=<?php echo $v['pgoods_id']; ?>"
						class="edit"><?php echo $lang['nc_edit']; ?></a> | <a
						href="javascript:void(0)"
						onclick="if(confirm('<?php echo $lang['nc_ensure_del']; ?>')){window.location='index.php?act=pointprod&op=prod_drop&pg_id=<?php echo $v['pgoods_id']; ?>';}else{return false;}"><?php echo $lang['nc_del']; ?></a></td>
				</tr>
        <?php } ?>
        <?php }else { ?>
        <tr class="no_data">
			...
			<tfoot>
				<tr>
					...
					<td colspan="16" id="dataFuncs"><label for="checkallBottom"><?php echo $lang['nc_select_all']; ?></label>
						...
						onclick="submit_form('prod_dropall');"><span><?php echo $lang['nc_del']?></span></a>
