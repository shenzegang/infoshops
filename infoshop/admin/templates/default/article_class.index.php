	...
	<form method='post'>
        ...
        <tr class="hover edit">
            ...
            <?php } ?></td>
					<td class="sort"><span title="<?php echo $lang['nc_editable'];?>"
						...
						nc_type="inline_edit" class="editable"><?php echo $v['ac_sort'];?></span></td>
					<td class="name"><span title="<?php echo $lang['nc_editable'];?>"
						required="1" fieldid="<?php echo $v['ac_id'];?>"
						...
						href="index.php?act=article_class&op=article_class_add&ac_parent_id=<?php echo $v['ac_id'];?>"><span><?php echo $lang['nc_add_sub_class'];?></span></a></td>
					<td class="align-center"><a
						href="index.php?act=article_class&op=article_class_edit&ac_id=<?php echo $v['ac_id'];?>"><?php echo $lang['nc_edit'];?></a>
            <?php if($v['ac_code'] == ''){?>
            | <a
						href="javascript:if(confirm('<?php echo $lang['article_class_index_ensure_del'];?>'))window.location = 'index.php?act=article_class&op=article_class_del&ac_id=<?php echo $v['ac_id'];?>';"><?php echo $lang['nc_del'];?></a>
            <?php }?></td>
				</tr>
        <?php } ?>
        <?php }else { ?>
        ...
        <tr>
					...
					<td colspan="16"><label for="checkall_2"><?php echo $lang['nc_select_all'];?></label>
						...
						onclick="if(confirm('<?php echo $lang['article_class_index_ensure_del'];?>')){$('form:first').submit();}"><span><?php echo $lang['nc_del'];?></span></a></td>
				</tr>
        <?php } ?>
      </tfoot>
		</table>
...
<script type="text/javascript"
	src="<?php echo RESOURCE_SITE_URL;?>/js/jquery.article_class.js"
