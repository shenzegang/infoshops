...
<div class="page">
	...
	<form id="goods_class_form" name="goodsClassForm"
		enctype="multipart/form-data" method="post">
		<input type="hidden" name="form_submit" value="ok" /> <input
			type="hidden" name="gc_id"
			value="<?php echo $output['class_array']['gc_id'];?>" /> <input
			type="hidden" name="gc_parent_id" id="gc_parent_id"
			value="<?php echo $output['class_array']['gc_parent_id'];?>" /> <input
			type="hidden" name="old_type_id"
			value="<?php echo $output['class_array']['type_id'];?>">
		<table class="table tb-type2">
			<tbody>
				<tr class="noborder">
					<td colspan="2" class="required"><label class="gc_name validation"
						for="gc_name"><?php echo $lang['goods_class_index_name'];?>:</label></td>
				</tr>
				<tr class="noborder">
					<td class="vatop rowform"><input type="text" maxlength="20"
						value="<?php echo $output['class_array']['gc_name'];?>"
						name="gc_name" id="gc_name" class="txt"></td>
					<td class="vatop tips"></td>
				</tr>
				<!--2015-8-31 数据库中未发现此字段 程序中未发现有使用的地方 暂时删除此模块-->
				<!--
                <tr class="noborder">
                    <td colspan="2" class="required"><label class="validation"
                                                            for="gc_sku"><?php /*echo $lang['goods_class_index_sku'];*/?>:</label></td>
                </tr>
                <tr class="noborder">
                    <td class="vatop rowform"><input type="text" value="<?php /*echo $output['class_array']['gc_sku'];*/?>"
                                                     name="gc_sku" id="gc_sku" maxlength="20" class="txt"></td>
                    <td class="vatop tips"></td>
                </tr>-->
				<tr>
				...
				<tr class="noborder">
					<td class="vatop rowform"><span class="type-file-show"><img
							...
							<div class="type-file-preview">
								<img src="<?php echo $output['class_array']['pic'];?>">
							...
							class='type-file-button' /> <input name="pic" type="file"
							class="type-file-file" id="pic" size="30" hidefocus="true"
							nc_type="change_pic"> </span></td>
					<td class="vatop tips"><?php echo '只有第一级分类可以上传图片，建议用16px * 16px，超出后自动隐藏';?></td>
				</tr>
				<tr>
					<td colspan="2" class="required" id="gcategory"><label
						for="gc_name"><?php echo $lang['goods_class_add_type'];?>:</label>
            ...
            <?php echo $lang['nc_quickly_targeted'];?>
				...
				<tr class="noborder">
					<td class="vatop rowform"><input type="hidden" name="t_name"
						id="t_name"
						value="<?php echo $output['class_array']['type_name'];?>" /> <input
						type="hidden" name="t_sign" id="t_sign" value="" />
						<div
							style="position: relative; max-height: 240px; overflow: hidden;"
							id="type_div">
							<div>
								<dl style="margin: 10px 0;">
									<dd style="display: inline-block; margin-right: 10px;">
										<input type="radio" name="t_id" value="0"
											<?php if($output['class_array']['type_id'] == 0){?>
											checked="checked" <?php }?> /> <span><?php echo $lang['goods_class_null_type'];?></span>
									</dd>
								</dl>
            ...
            <dl style="margin: 10px 0;">
									<dt id="type_dt_<?php echo $k;?>" style="font-weight: 600;"><?php echo $val['name']?></dt>
            <?php foreach($val['type'] as $v){?>
            <dd style="display: inline-block; margin-right: 10px;">
										<input type="radio" name="t_id"
											value="<?php echo $v['type_id']?>"
											<?php if($output['class_array']['type_id'] == $v['type_id']){?>
											checked="checked" <?php }?> /> <span><?php echo $v['type_name'];?></span>
									</dd>
            <?php }?>
            </dl>
            <?php }?>
            ...
            <?php }?>
						...
						<div class=" mtm">
							<input type="checkbox" name="t_associated" value="1"
								checked="checked" id="t_associated" /><label for="t_associated"><?php echo $lang['goods_class_edit_related_to_subclass'];?></label>
						</div></td>
					<td class="vatop tips"><?php echo $lang['goods_class_add_type_desc_one'];?><a
						onclick="window.parent.openItem('type,type,goods')"
						href="JavaScript:void(0);"><?php echo $lang['nc_type_manage'];?></a><?php echo $lang['goods_class_add_type_desc_two'];?></td>
				</tr>
				<tr>
				...
				<tr class="noborder">
					<td class="vatop rowform"><input type="text"
						value="<?php echo $output['class_array']['gc_sort'] == ''?0:$output['class_array']['gc_sort'];?>"
						name="gc_sort" id="gc_sort" class="txt"></td>
