...
<div class="page">
	...
	<form id="add_form" method="post" enctype="multipart/form-data"
		action="index.php?act=microshop&op=goodsclass_save">
		<input name="class_id" type="hidden"
			value="<?php echo $output['class_info']['class_id'];?>" />
		<table class="table tb-type2">
			<tbody>
				<tr class="noborder">
					<td colspan="2" class="required"><label class="validation"
						for="class_name"><?php echo $lang['microshop_class_name'].$lang['nc_colon'];?></label></td>
				</tr>
				<tr class="noborder">
					<td class="vatop rowform"><input type="text"
						value="<?php if(isset($output['class_info']['class_name'])) echo $output['class_info']['class_name'];?>"
						name="class_name" id="class_name" class="txt"></td>
        ...
        <tr>
				...
				<tr class="noborder">
					<td class="vatop rowform"><select id="class_parent_id"
						name="class_parent_id" class="valid">
							<option value="0"><?php echo $lang['nc_common_pselect'];?></option>
              <?php if(!empty($output['list']) && is_array($output['list'])) {?>
              <?php foreach($output['list'] as $key=>$val) {?>
              <option value="<?php echo $val['class_id'];?>"
								<?php if($output['class_parent_id'] == $val['class_id']) echo 'selected';?>><?php echo $val['class_name'];?></option>
              <?php } ?>
              <?php } ?>
            </select></td>
        ...
        <tr>
              ...
              <img
									src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_MICROSHOP.DS.'default_goods_class_image.gif';?>">
              <?php } else { ?>
              <img
									src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_MICROSHOP.DS.$output['class_info']['class_image'];?>">
              <?php } ?>
            </div>
					</span> <span class="type-file-box"> <input name="old_class_image"
							type="hidden"
							value="<?php echo $output['class_info']['class_image'];?>" /> <input
							name="class_image" type="file" class="type-file-file"
							id="class_image" size="30" hidefocus="true"
							nc_type="microshop_goods_class_image">
					</span></td>
					<td class="vatop tips"></td>
				</tr>
        ...
        <tr class="class_keyword">
					<td colspan="2" class="required"><label for="class_sort"
						class="validation"><?php echo $lang['microshop_class_keyword'].$lang['nc_colon'];?></label></td>
				</tr>
				<tr class="noborder class_keyword">
					<td class="vatop rowform"><textarea name="class_keyword" rows="25"
							cols="30"><?php echo empty($output['class_info']['class_keyword'])?'':$output['class_info']['class_keyword'];?></textarea></td>
					<td class="vatop tips"><?php echo $lang['class_keyword_explain'];?></td>
				</tr>
        <?php } ?>
        <tr>
					<td colspan="2" class="required"><label for="class_sort"
						class="validation"><?php echo $lang['nc_sort'].$lang['nc_colon'];?></label></td>
				</tr>
				<tr class="noborder">
					<td class="vatop rowform"><input id="class_sort" name="class_sort"
						type="text" class="txt"
						value="<?php echo !isset($output['class_info'])?'255':$output['class_info']['class_sort'];?>" /></td>
