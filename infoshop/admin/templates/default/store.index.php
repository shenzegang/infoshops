...
<div class="page">
	...
	<form method="get" name="formSearch" id="formSearch">
		<input type="hidden" value="store" name="act"> <input type="hidden"
			value="store" name="op">
		<table class="tb-type1 noborder search">
          ...
          <option value="<?php echo $v['sg_id'];?>"
								<?php if($output['grade_id'] == $v['sg_id']){?> selected
								<?php }?>><?php echo $v['sg_name'];?></option>
          <?php } ?>
          <?php } ?>
        </select></td>
					<th><label for="owner_and_name"><?php echo $lang['store_user'];?></label></th>
					<td><input type="text"
						value="<?php echo $output['owner_and_name'];?>"
                ...
                <option value="<?php echo $k;?>"
								<?php if($_GET['store_type'] == $k){?> selected <?php }?>><?php echo $v;?></option>
                <?php } ?>
                <?php } ?>
            </select></td>
					<th><label for="store_name"><?php echo $lang['store_name'];?></label></th>
					<td><input type="text" value="<?php echo $output['store_name'];?>"
        ...
        <a href="index.php?act=store&op=store" class="btns "
	...
	<form method="post" id="store_form">
		...
		<table class="table tb-type2">
			<thead>
				<tr class="thead">
					...
					<th class="align-center">本月已发送短信</th>
        ...
        <tr class="hover edit <?php echo getStoreStateClassName($v);?>">
                ...
                <?php if($v['store_id'] == DEFAULT_PLATFORM_STORE_ID) {echo '(平台)';}?>
					...
					<td class="align-center w72"><?php echo $v['sms_count'];?></td>
					<td class="align-center w120"><a
						href="index.php?act=store&op=store_joinin_detail&member_id=<?php echo $v['member_id'];?>">查看</a>&nbsp;&nbsp;<a
						href="index.php?act=store&op=store_edit&store_id=<?php echo $v['store_id']?>"><?php echo $lang['nc_edit'];?></a>&nbsp;&nbsp;
                <?php if($v['store_id'] != DEFAULT_PLATFORM_STORE_ID) {?>
                <a
						href="index.php?act=store&op=store_bind_class&store_id=<?php echo $v['store_id']?>">经营类目</a>
                <?php } ?>
            </td>
				</tr>
        <?php } ?>
