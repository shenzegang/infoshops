...
<div class="page">
	...
	<form method="get" name="formSearch">
		<input type="hidden" value="microshop" name="act"> <input
			type="hidden" value="personal_manage" name="op">
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					<th><label for="personal_id"><?php echo $lang['microshop_text_id'];?></label></th>
					<td><input type="text" value="<?php echo $_GET['personal_id'];?>"
						name="personal_id" class="txt"></td>
	...
	<form method="post" id="list_form">
        ...
        <tr class="hover edit">
					<td><input type="checkbox"
						value="<?php echo $value['personal_id'];?>" name="del_id[]"
						class="checkitem"></td>
					<td class="w48 sort"><span nc_type="microshop_sort"
						column_id="<?php echo $value['personal_id'];?>"
						title="<?php echo $lang['nc_editable'];?>" class="editable "><?php echo $value['microshop_sort'];?></span></td>
            ...
            <a href="<?php echo MICROSHOP_SITE_URL;?>/index.php?act=personal&op=detail&personal_id=<?php echo $value['personal_id'];?>" style="background:url(<?php echo $personal_image_array[$i];?>) no-repeat center center; width:60px; height:60px; float:left; margin-right:5px;" target="_blank">
							...
							<div class="type-file-preview">
								<img src="<?php echo $personal_image_array_240[$i];?>" />
							</div>
					</a>
            <?php } ?></td>
					<td><a
						href="<?php echo MICROSHOP_SITE_URL.DS.'index.php?act=personal&op=detail&personal_id='.$value['personal_id'];?>"
						target="_blank"> <?php echo $value['personal_id'];?> </a></td>
					<td><a
						href="<?php echo MICROSHOP_SITE_URL.DS.'index.php?act=home&member_id='.$value['commend_member_id'];?>"
						target="_blank"> <?php echo $value['member_name'];?> </a></td>
					<td><?php echo $value['commend_message'];?></td>
					<td class="align-center"><?php echo date('Y-m-d',$value['commend_time']);?></td>
					<td class="yes-onoff align-center"><a href="JavaScript:void(0);"
						class=" <?php echo $value['microshop_commend']? 'enabled':'disabled'?>"
						ajax_branch='personal_commend' nc_type="inline_edit"
						fieldname="microshop_commend"
						fieldid="<?php echo $value['personal_id']?>"
					...
					<td class="align-center"><a href="###"
						onclick="submit_delete(<?php echo $value['personal_id'];?>)"><?php echo $lang['nc_del'];?></a></td>
				</tr>
        <?php } ?>
        <?php }else { ?>
        <tr class="no_data">
