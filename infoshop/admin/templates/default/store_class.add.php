			...
			<tbody>
				<tr class="noborder">
					<td colspan="2" class="required"><label class="validation"
						for="sc_name"><?php echo $lang['store_class_name'];?>:</label></td>
				</tr>
				<tr class="noborder">
				...
				<tr class="noborder">
					<td class="vatop rowform"><select name="sc_parent_id"
						id="sc_parent_id">
							<option value="0"><?php echo $lang['nc_please_choose'];?>...</option>
              <?php if(!empty($output['parent_list']) && is_array($output['parent_list'])){ ?>
              <?php foreach($output['parent_list'] as $k => $v){ ?>
              <option
								<?php if($output['sc_parent_id'] == $v['sc_id']){ ?>
								selected='selected' <?php } ?> value="<?php echo $v['sc_id'];?>"><?php echo $v['sc_name'];?></option>
              <?php } ?>
              <?php } ?>
            </select></td>
