...
<div class="page">
	...
	<form id="type_form" method="post">
		...
		<input type="hidden" name="form_submit" value="ok" /> <input
			type="hidden" name="t_id"
			value="<?php echo $output['type_info']['type_id'];?>" />
		<table class="table tb-type2">
			<tbody>
				<tr class="noborder">
					<td class="required" colspan="2"><label class="validation"
						for="t_mane"><?php echo $lang['type_index_type_name'].$lang['nc_colon'];?></label></td>
				</tr>
				<tr class="noborder">
					<td class="vatop rowform"><input type="text" class="txt"
						name="t_mane" id="t_mane"
				...
				<tr class="noborder">
					<td class="vatop rowform" id="gcategory"><input type="hidden"
						value="<?php echo $output['type_info']['class_id'];?>"
						class="mls_id" name="class_id" /> <input type="hidden"
						value="<?php echo $output['type_info']['class_name'];?>"
						class="mls_name" name="class_name" /> <span class="mr10"><?php echo $output['type_info']['class_name'];?></span>
            <?php if (!empty($output['type_info']['class_id'])) {?>
            <input class="edit_gcategory" type="button"
						value="<?php echo $lang['nc_edit'];?>">
            <?php }?>
            <select
						<?php if (!empty($output['type_info']['class_id'])) {?>
						style="display: none;" <?php }?> class="class-select">
              ...
              <?php } ?>
				...
				<tr>
					<td class="required" colspan="2"><label class="validation"
						for="t_sort"><?php echo $lang['nc_sort'].$lang['nc_colon'];?></label></td>
				</tr>
				<tr class="noborder">
					<td class="vatop rowform"><input type="text" class="txt"
						name="t_sort" id="t_sort"
		...
		<div
			style="width: 49%; float: left; margin: 10px 0; border: solid #DEEFFB; border-width: 0 0 1px 0;">
			<table class="table tb-type2">
				<thead class="thead">
					<tr class="space">
						<th class="required" colspan="15"><label
							...
							<input type="hidden" name="spec_checkbox" id="spec_checkbox"
							value="" />
                  ...
                  <?php } ?>
					...
					<tr>
						<th style="text-align: right;" colspan="15"><a class="btns"
							nctype="spec_hide" href="javascript:viod(0);"><span><?php echo $lang['type_common_checked_hide'];?></span></a></th>
					</tr>
				</thead>
			</table>
			<div style="position: relative; max-height: 240px; overflow: hidden;"
				...
				<table class="table tb-type2" id="spec_table">
					<input type="hidden" value="" name="spec[form_submit]"
            ...
            <tr class="hover edit">
              ...
              <?php foreach($val['spec'] as $v){?>
                <li class="left w33pre h36"><input class="checkitem"
										nc_type="change_default_spec_value" type="checkbox"
										<?php if(in_array($v['sp_id'], $output['spec_related'])){ echo 'checked="checked"';}?>
          ...
          <tbody>
						<tr>
							<td class="tips" colspan="15"><?php echo $lang['type_add_spec_null_one'];?><a
								...
								onclick="window.parent.openItem('spec,spec,goods')"><?php echo $lang['nc_spec_manage'];?></a><?php echo $lang['type_add_spec_null_two']?></td>
						</tr>
					</tbody>
          <?php }?>
        </table>
		...
		<div
			style="width: 49%; float: right; margin: 10px 0; border: solid #DEEFFB; border-width: 0 0 1px 0;">
			<table class="table tb-type2">
				<thead class="thead">
					<tr class="space">
                  ...
                  <?php } ?>
					...
					<tr>
						<th style="text-align: right;" colspan="15"><a class="btns"
							nctype="brand_hide" href="javascript:viod(0);"><span><?php echo $lang['type_common_checked_hide'];?></span></a></th>
					</tr>
				</thead>
			</table>
			<div style="position: relative; max-height: 240px; overflow: hidden;"
          ...
          <tbody>
						<input type="hidden" value="" name="brand[form_submit]"
							nc_type="submit_value" />
          ...
          <tr class="hover edit">
                ...
                <li class="left w33pre h36"><input type="checkbox"
										...
										value="<?php echo $bval['brand_id']?>"
										<?php if(in_array($bval['brand_id'], $output['brang_related'])){ echo 'checked="checked"';}?>
										id="brand_<?php echo $bval['brand_id'];?>" /> <label
										for="brand_<?php echo $bval['brand_id'];?>"><?php echo $bval['brand_name']?></label>
									</li>
                <?php }?>
                <?php }?>
              </ul></td>
          ...
          <tbody>
						<tr>
							<td class="tips" colspan="15"><?php echo $lang['type_add_brand_null_one'];?><a
								href="JavaScript:void(0);"
								onclick="window.parent.openItem('brand,brand,goods')"><?php echo $lang['nc_brand_manage'];?></a><?php echo $lang['type_add_brand_null_two']?></td>
						</tr>
					</tbody>
          <?php }?>
        </table>
        ...
        <tr class="hover edit">
					<input type="hidden" value=""
						name="at_value[<?php echo $aval['attr_id'];?>][form_submit]"
						nc_type="submit_value" />
					<input type="hidden" value="<?php echo $aval['attr_id'];?>"
						name="at_value[<?php echo $aval['attr_id'];?>][a_id]"
						nc_type='ajax_attr_id' />
					<td class="w48"><input type="checkbox"
						name="a_del[<?php echo $aval['attr_id'];?>]"
						value="<?php echo $aval['attr_id'];?>" /></td>
					<td class="w48 sort"><input type="text"
						class="change_default_submit_value"
						name="at_value[<?php echo $aval['attr_id'];?>][sort]"
						value="<?php echo $aval['attr_sort'];?>" /></td>
					<td class="w25pre name"><input type="text"
						class="change_default_submit_value"
					...
					<td class="align-center power-onoff"><input type="checkbox"
						class="change_default_submit_value"
						<?php if($aval['attr_show'] == '1'){echo 'checked="checked"';}?>
						nc_type="checked_show" /> <input type="hidden"
						name="at_value[<?php echo $aval['attr_id'];?>][show]"
						value="<?php echo $aval['attr_show'];?>" /></td>
					<td class="w60 align-center"><a
						href="index.php?act=type&op=attr_edit&attr_id=<?php echo $aval['attr_id'];?>"><?php echo $lang['nc_edit'];?></a></td>
				</tr>
        <?php }?>
        <?php }?>
      </tbody>
			<tbody>
				<tr>
					<td colspan="15"><a id="add_type" class="btn-add marginleft"
						href="JavaScript:void(0);"> <span><?php echo $lang['type_add_attr_add_one'];?></span>
					</a></td>
				</tr>
			</tbody>
		</table>
		<table class="table tb-type2">
			<tfoot>
				<tr class="tfoot">
					<td colspan="15"><a id="submitBtn" class="btn"
						href="JavaScript:void(0);"> <span><?php echo $lang['nc_submit'];?></span>
					</a></td>
				</tr>
			</tfoot>
		</table>
