...
<div class="page">
		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td><input type="text" value="<?php echo $_GET['class_name'];?>"
						name="class_name" id="class_name" class="txt"></td>
					<th><label for="user_name"><?php echo $lang['snsalbum_member_name'];?></label></th>
					<td><input type="text" value="<?php echo $_GET['user_name'];?>"
						name="user_name" id="user_name" class="txt"></td>
	...
	<form method='post' id="form_goods">
        ...
        <tr class="hover edit">
					<td></td>
					<td class="w60 picture"><div class="size-44x44">
							<span class="thumb size-44x44"><i></i><img
								src="<?php if($v['ac_cover'] != ''){echo UPLOAD_SITE_URL.DS.ATTACH_MALBUM.DS.$v['member_id'].DS.$v['ac_cover'];}else{echo ADMIN_SITE_URL.'/templates/'.TPL_NAME.'/images/member/default_image.png';}?>"
								onerror="this.src='<?php echo ADMIN_SITE_URL.'/templates/'.TPL_NAME.'/images/member/default_image.png';?>'"
					...
					<td class="align-center"><a
						href="index.php?act=sns_malbum&op=pic_list&id=<?php echo $v['ac_id'];?>"><?php echo $lang['nc_view'];?></a></td>
				</tr>
        <?php } ?>
        <?php }else { ?>
        <tr class="no_data">
