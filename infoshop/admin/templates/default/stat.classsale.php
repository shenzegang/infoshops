...
<div class="page">
	...
	<form method="get" action="index.php" name="formSearch" id="formSearch">
		<input type="hidden" name="act" value="stat_trade" /> <input
			type="hidden" name="op" value="class_sale" />
		<div class="w100pre" style="width: 100%;">
			<table class="tb-type1 noborder search left">
				<tbody>
					<tr>
						...
						<td><select name="class_type" id="class_type" class="querySelect">
								<option value="goods_class"
									<?php echo $_REQUEST['class_type']=='goods_class'?'selected':''; ?>>商品类目</option>
								<option value="store_class"
									<?php echo $_REQUEST['class_type']=='store_class'?'selected':''; ?>>店铺类目</option>
						</select></td>
              ...
              <option
									<?php if(intval($_GET['store_class']) == $v['sc_id']){ ?>
									selected="selected" <?php } ?>
									value="<?php echo $v['sc_id']; ?>"><?php echo $v['sc_name']; ?></option>
              <?php } ?>
              <?php } ?>
            </select></td>
	...
	<div style="text-align: right;">
		<input type="hidden" id="export_type" name="export_type"
			data-param='{"url":"<?php echo $output['actionurl'];?>&class_type=<?php echo trim($_GET['class_type']); ?>&cate_id=<?php echo intval($_GET['cate_id']); ?>&store_class=<?php echo intval($_GET['store_class']); ?>&exporttype=excel"}'
			value="excel" /> <a class="btns" href="javascript:void(0);"
			id="export_btn"><span>下载分类数据</span></a>
	</div>
