...
function make(){
    ...
    var d = DialogManager.create('tip');
	d.setTitle('消息');
	d.setContents('<div style="padding:20px; text-align:center; font-size:16px;">账单生成中...</div>');
	d.setWidth(200);
	d.show('center', 1);
    
    $.ajax({
        type: 'get',
        url: '/data/crontab/index.php',
        error: function(){
            d.setContents('<div style="padding:20px; text-align:center; font-size:16px; color:#F00;">生成失败！</div>');
            d.unlock();
        },
        success: function(data){
            if(data == 'success'){
                d.setContents('<div style="padding:20px; text-align:center; font-size:16px; color:#F00;">对账单生成成功！</div>');
                d.unlock();
            }else{
                d.setContents('<div style="padding:20px; text-align:center; font-size:16px; color:#F00;">生成失败！</div>');
                d.unlock();
	...
	<div class="fixed-bar">
		<div class="item-title">
			...
			<ul class="tab-base">
				...
				<li><a href="javascript:make();"><span>生成账单</span></a></li>
			</ul>
		</div>
	</div>
	...
	<form method="get" action="index.php" target="" name="formSearch"
		...
		<table class="tb-type1 noborder search">
			...
			<option value="<?php echo $i;?>"
								<?php if ($_GET['query_year'] == $i) {?> selected <?php } ?>><?php echo $i;?></option>
			<?php } ?>
			</select></td>
      ...
      <tr class="hover">
            ...
            <?php echo substr($bill['os_month'],0,4).'-'.substr($bill['os_month'],4);?>
				...
				<td class="align-center"><a
					href="index.php?act=bill&op=show_statis&os_month=<?php echo $bill['os_month'];?>"><?php echo $lang['nc_view'];?></a>
				</td>
			</tr>
      <?php }?>
      <?php }else{?>
