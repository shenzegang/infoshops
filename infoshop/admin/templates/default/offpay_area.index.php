...
<div class="page">
	...
	<form id="area_form" method="post">
		<input type="hidden" name="form_submit" value="ok" /> <input
			type="hidden" name="act" value="offpay_area" /> <input type="hidden"
			name="op" value="add" /> <input type="hidden" name="county"
			id="county" value="" />
        ...
        <tr>
					...
					<td><label> <input type="checkbox"
							<?php if (array_key_exists($pid, $output['province_checked_array'])) echo 'checked';?>
							value="<?php echo $pid;?>" name="province[]"> <strong><?php echo $pinfo['area_name']?></strong></label></td>
					<td><?php if (is_array($pinfo['child'])) {?>
            <?php foreach($pinfo['child'] as $city_id => $city_name) {?>
            <div class="area-list">
							<label> <input
								<?php if ($output['city_checked_array'][$city_id]) echo 'checked';?>
								type="checkbox" nc_province="<?php echo $pid;?>"
								value="<?php echo $city_id;?>" name="city[]">
                <?php echo $city_name;?> </label> (<span
								city_id="<?php echo $city_id;?>" title="已选下级地区"><?php echo count($output['city_checked_child_array'][$city_id]);?></span>)<a
								city_id="<?php echo $city_id;?>"
								nc_title="<?php echo $city_name;?>"
								province_id="<?php echo $pid;?>" nc_type="edit"
								href="javascript:void(0);" title="选择下级地区"><i class="icon-pencil"></i></a>
						</div>
            <?php }?></td>
				</tr>
        <?php }?>
