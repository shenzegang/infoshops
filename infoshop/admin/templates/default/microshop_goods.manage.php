...
<div class="page">
		...
		<div class="item-title">
			...
			<ul class="tab-base">
                ...
                <li><a href="<?php echo $menu['menu_url'];?>"
					class="current"><span><?php echo $menu['menu_name'];?></span></a></li>
                <?php }  else { ?>
	...
	<form method="get" name="formSearch">
		<input type="hidden" value="microshop" name="act"> <input
			type="hidden" value="goods_manage" name="op">
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					<th><label for="commend_id"><?php echo $lang['microshop_text_id'];?></label></th>
					<td><input type="text" value="<?php echo $_GET['commend_id'];?>"
						name="commend_id" class="txt"></td>
					<th><label for="member_name"><?php echo $lang['microshop_member'];?></label></th>
					<td><input type="text" value="<?php echo $_GET['member_name'];?>"
						name="member_name" class="txt"></td>
					<th><label for="commend_goods_name"><?php echo $lang['microshop_goods_name'];?></label></th>
					<td><input type="text"
						value="<?php echo $_GET['commend_goods_name'];?>"
						name="commend_goods_name" class="txt"></td>
					<th><label for="commend_time_from"><?php echo $lang['microshop_commend_time'];?></label></th>
					<td><input class="txt date" type="text"
						value="<?php echo $_GET['commend_time_from'];?>"
						id="commend_time_from" name="commend_time_from"> <label
						for="commend_time_to">~</label> <input class="txt date"
						type="text" value="<?php echo $_GET['commend_time_to'];?>"
						id="commend_time_to" name="commend_time_to" /></td>
			...
			<tbody>
                ...
                <tr class="hover edit">
					<td><input type="checkbox"
						value="<?php echo $value['commend_id'];?>" name="del_id[]"
						class="checkitem"></td>
					<td class="w48 sort"><span nc_type="microshop_sort"
						column_id="<?php echo $value['commend_id'];?>"
						title="<?php echo $lang['nc_editable'];?>" class="editable "><?php echo $value['microshop_sort'];?></span>
					</td>
					<td>
                        ...
                        <a href="<?php echo MICROSHOP_SITE_URL.DS.'index.php?act=goods&op=detail&goods_id='.$value['commend_id'];?>" style="background:url(<?php echo $image_url;?>) no-repeat center center; width:60px; height:60px; float:left; margin-right:5px;" target="_blank">
							...
							src="<?php echo ADMIN_TEMPLATES_URL;?>/images/transparent.gif"
							style="width: 60px; height: 60px; display: block;" />
							<div class="type-file-preview">
                                <?php $image_url_240 = cthumb($value['commend_goods_image'], 240, $value['commend_goods_store_id']);?>
                                <img src="<?php echo $image_url_240;?>"
									title="<?php echo $value['commend_goods_name'];?>"
									alt="<?php echo $value['commend_goods_name'];?>" />
							</div>
					</a>
					</td>
					<td><a
						href="<?php echo MICROSHOP_SITE_URL.DS.'index.php?act=goods&op=detail&goods_id='.$value['commend_id'];?>"
						target="_blank">
                            <?php echo $value['commend_id'];?>
                        </a></td>
					<td><a
						href="<?php echo MICROSHOP_SITE_URL.DS.'index.php?act=home&member_id='.$value['commend_member_id'];?>"
						target="_blank">
					...
					<td class="yes-onoff"><a href="JavaScript:void(0);"
						class=" <?php echo $value['microshop_commend']? 'enabled':'disabled'?>"
						ajax_branch='goods_commend' nc_type="inline_edit"
						fieldname="microshop_commend"
						fieldid="<?php echo $value['commend_id']?>"
						fieldvalue="<?php echo $value['microshop_commend']?'1':'0'?>"
						title="<?php echo $lang['editable'];?>"><img
							src="<?php echo ADMIN_TEMPLATES_URL;?>/images/transparent.gif"></a>
					</td>
					<td class="align-center"><a href="###"
						onclick="submit_delete(<?php echo $value['commend_id'];?>)"><?php echo $lang['nc_del'];?></a></td>
				</tr>
                <?php } ?>
                <?php }else { ?>
                <tr class="no_data">
			...
			<tfoot>
				<tr class="tfoot">
					...
					<td colspan="16"><label for="checkallBottom"><?php echo $lang['nc_select_all']; ?></label>
						...
						onclick="submit_batch();"><span><?php echo $lang['nc_del'];?></span></a>
