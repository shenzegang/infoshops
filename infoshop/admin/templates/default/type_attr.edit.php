...
<div class="page">
	...
	<form id="attr_form" method="post">
		<input type="hidden" name="form_submit" value="ok" /> <input
			type="hidden" name="attr_id"
			value="<?php echo $output['attr_info']['attr_id']?>" /> <input
			type="hidden" name="type_id"
			value="<?php echo $output['attr_info']['type_id']?>" />
		<table class="table tb-type2">
			<tbody>
				<tr class="noborder">
					<td class="required" colspan="2"><label class="validation"
						for="attr_name"><?php echo $lang['type_add_attr_name'];?></label></td>
				</tr>
				<tr class="noborder">
					<td class="vatop rowform"><input type="text" class="txt"
						name="attr_name" id="attr_name"
						value="<?php echo $output['attr_info']['attr_name'];?>" /></td>
					<td class="vatop tips"><?php echo $lang['type_attr_edit_name_desc'];?></td>
				</tr>
				<tr>
					<td class="required" colspan="2"><label class="validation"
						for="attr_sort"><?php echo $lang['nc_sort'];?></label></td>
				</tr>
				<tr class="noborder">
					<td class="vatop rowform"><input type="text" class="txt"
						name="attr_sort" id="attr_sort"
						value="<?php echo $output['attr_info']['attr_sort'];?>" /></td>
				...
				<tr class="noborder">
					<td class="vatop rowform onoff"><label for="attr_show1"
						class="cb-enable <?php if($output['attr_info']['attr_show'] == '1'){?>selected<?php }?>"><span><?php echo $lang['nc_yes'];?></span></label>
						<label for="attr_show0"
						class="cb-disable <?php if($output['attr_info']['attr_show'] == '0'){?>selected<?php }?>"><span><?php echo $lang['nc_no'];?></span></label>
						<input id="attr_show1" name="attr_show"
						<?php if($output['attr_info']['attr_show'] == '1'){?>
						checked="checked" <?php }?> value="1" type="radio" /> <input
						id="attr_show0" name="attr_show"
						<?php if($output['attr_info']['attr_show'] == '0'){?>
        ...
        <tr class="hover edit">
					<input type="hidden" nc_type="submit_value"
						name='attr_value[<?php echo $val['attr_value_id'];?>][form_submit]'
						value='' />
					<td class="w48"><input type="checkbox"
						name="attr_del[<?php echo $val['attr_value_id'];?>]"
						value="<?php echo $val['attr_value_id'];?>" /></td>
					<td class="w48 sort"><input type="text"
						nc_type="change_default_submit_value"
						name="attr_value[<?php echo $val['attr_value_id'];?>][sort]"
						value="<?php echo $val['attr_value_sort'];?>" /></td>
					<td class="w270 name"><input type="text"
						nc_type="change_default_submit_value"
						name="attr_value[<?php echo $val['attr_value_id'];?>][name]"
						value="<?php echo $val['attr_value_name'];?>" /></td>
					<td></td>
        ...
        <?php }?>
			...
			<tbody>
				<tr>
					<td colspan="15"><a class="btn-add marginleft" id="add_type"
						href="JavaScript:void(0);"> <span><?php echo $lang['type_add_attr_add_one_value'];?></span>
					</a></td>
				</tr>
			</tbody>
		</table>
