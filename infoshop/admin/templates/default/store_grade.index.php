...
<div class="page">
		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td><input type="text"
						value="<?php echo $output['like_sg_name'];?>" name="like_sg_name"
						id="like_sg_name" class="txt"></td>
					<td><a href="javascript:document.formSearch.submit();"
						class="btn-search " title="<?php echo $lang['nc_query']; ?>">&nbsp;</a>
            <?php if($output['like_sg_name'] != ''){?>
            <a class="btns "
						href="index.php?act=store_grade&op=store_grade"
	...
	<form id="form_grade" method='post' name="">
        ...
        <tr class="hover">
					<td><?php if($v['sg_id'] > 1){ ?>
            <input type="checkbox" name='check_sg_id[]'
						value="<?php echo $v['sg_id'];?>" class="checkitem">
            <?php } ?></td>
					...
					<td class="w270"><a
						href="index.php?act=store_grade&op=store_grade_edit&sg_id=<?php echo $v['sg_id'];?>"><?php echo $lang['nc_edit'];?></a> |
            <?php if($v['sg_id'] == '1'){ ?>
            <?php echo $lang['default_store_grade_no_del'];?> |
            <?php }else { ?>
            <a
						href="javascript:if(confirm('<?php echo $lang['problem_del'];?>'))window.location = 'index.php?act=store_grade&op=store_grade_del&sg_id=<?php echo $v['sg_id'];?>';"><?php echo $lang['nc_del'];?></a> |
            <?php } ?>
            <a
						href="index.php?act=store_grade&op=store_grade_templates&sg_id=<?php echo $v['sg_id'];?>"><?php echo $lang['set_template'];?></a></td>
				</tr>
        <?php } ?>
        <?php }else { ?>
        <tr class="no_data">
			...
			<tfoot>
				<tr class="tfoot">
					...
					<td colspan="15"><label for="checkallBottom"><?php echo $lang['nc_select_all']; ?></label>
						...
						onclick="if(confirm('<?php echo $lang['problem_del'];?>')){$('#form_grade').submit();}"><span><?php echo $lang['nc_del'];?></span></a></td>
				</tr>
			</tfoot>
		</table>
	</form>
