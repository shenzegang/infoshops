...
<div class="w100pre close_float" style="text-align: right;">
	<input type="hidden" id="export_type" name="export_type"
		data-param='{"url":"<?php echo $output['actionurl'];?>&orderby=<?php echo $output['orderby'];?>&exporttype=excel"}'
		value="excel" /> <a class="btns" href="javascript:void(0);"
		id="export_btn"><span>导出Excel</span></a>
</div>
<input type="hidden" id="orderby" name="orderby"
	value="<?php echo $output['orderby']; ?>" />
...
<table class="table tb-type2 nobdb">
	<thead>
        ...
        <!-- <th class="<?php echo $v['class']?$v['class']:'align-center';?>"> -->
        	...
        	<a nc_type="orderitem"
				href="<?php echo $output['actionurl'];?>&orderby=<?php echo $v['key'].' asc';?>"
				class="selected desc"><?php echo $v['text'];?><i></i></a>
			</th>
        	<?php } elseif ($output['orderby'] == $v['key'].' asc'){?>
        	<a nc_type="orderitem"
				href="<?php echo $output['actionurl'];?>&orderby=<?php echo $v['key'].' desc';?>"
				class="selected asc"><?php echo $v['text'];?><i></i></a>
			</th>
        	<?php } else {?>
        	<a nc_type="orderitem"
				href="<?php echo $output['actionurl'];?>&orderby=<?php echo $v['key'].' desc';?>"><?php echo $v['text'];?><i></i></a>
			</th>
        	<?php }?>
        <?php } else {?>
    ...
    <?php if(!empty($output['statlist'])){ ?>
        <?php foreach ((array)$output['statlist'] as $k=>$v){?>
          <tr class="hover">
          	...
          	<td
				class="<?php echo $h_v['class']?$h_v['class']:'align-center';?>"><?php echo $v[$h_v['key']];?></td>
          	<?php }?>
          </tr>
        <?php } ?>
    <?php } else {?>
