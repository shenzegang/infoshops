...
<div class="page">
	...
	<form method="get" name="formSearch">
		<input type="hidden" value="microshop" name="act"> <input
			type="hidden" value="comment_manage" name="op">
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					<th><label for="comment_id"><?php echo $lang['microshop_comment_id'];?></label></th>
					<td><input type="text" value="<?php echo $_GET['comment_id'];?>"
						name="comment_id" class="txt"></td>
					...
					<td><select name="comment_type">
                            ...
                            <option value="<?php echo $key;?>"
								<?php if($key==$_GET['comment_type']) { echo 'selected'; } ?>><?php echo $value['name'];?></option>
                            <?php } ?>
                            <?php } ?>
                        </select></td>
					<th><label for="comment_object_id"><?php echo $lang['microshop_comment_object_id'];?></label></th>
					<td><input type="text"
						value="<?php echo $_GET['comment_object_id'];?>"
						name="comment_object_id" class="txt"></td>
					<th><label for="comment_message"><?php echo $lang['microshop_comment_message'];?></label></th>
					<td><input type="text"
						value="<?php echo $_GET['comment_message'];?>"
						name="comment_message" class="txt"></td>
			...
			<tbody>
                ...
                <tr class="hover edit">
					<td><input type="checkbox"
						value="<?php echo $value['comment_id'];?>" name="del_id[]"
						class="checkitem"></td>
					<td><?php echo $value['comment_id'];?></td>
					<td><a
						href="<?php echo MICROSHOP_SITE_URL.DS.'index.php?act=home&member_id='.$value['comment_member_id'];?>"
						target="_blank">
                            <?php echo $value['member_name'];?>
                        </a></td>
					<td><?php echo $output['channel_array'][$value['comment_type']]['name'];?></td>
					<td><a
						href="<?php echo MICROSHOP_SITE_URL.DS.'index.php?act='.$output['channel_array'][$value['comment_type']]['key'].'&op=detail&'.$output['channel_array'][$value['comment_type']]['key'].'_id='.$value['comment_object_id'];?>"
						target="_blank">
                            <?php echo $value['comment_object_id'];?>
                        </a></td>
					<td><?php echo parsesmiles($value['comment_message']);?></td>
					<td><a href="###"
						onclick="submit_delete(<?php echo $value['comment_id'];?>)"><?php echo $lang['nc_del'];?></a></td>
				</tr>
                <?php } ?>
                <?php }else { ?>
                <tr class="no_data">
