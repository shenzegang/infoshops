...
<div class="page">
	...
	<form id="pointprod_form" method="post" enctype="multipart/form-data">
      ...
      <?php if($output['theme_info']['theme_special'] == 1){?>
			...
			<tbody>
				<tr class="noborder">
					<td class="w132"><?php echo $lang['circle_poll_option'];?></td>
					<td class="w48"><?php echo $lang['circle_poll_option_count'];?></td>
					<td class="w830"><?php echo $lang['circle_poll_option_participant'];?></td>
				</tr>
        <?php if(!empty($output['option_list'])){?>
        <?php foreach ($output['option_list'] as $val){?>
        <tr class="noborder">
					<td><?php echo $val['pollop_option'];?></td>
					<td><?php echo $val['pollop_votes'];?></td>
					<td><?php echo $val['pollop_votername'];?></td>
				</tr>
        <?php }?>
        <?php }?>
      </tbody>
      ...
      <thead>
			...
			<tfoot>
				...
				<tr class="tfoot">
					<td colspan="3"><a
						href="index.php?act=circle_theme&op=theme_reply&t_id=<?php echo $output['theme_info']['theme_id'];?>"
						class="btn"><span><?php echo $lang['circle_theme_check_reply'];?></span></a>
						<a href="JavaScript:void(0);"
						onclick="if(confirm('<?php echo $lang['circle_theme_del_confirm'];?>')){location.href='index.php?act=circle_theme&op=theme_del&c_id=<?php echo $output['theme_info']['circle_id'];?>&t_id=<?php echo $output['theme_info']['theme_id'];?>';}else{return false;}"
						class="btn" id="delTheme"><span><?php echo $lang['circle_theme_del'];?></span></a>
					</td>
				</tr>
			</tfoot>
		</table>
