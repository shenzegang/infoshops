...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#close_button").click(function () {
            $('#close_form').attr('action','index.php?act=complain&op=complain_close');
            $("final_handle_message").text('');
            $(".complain_dialog").show();
            $("#close_complain").hide();
        });
        //支持买家
        $("#support_buyer_button").click(function () {
            $('#close_form').attr('action','index.php?act=complain&op=complain_support_buyer');
            //alert($('#close_form').attr("action"));
            $("final_handle_message").text('');
            $(".complain_dialog").show();
            $("#close_complain").hide();
        });
        //支持卖家
        $("#support_seller_button").click(function () {
            $('#close_form').attr('action','index.php?act=complain&op=complain_support_seller');
            $("final_handle_message").text('');
            $(".complain_dialog").show();
            $("#close_complain").hide();
        });
...
<div class="page">
    ...
    if (intval($output['complain_info']['complain_state']) > 20 && intval($output['complain_info']['complain_state']) < 50) {
        include template('complain_talk.info');
    }
    if (intval($output['complain_info']['complain_state']) >40 && !empty($output['complain_info']['final_handle_message'])) {
    ...
    <?php if (intval($output['complain_info']['complain_state']) !== 99) { ?>
        <table class="table tb-type2 order mtw">
            ...
            <tr id="close_complain">
                <td>
                    <form method='post' id="verify_form"
                          action="index.php?act=complain&op=complain_verify">
                        <input id="complain_id" name="complain_id" type="hidden"
                               value="<?php echo $output['complain_info']['complain_id']; ?>"/>
                        <?php if (intval($output['complain_info']['complain_state']) === 10) { ?>
                            <!-- 审核-->
                            <a id="verify_button" class="btn"
                               href="javascript:void(0)"><span><?php echo $lang['complain_text_verify']; ?></span></a>
                        <?php } ?>
                        <?php if (intval($output['complain_info']['complain_state']) === 40) { ?>
                            <!-- 支持买家-->
                            <a id="support_buyer_button" class="btn"
                               href="javascript:void(0)"><span><?php echo $lang['support_buyer']; ?></span></a>
                            <!-- 支持卖家-->
                            <a id="support_seller_button" class="btn"
                               href="javascript:void(0)"><span><?php echo $lang['support_seller']; ?></span></a>
                        <?php } ?>
                        <?php if (intval($output['complain_info']['complain_state']) < 60) { ?>
                            <!-- 关闭投诉-->
                            <a id="close_button" class="btn"
                               href="javascript:void(0)"><span><?php echo $lang['complain_text_close']; ?></span></a>
                        <?php } ?>
                        <a href="JavaScript:void(0);" class="btn"
                           onclick="history.go(-1)"><span><?php echo $lang['nc_back']; ?></span></a>
                    </form>
                </td>
            </tr>
            <form method='post' id="close_form"
                                    ...
                                    <tr>
                                        <td width="30">
                                            <?php if ($val['goods_refund'] > 0) { ?>
                                                <input class="checkitem"
                                                       name="checked_goods[<?php echo $val['rec_id']; ?>]"
                                                       type="checkbox" value="<?php echo $val['rec_id']; ?>"/>
                                            <?php } ?>
                                        </td>
                                        <td width="65" align="center" valign="middle"><a
                ...
                <input name="complain_id" type="hidden"
                       value="<?php echo $output['complain_info']['complain_id']; ?>"/>
                <tr class="noborder complain_dialog">
                    <td><textarea id="final_handle_message" name="final_handle_message"
                                  class="tarea"></textarea></td>
                </tr>
                <tr class="complain_dialog">
                    <td><a id="btn_handle_submit" class="btn"
                           href="javascript:void(0)"><span><?php echo $lang['nc_submit']; ?></span></a>
                        <a id="btn_close_cancel" class="btn"
                           href="javascript:void(0)"><span><?php echo $lang['nc_cancel']; ?></span></a>
                    </td>
                </tr>
            </form>
            </tbody>
