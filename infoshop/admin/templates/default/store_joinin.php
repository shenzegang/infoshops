...
<div class="page">
	...
	<form method="get" name="formSearch">
		<input type="hidden" value="store" name="act"> <input type="hidden"
			value="store_joinin" name="op">
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					<th><label for="store_name"><?php echo $lang['store_name'];?></label></th>
					<td><input type="text" value="" name="store_name" id="store_name"
						class="txt"></td>
					<th><label for="owner_and_name"><?php echo $lang['store_user'];?></label></th>
					<td><input type="text" value="" name="owner_and_name"
						id="owner_and_name" class="txt"></td>
              ...
              <option value="<?php echo $v['sg_id'];?>"
								<?php if($v['sg_id']==$_GET['grade_id']) { echo 'selected'; }?>><?php echo $v['sg_name'];?></option>
              <?php } ?>
              <?php } ?>
            </select></td>
                    ...
                    <option value="<?php echo $k;?>"
								<?php if($k==$_GET['joinin_state']) { echo 'selected'; }?>><?php echo $v;?></option>
                    <?php } ?>
                    <?php } ?>
                </select></td>
                ...
                <a href="index.php?act=store&op=store_joinin"
						class="btns " title="<?php echo $lang['nc_cancel_search'];?>"><span><?php echo $lang['nc_cancel_search'];?></span></a>
                <?php }?></td>
				</tr>
			</tbody>
		</table>
...
<!--						--><?php //echo empty($v['personal']) ? '企业' : '个人';?>
						<?php if($v['personal'] == '0') {echo  '企业';}?>
						<?php if($v['personal'] == '1') {echo  '个体户';}?>
						<?php if($v['personal'] == '2') {echo  '个人';}?>
					</td>
              ...
              <a
						href="index.php?act=store&op=store_joinin_detail&member_id=<?php echo $v['member_id'];?>">审核</a>
              <?php } else { ?>
              <a
						href="index.php?act=store&op=store_joinin_edit&member_id=<?php echo $v['member_id'];?>">资料修改</a>
						<a
						href="index.php?act=store&op=store_joinin_detail&member_id=<?php echo $v['member_id'];?>">查看</a>
              <?php } ?>
          </td>
				</tr>
        <?php } ?>
