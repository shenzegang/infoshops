...
<div class="page">
	...
	<form method="get" name="formSearch">
		...
		<table class="tb-type1 noborder search">
            ...
            <a class="btns" target="_blank"
						href="<?php echo urlShop('show_store','index', array('store_id'=>$output['list'][0]['store_id']));?>"><span><?php echo $output['store_name'];?></span></a>
            <?php }?>
            </td>
				</tr>
			</tbody>
	...
	<form method='post' id="picForm" name="picForm">
		<input type="hidden" name="form_submit" value="ok" /> <input
			type="hidden" name="act" value="goods_album" /> <input type="hidden"
			name="op" value="list" />
        ...
        <tr class="hover edit">
					<td><input value="<?php echo $v['aclass_id']?>" class="checkitem"
						type="checkbox" name="aclass_id[]"></td>
					<td>
          <?php if($v['aclass_cover'] != ''){ ?>
              <img
						src="<?php echo cthumb($v['aclass_cover'], 60, $v['store_id']);?>"
						onload="javascript:DrawImage(this,70,70);">
              <?php }else{?>
              <img
						src="<?php echo ADMIN_SITE_URL.'/templates/'.TPL_NAME.'/images/member/default_image.png';?>"
						onload="javascript:DrawImage(this,70,70);">
              <?php }?>
          </td>
					<td class="name"><?php echo $v['aclass_name'];?></td>
					<td class="class"><a
						href="<?php echo urlShop('show_store','index', array('store_id'=>$v['store_id']));?>"
						target="_blank"><?php echo $v['store_name'];?></td>
					<td><?php echo $output['pic_count'][$v['aclass_id']]?$output['pic_count'][$v['aclass_id']]:0;?></td>
					<td class="align-center"><a
						href="index.php?act=goods_album&op=pic_list&aclass_id=<?php echo $v['aclass_id'];?>"><?php echo $lang['d'];?><?php echo $lang['g_album_pic_one'];?></a>&nbsp;|&nbsp;<a
						href="javascript:void(0)"
						onclick="if(confirm('<?php echo $lang['nc_ensure_del'];?>')){location.href='index.php?act=goods_album&op=aclass_del&aclass_id=<?php echo $v['aclass_id'];?>';}else{return false;}"><?php echo $lang['nc_del'];?></a></td>
				</tr>
        <?php } ?>
        <?php }else { ?>
        <tr class="no_data">
        ...
        <tr colspan="15" class="tfoot">
					...
					<td colspan="16"><label for="checkallBottom"><?php echo $lang['nc_select_all']; ?></label>
						...
						onclick="if(confirm('<?php echo $lang['nc_ensure_del'];?>')){$('#picForm').submit()}"><span><?php echo $lang['nc_del'];?></span></a>
