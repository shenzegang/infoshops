...
<div class="page">
	...
	<table class="table tb-type2 noborder search">
      ...
      <a class="btns"
					onclick="if (confirm('审核后将无法撤销，进入下一步付款环节，确认审核吗?')){return true;}else{return false;}"
					href="index.php?act=bill&op=bill_check&bill_id=<?php echo $_GET['bill_id'];?>"><span><?php echo $lang['nc_exdport'];?>审核</span></a>
       <?php }elseif ($output['bill_info']['ob_state'] == 3){?>
		<a class="btns"
					href="index.php?act=bill&op=bill_pay&bill_id=<?php echo $_GET['bill_id'];?>"><span><?php echo $lang['nc_exdport'];?>付款完成</span></a>
       <?php }elseif ($output['bill_info']['ob_state'] == 4){?>
      <a class="btns" target="_blank"
					href="index.php?act=bill&op=bill_print&bill_id=<?php echo $_GET['bill_id'];?>"><span><?php echo $lang['nc_exposrt'];?>打印</span></a>
      <?php }?>
      <a class="btns" target="_blank"
					href="index.php?act=bill&op=export_step1&bill_id=<?php echo $_GET['bill_id'];?>"><span><?php echo $lang['nc_exposrt'];?>导出结算明细</span></a>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td>起止日期：<?php echo date('Y-m-d',$output['bill_info']['ob_start_date']);?> &nbsp;至&nbsp; <?php echo date('Y-m-d',$output['bill_info']['ob_end_date']);?>
      	</td>
			</tr>
			<tr>
	...
	<form method="get" action="index.php" name="formSearch" id="formSearch">
		<input type="hidden" name="act" value="bill" /> <input type="hidden"
			name="op" value="show_store_month_order" /> <input type="hidden"
			name="bill_id" value="<?php echo $_GET['bill_id'];?>" />
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					<th><label for="add_time_from"><?php echo $lang['order_time_from'];?>订单类型</label></th>
					<td><select name="query_type" class="querySelect">
							<option value="real" <?php if($_GET['query_type'] == 'real'){?>
								selected <?php }?>>实收订单</option>
							<option value="refund"
								<?php if($_GET['query_type'] == 'refund'){?> selected <?php }?>>退款订单</option>
					</select></td>
					<th><label for="add_time_from"><?php echo $lang['order_time_from'];?>下单时间</label></th>
					<td><input class="txt date" type="text"
						value="<?php echo $_GET['query_start_date'];?>"
						id="query_start_date" name="query_start_date"> <label>~</label> <input
						class="txt date" type="text"
						value="<?php echo $_GET['query_end_date'];?>" id="query_end_date"
						name="query_end_date" /></td>
					<td><a href="javascript:viod(0);" id="ncsubmit" class="btn-search "
						title="<?php echo $lang['nc_query'];?>">&nbsp;</a></a></td>
				</tr>
			</tbody>
		</table>
	</form>
      ...
      <tr class="hover">
        ...
        <?php
            ...
            ?>
				...
				<td><a
					href="index.php?act=order&op=show_order&order_id=<?php echo $order['order_id'];?>"><?php echo $lang['nc_view'];?></a>
				</td>
			</tr>
      <?php }?>
      <?php }else{?>
