...
<div class="page">
	...
	<form id="membertag_form" method="post" enctype="multipart/form-data">
		<input type="hidden" name="form_submit" value="ok" /> <input
			type="hidden" name="id"
			value="<?php echo $output['mtag_info']['mtag_id'];?>" /> <input
			type="hidden" name="old_membertag_name"
			value="<?php echo $output['mtag_info']['mtag_img'];?>" />
		<table class="table tb-type2">
			<tbody>
				<tr class="noborder">
					<td colspan="2" class="required"><label class="validation"
						for="membertag_name"><?php echo $lang['sns_member_tag_name'];?>:</label></td>
				</tr>
				<tr class="noborder">
					<td class="vatop rowform"><input type="text"
						value="<?php echo $output['mtag_info']['mtag_name'];?>"
						name="membertag_name" id="membertag_name" class="txt"></td>
				...
				<tr class="noborder">
					<td class="vatop rowform onoff"><label for="mtag_recommend1"
						class="cb-enable <?php if($output['mtag_info']['mtag_recommend'] == 1){?>selected<?php }?>"><span><?php echo $lang['nc_yes'];?></span></label>
						<label for="mtag_recommend0"
						class="cb-disable <?php if($output['mtag_info']['mtag_recommend'] == 0){?>selected<?php }?>"><span><?php echo $lang['nc_no'];?></span></label>
						<input id="mtag_recommend1" name="membertag_recommend"
						<?php if($output['mtag_info']['mtag_recommend'] == 1){?>
						checked="checked" <?php }?> value="1" type="radio"> <input
						id="mtag_recommend0" name="membertag_recommend"
						<?php if($output['mtag_info']['mtag_recommend'] == 0){?>
				...
				<tr class="noborder">
					<td class="vatop rowform"><input type="text"
						value="<?php echo $output['mtag_info']['mtag_sort'];?>"
						name="membertag_sort" id="mtag_sort" class="txt"></td>
				...
				<tr class="noborder"
					style="background: none repeat scroll 0% 0% rgb(251, 251, 251);">
					<td class="vatop rowform"><textarea class="tarea" rows="6"
							name="membertag_desc" id="membertag_desc"><?php echo $output['mtag_info']['mtag_desc'];?></textarea>
					</td>
				...
				<tr class="noborder">
					<td class="vatop rowform"><span class="type-file-show"><img
							...
							<div class="type-file-preview">
								<img
									src="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_PATH.'/membertag/'.($output['mtag_info']['mtag_img'] != ''?$output['mtag_info']['mtag_img']:'default_tag.gif');?>">
							</div> </span><span class="type-file-box"> <input
							name="membertag_img" type="file" class="type-file-file"
							id="membertag_img" size="30" hidefocus="true">
					</span></td>
