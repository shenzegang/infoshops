...
<div class="page">
	...
	<form method="get" action="index.php" name="formSearch" id="formSearch">
		<input type="hidden" name="act" value="stat_trade" /> <input
			type="hidden" name="op" value="goods_sale" /> <input type="hidden"
			name="" value="" />
		<div class="w100pre" style="width: 100%;">
			<table class="tb-type1 noborder search left">
				<tbody>
					<tr>
						<th>商品名称</th>
						<td><input class="txt-long" type="text" name="goods_name"
							value="<?php echo $_GET['goods_name'];?>" /></td>
						<th>店铺名称</th>
						<td><input class="txt-long" type="text" name="store_name"
							value="<?php echo $_GET['store_name'];?>" /></td>
						<th>平台货号</th>
						<td><input class="txt-long" type="text" name="goods_serial"
							value="<?php echo $_GET['goods_serial'];?>" /></td>
					</tr>
					<tr>
              ...
              <option value="<?php echo $v['brand_id'];?>"
									<?php if(intval($_GET['search_brand_id']) == $v['brand_id']){?>
									selected <?php }?>><?php echo $v['brand_name'];?></option>
              <?php } ?>
              <?php } ?>
            </select></td>
						<th><label>分类</label></th>
						<td id="gcategory"><input type="hidden" id="cate_id"
							name="cate_id" value="" class="mls_id" /> <input type="hidden"
							id="cate_name" name="cate_name" value="" class="mls_names" /> <select
							class="querySelect">
              ...
              <option value="<?php echo $val['gc_id']; ?>"
									<?php if($output['search']['cate_id'] == $val['gc_id']){?>
									selected <?php }?>><?php echo $val['gc_name']; ?></option>
              <?php } ?>
              <?php } ?>
            </select></td>
	...
	<div style="text-align: right;">
		<input type="hidden" id="export_type" name="export_type"
			data-param='{"url":"<?php echo $output['actionurl'];?>&goods_name=<?php echo trim($_GET['goods_name']); ?>&store_name=<?php echo trim($_GET['store_name']); ?>&goods_serial=<?php echo trim($_GET['goods_serial']); ?>&brand_id=<?php echo intval($_GET['brand_id']); ?>&cate_id=<?php echo intval($_GET['cate_id']); ?>&exporttype=excel"}'
