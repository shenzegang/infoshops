...
<div class="page">
	<div class="fixed-bar">
		<div class="item-title">
			...
			<ul class="tab-base">
				...
				<li><a href="index.php?act=web_food&op=focus_edit"><span><?php echo '焦点区';?></span></a></li>
	...
	<form id="sale_list_form" method="post" name="form1">
		<input type="hidden" name="web_id"
			value="<?php echo $output['code_sale_list']['web_id'];?>"> <input
			type="hidden" name="code_id"
			value="<?php echo $output['code_sale_list']['code_id'];?>">
      ...
      <dl sale_id="<?php echo $key;?>">
				...
				<dt>
					...
					<input name="sale_list[<?php echo $key;?>][recommend][name]"
						value="<?php echo $val['recommend']['name'];?>" type="hidden"> <a
						href="JavaScript:show_sale_dialog(<?php echo $key;?>);"><i
            ...
            <li>
							...
							name="sale_list[<?php echo $key;?>][goods_list][<?php echo $v['goods_id'];?>][goods_id]"
							value="<?php echo $v['goods_id'];?>" type="hidden"> <input
							name="sale_list[<?php echo $key;?>][goods_list][<?php echo $v['goods_id'];?>][market_price]"
							value="<?php echo $v['market_price'];?>" type="hidden"> <input
							name="sale_list[<?php echo $key;?>][goods_list][<?php echo $v['goods_id'];?>][goods_name]"
							value="<?php echo $v['goods_name'];?>" type="hidden"> <input
							name="sale_list[<?php echo $key;?>][goods_list][<?php echo $v['goods_id'];?>][goods_price]"
							value="<?php echo $v['goods_price'];?>" type="hidden"> <input
							name="sale_list[<?php echo $key;?>][goods_list][<?php echo $v['goods_id'];?>][goods_pic]"
      ...
      <?php } ?>
      <?php } ?>
      <div class="add-sale" id="add_list">
				<a class="btn-add-nofloat" href="JavaScript:add_sale_list();"><?php echo $lang['web_config_add_recommend'];?></a><?php echo $lang['web_config_sale_max'];?></div>
		</div>
		<a
			href="index.php?act=web_food&op=html_update&web_id=<?php echo $output['code_sale_list']['web_id'];?>"
...
<div id="sale_list_dialog" style="display: none;">
  ...
  <dl select_sale_id="<?php echo $key;?>">
		<dt>
			...
			<div class="dialog-handle-box">
				<span class="left"> <input
					name="sale_list[<?php echo $key;?>][recommend][name]"
		...
		<dd>
			<div class="s-tips">
				<i></i><?php echo $lang['web_config_goods_list_tips'];?></div>
			<ul class="dialog-goodslist-s1 goods-list">
        <?php if(!empty($val['goods_list']) && is_array($val['goods_list'])){ ?>
        <?php foreach($val['goods_list'] as $k => $v){ ?>
        <li>
					<div ondblclick="del_sale_goods(<?php echo $v['goods_id'];?>);"
						class="goods-pic">
						<span class="ac-ico"
							onclick="del_sale_goods(<?php echo $v['goods_id'];?>);"></span> <span
							class="thumb size-72x72"><i></i><img
							select_goods_id="<?php echo $v['goods_id'];?>"
							title="<?php echo $v['goods_name'];?>"
							src="<?php echo strpos($v['goods_pic'],'http')===0 ? $v['goods_pic']:UPLOAD_SITE_URL."/".$v['goods_pic'];?>"
							onload="javascript:DrawImage(this,72,72);"
							goods_price="<?php echo $v['goods_price'];?>"
							market_price="<?php echo $v['goods_marketprice'];?>" /></span>
					</div>
  ...
  <div id="select_sale_list" style="display: none;"></div>
		...
		<table class="tb-type1 noborder search">
			<tbody>
				<tr>
					...
					<td colspan="3" class="dialog-select-bar" id="gcategory"><input
						type="hidden" id="cate_id" name="cate_id" value="0" class="mls_id" />
						<input type="hidden" id="cate_name" name="cate_name" value=""
						class="mls_names" /> <select>
              ...
              <?php } ?>
				...
				<tr>
					...
					<td><input type="text" value="" name="order_goods_name"
						id="order_goods_name" class="txt"> <a href="JavaScript:void(0);"
						onclick="get_goods_list();" class="btn-search "
	...
	<div class="clear"></div>
	<a href="JavaScript:void(0);" onclick="update_sale();" class="btn"><span><?php echo $lang['web_config_save'];?></span></a>
</div>
<script
