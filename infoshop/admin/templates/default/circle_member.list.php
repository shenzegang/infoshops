...
<script type="text/javascript"
	src="<?php echo RESOURCE_SITE_URL;?>/js/jquery-ui/jquery.ui.js"
...
<div class="page">
	...
	<form method="get" name="formSearch">
		<input type="hidden" name="act" value="circle_member"> <input
			type="hidden" name="op" value="member_list">
		<table class="tb-type1 noborder search">
			<tbody>
				...
				<tr>
					...
					<td><select name="searchidentity">
							<option value=""><?php echo $lang['nc_common_pselect'];?></option>
							<option value="1"
								<?php if ($_GET['searchidentity'] == '1') { echo 'selected="selected"';}?>>圈主</option>
							<option value="2"
								<?php if ($_GET['searchidentity'] == '2') { echo 'selected="selected"';}?>>管理</option>
							<option value="3"
								<?php if ($_GET['searchidentity'] == '3') { echo 'selected="selected"';}?>>成员</option>
					</select></td>
	...
	<form method='post' id="member_form" name="member_form">
        ...
        <tr class="hover edit member">
					<td class="w24"><input type="checkbox" name="check_param[]"
						value="<?php echo $val['member_id'];?>|<?php echo $val['circle_id'];?>"
						class="checkitem"></td>
					...
					<td><p>
							<strong><?php echo $val['member_name'];?></strong>
						</p></td>
					<td><p class="name">
							<strong><?php echo $val['circle_name'];?></strong>(<?php
...
switch (intval($val['is_identity'])) {
                ...
                ?>)
						...
						<p class="smallfont"><?php echo $lang['circle_member_join_time'];?><?php echo @date('Y-m-d H:i', $val['cm_applytime']);?>
					...
					<td class="align-center yes-onoff"><a href="JavaScript:void(0);"
						...
						fieldid="<?php echo $val['member_id'].'|'.$val['circle_id'];?>"
						fieldvalue="<?php echo $val['is_recommend'];?>"
						title="<?php echo $val['is_recommend'] ? L('nc_yes') : L('nc_no');?>"><img
							src="<?php echo ADMIN_TEMPLATES_URL;?>/images/transparent.gif"></a>
					</td>
					<td class="w72"><a href="javascript:void(0);"
						onclick="delmember('<?php echo $val['member_id']?>|<?php echo $val['circle_id'];?>')"><?php echo $lang['nc_del'];?></a>
						| <a
						href="<?php echo SHOP_SITE_URL;?>/index.php?act=sns_circle&mid=<?php echo $val['member_id'];?>"
						target="_blank"><?php echo $lang['nc_view'];?></a></td>
				</tr>
        <?php } ?>
        <?php }else { ?>
        <tr class="no_data">
      ...
      <tfoot>
				<tr class="tfoot">
					...
					<td id="batchAction" colspan="15"><span class="all_checkbox"> <label
						...
						onclick="delBatchMember();"><span><?php echo $lang['nc_del'];?></span></a>
