		...
		<table class="table tb-type2 nomargin">
          ...
          <tr>
					<td><input class="checkitem" type="checkbox"
						value="<?php echo $val['mlref_id'];?>" name="del_id[]"></td>
					...
					<td class="align-center yes-onoff"><a href="JavaScript:void(0);"
						class="<?php echo $val['mlref_status']? 'enabled' : 'disabled' ;?>"
						ajax_branch="status" nc_type="inline_edit"
						fieldname="mlref_status" fieldid="<?php echo $val['mlref_id']?>"
						fieldvalue="<?php echo $val['mlref_status']? '1' : '0' ;?>"
						title="<?php echo $lang['nc_editable'];?>"><img
							src="<?php echo ADMIN_TEMPLATES_URL;?>/images/transparent.gif" /></a>
					</td>
					<td><a
						href="index.php?act=circle_memberlevel&op=ref_edit&mlref_id=<?php echo $val['mlref_id'];?>"><?php echo $lang['nc_edit'];?></a>
						| <a href="javascript:void(0);"
						onclick="if(confirm('<?php echo $lang['nc_ensure_del'];?>')){location.href='index.php?act=circle_memberlevel&op=ref_del&mlref_id=<?php echo $val['mlref_id'];?>'}"><?php echo $lang['nc_del'];?></a></td>
				</tr>
          <?php } ?>
          <?php }else { ?>
          <tr class="no_data">
			...
			<tfoot>
				<tr class="tfoot">
					...
					<td colspan="20"><label for="checkallBottom"><?php echo $lang['nc_select_all']; ?></label>
						...
						onclick="if(confirm('<?php echo $lang['nc_ensure_del'];?>')){$('#clmdForm').submit();}"><span><?php echo $lang['nc_del'];?></span></a></td>
				</tr>
			</tfoot>
		</table>
	</form>
