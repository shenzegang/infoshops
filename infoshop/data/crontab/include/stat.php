...
class statControl
{
    ...
    public function memberstatOp()
    {
        ...
        for ($stime = $start_time; $stime < time(); $stime = $stime + 86400) {
            ...
            $where['order_log.log_time'] = array(
                'between',
                array(
                    $search_stime,
                    $search_etime
            ...
            $where['payment_code'] = array(
                'exp',
                "(order.payment_code='offline' and order_log.log_orderstate = '" . ORDER_STATE_SUCCESS . "') or (order.payment_code<>'offline' and order_log.log_orderstate = '" . ORDER_STATE_PAY . "' )"
            );
            ...
            foreach ((array) $update_arr as $k => $v) {
                foreach ($v as $m_k => $m_v) {
                    // 查询记录是否存在
                    $statmember_info = $model->getOneStatmember(array(
                        'statm_time' => $k,
                        'statm_memberid' => $m_k
                    ));
                    if ($statmember_info) {
            ...
            // 查询会员信息
            $memberid_list = Model('member')->getMemberList(array(
                'member_id' => array(
                    'in',
                    $memberid_arr
                )
            ...
            // 查询记录是否存在
            $statmemberlist_tmp = $model->statByStatmember(array(
                'statm_time' => $stime
            ));
