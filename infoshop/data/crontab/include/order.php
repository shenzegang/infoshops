...
class orderControl
{
    ...
    public function _create_order_statis()
    {
        ...
        $settlement_day = C("settlement_day") == null ? 7:C("settlement_day");
        if ($w == 0) {
            $Monday = strtotime(date('Y-m-d 00:00:00', $start_unixtime - 86400 * ($settlement_day-1)));
            ...
            $Sunday = strtotime(date('Y-m-d 23:59:59', $start_unixtime - 86400 * ($w - $settlement_day))); // print_r(date('Y-m-d',strtotime('-'.($i * 7).' day')));exit;print_r(date('Y-m-d',$Sunday));exit;
        }

        while (($time = strtotime('-' . ($i * $settlement_day) . ' day')) > $Monday) { // print_r(date('Ymd H:i:s',$time));exit;
            $now = date('w', $time);
            if ($now == 0) {
                $nowMonday = $time - 86400 * ($settlement_day-1);
                $nowSunday = $time;
            } else {
                $nowMonday = $time - 86400 * ($now - 1);
                $nowSunday = $time - 86400 * ($now - $settlement_day);
            }
            ...
            $order_bill_info = $model_bill->getOrderBillInfo(array(
                'ob_start_date' => $data[$key]['os_start_date']
            ), $fileds);
        ...
        for ($i = 0; $i <= $store_count; $i = $i + 300) {
            ...
            if ($store_list) {
                ...
                foreach ($store_list as $store_info) {
                    ...
                    if (! $model_bill->getOrderBillInfo(array(
                        'ob_no' => $data_bill['ob_no']
                    ))) {
    ...
    private function _calc_order_bill($data_bill)
    {
        ...
        $order_condition['finnshed_time'] = array(
            'time',
            array(
                $data_bill['ob_start_date'],
                $data_bill['ob_end_date']
            )
        );
        ...
        for ($i = 0; $i <= $order_count; $i = $i + 300) {
            ...
            if (! empty($order_id_array)) {
                ...
                $order_goods_condition['order_id'] = array(
                    'in',
                    $order_id_array
                );
        ...
        $refund_condition['goods_id'] = array(
            'gt',
            0
        );
        $refund_condition['admin_time'] = array(
            array(
                'egt',
                $data_bill['ob_start_date']
            ),
        ...
        $cost_condition['cost_time'] = array(
            array(
                'egt',
                $data_bill['ob_start_date']
            ),
        ...
        $update['ob_state'] = 1;
        $result = $model_bill->editOrderBill($update, array(
            'ob_no' => $data_bill['ob_no']
        ));
        return $result;
    }
