...
class sns_bindingModel extends Model
{
    ...
    public function getUsableApp($member_id)
    {
        ...
        if (empty($app_arrnew))
            return false;
            // 查询该用户的绑定信息
        $bind_list = $this->table('sns_binding')
            ->where(array(
            'snsbind_memberid' => $member_id
        ))
            ->select();
        if (! empty($bind_list)) {
    ...
    public function getUsableOneApp($member_id, $appkey)
    {
        ...
        $bind_info = $this->table('sns_binding')
            ->where($where_arr)
            ->find();
    ...
    {
        $app_arr = array();
        $app_arr['qqweibo'] = array(
            'name' => Language::get('nc_shareset_qqweibo'),
            'url' => "http://t.qq.com",
            'applyurl' => 'http://dev.open.t.qq.com/developer'
        );
        $app_arr['sinaweibo'] = array(
            'name' => Language::get('nc_shareset_sinaweibo'),
            'url' => "http://www.weibo.com",
            'applyurl' => 'http://open.weibo.com/developers'
        );
        return $app_arr;
    }
