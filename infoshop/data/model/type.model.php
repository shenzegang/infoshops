...
class typeModel extends Model
{
    ...
    {
        return $this->table('goods_attr_index')
            ->where($conditoin)
            ->order($order)
            ->limit($limit)
            ->page($page)
    ...
    {
        $result = $this->table('type_spec,spec')
            ->field($field)
            ->where($where)
            ->join('inner')
    ...
    {
        $spec_list = $this->typeRelatedJoinList(array(
            'type_id' => $type_id
        ), 'spec', 'spec.sp_id as sp_id, spec.sp_name as sp_name');
        $attr_list = $this->typeRelatedJoinList(array(
            'attribute.type_id' => $type_id
        ), 'attr', 'attribute.attr_id as attr_id, attribute.attr_name as attr_name, attribute_value.attr_value_id as attr_value_id, attribute_value.attr_value_name as attr_value_name');
        $brand_list = $this->typeRelatedJoinList(array(
            'type_id' => $type_id,
            'brand_apply' => 1
        ), 'brand', 'brand.brand_id as brand_id,brand.brand_name as brand_name');
        ...
        if (is_array($spec_list) && ! empty($spec_list)) {
            ...
            foreach ($spec_list as $val) {
                $spec_value_list = Model('spec')->getSpecValueList(array(
                    'sp_id' => $val['sp_id'],
                    'gc_id' => $gc_id,
                    'store_id' => $store_id
                ));
