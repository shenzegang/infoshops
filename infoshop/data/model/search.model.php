...
class searchModel
{
    ...
    private function _createXS($pagesize, $appname)
    {
        ...
        // 设置分面
        if ($this->_open_face) {
            $this->_xs_search->setFacets(array(
                'brand_id',
                'attr_id'
            ));
        }
    ...
    public function getAttrList()
    {
        ...
        if (! $data) {
            ...
            $data['gcid_array'] = array_merge(array(
                $param['gc_id']
            ), $child, $childchild);
            if (! empty($data) && count($data['gcid_array']) == 1) {
                ...
                // 品牌列表
                $typebrand_list = $model_type->getTypeBrandList(array(
                ...
                if (! empty($typebrand_list)) {
                    ...
                    $brand_array = Model('brand')->getBrandPassList(array(
                        'brand_id' => array(
                            'in',
                            $brandid_array
                        )
                ...
                $attribute_list = $model_attribute->getAttributeShowList(array(
                    'type_id' => $data['type_id']
                ), 'attr_id,attr_name');
                $attributevalue_list = $model_attribute->getAttributeValueList(array(
                    'type_id' => $data['type_id']
        ...
        if ($keyword != '') {
            ...
            if (! empty($tag_list) && is_array($tag_list)) {
                foreach ($tag_list as $key => $val) {
                    $tag_value = str_replace(',', '==CorShop==', $val['gc_tag_value']);
