...
class cacheModel extends Model
{
    ...
    private function _goods_class()
    {
        ...
        $result = $this->table('goods_class')
            ->field($fields)
            ->order('gc_parent_id asc, gc_sort asc')
            ->limit(10000)
            ->select();
        if (! is_array($result))
            return null;
    ...
    {
        $list = $this->table('goods_class')
            ->field('gc_id,gc_title,gc_keywords,gc_description')
            ->where(array(
            'gc_keywords' => array(
                'neq',
                ''
    ...
    private function _express()
    {
        ...
        $list = $this->table('express')
            ->field($fields)
            ->order('e_order,e_letter')
            ->where(array(
            'e_state' => 1
        ))
            ->select();
    ...
    private function _groupbuy()
    {
        ...
        $category = $this->table('groupbuy_class')
        ...
        $price = $this->table('groupbuy_price_range')
            ->order('range_start')
            ->select();
        if (! is_array($price))
            $price = array();
    ...
    private function _class_tag()
    {
        ...
        $list = $this->table('goods_class_tag')
            ->field($field)
            ->where(true)
            ->select();
        if (! is_array($list))
    ...
    {
        $list = $this->table('store_class')
            ->where(true)
            ->order('sc_parent_id,sc_sort')
            ->select();
        $tmp = array();
        if (is_array($list)) {
            foreach ($list as $key => $value) {
    ...
    {
        $list = $this->table('circle_mldefault')
            ->where(true)
            ->select();
