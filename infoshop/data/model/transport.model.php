...
class transportModel extends Model
{
    ...
    {
        return $this->table('transport_extend')
            ->where($condition)
            ->select();
    }
    ...
    public function delTansport($condition)
    {
        try {
            ...
            $delete = $this->table('transport')
                ->where($condition)
                ->delete();
            if ($delete) {
                $delete = $this->table('transport_extend')
                    ->where(array(
                    'transport_id' => $condition['id']
                ))
                    ->delete();
            }
            if (! $delete)
    ...
    public function calc_transport($transport_id, $buy_num, $area_id)
    {
        if (empty($transport_id) || empty($buy_num) || empty($area_id))
            return 0;
        $extend_list = $this->getExtendList(array(
            'transport_id' => $transport_id
        ));
