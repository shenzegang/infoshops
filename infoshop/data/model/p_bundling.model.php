...
class p_bundlingModel extends Model
{
 ...
 // 关闭
    ...
    {
        return $this->table('p_bundling_quota')
            ->where($condition)
            ->find();
    }
    ...
    {
        $condition['bl_quota_endtime'] = array(
            'gt',
            TIMESTAMP
        );
    ...
    public function editBundlingTimeout($condition)
    {
        ...
        if (! empty($quota_list)) {
            ...
            return $this->editBundlingQuotaClose(array(
                'bl_quota_id' => array(
                    'in',
                    $quotaid_array
                )
    ...
    {
        return $this->table('p_bundling_goods')
            ->field($field)
            ->where($condition)
            ->group($group)
            ->order($order)
