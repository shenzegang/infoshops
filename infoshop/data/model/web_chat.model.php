...
class web_chatModel extends Model
{
    ...
    public function getMember($member_id)
    {
        ...
        $member = $this->table('member')
            ->field('member_id,member_name,member_avatar')
            ->where(array(
            'member_id' => $member_id
        ))
            ->find();
        $member['store_name'] = '';
        $member['grade_id'] = '';
        $seller = $this->table('seller')
            ->field('member_id,store_id,seller_name')
            ->where(array(
            'member_id' => $member_id
        ))
            ->find();
        if (! empty($seller) && $seller['store_id'] > 0) {
            $store_info = $this->table('store')
