...
class cartModel extends Model {
	...
	public function addCart($data = array(), $save_type = '', $quantity = null) {
		...
		$this -> getCartNum($save_type, array(
	 ...
	 *
	 * @param string $type
	 *        	存储类型 db,cache,cookie
	 * @param unknown_type $condition        	
	 */
	public function listCart($type, $condition = array()) {
		if($type == 'db'){
			$cart_list = $this -> where($condition) -> select(array(
					'cache' => false 
			));
		}elseif($type == 'cache'){
			$obj_cache = Cache :: getInstance(C('cache.type'));
		...
		if(is_array($cart_list)){
			foreach($cart_list as $val){
				//sj 20150915 商城右侧 计算购物车商品价格 去除已下架或库存不足的价格
				$condition['goods_id'] = $val['goods_id'];
				$goods = $model_goods -> getGoodsList($condition);
				if($goods[0]['goods_state'] == '1' && $goods[0]['goods_storage'] > 0){
    ...
    public function delSelectedCart($type, $condition = array())
    {
        if ($type == 'db') {
            $result = $this->where($condition)->delete();
        }
	...
	public function getGoodsOnlineInfo($goods_id, $quantity) {
		// 取目前在售商品
		$goods_info = Model('goods') -> getGoodsOnlineInfo(array(
				'goods_id' => $goods_id 
		));
		if(empty($goods_info)){
			return null;
		...
		$new_array['is_gift'] = $goods_info['is_gift'];
		$new_array['gift_points'] = $goods_info['gift_points'];
	...
	public function getOnlineCartList($cart_list) {
		...
		$model_goods = Model('goods');
		$goods_online_list = $model_goods -> getGoodsOnlineList(array(
				'goods_id' => array(
						in, 
						$goods_id_array 
				) 
		...
		foreach((array)$cart_list as $key => $cart_info){
			...
			if(in_array($cart_info['goods_id'], array_keys($goods_online_array))){
				...
				$cart_list[$key]['is_gift'] = $goods_online_info['is_gift'];
				$cart_list[$key]['gift_points'] = $goods_online_info['gift_points'];
	...
	public function getBundlingCartList($cart_list) {
		...
		foreach($cart_list as $key => $cart_info){
			...
			// 取得优惠套装商品列表
			$cart_list[$key]['bl_goods_list'] = $model_bl -> getBundlingGoodsList(array(
					'bl_id' => $cart_info['bl_id'] 
			));
			// 取最新在售商品信息
			$goods_id_array = array();
	...
	public function calcCartList($store_cart_list) {
		...
		foreach($store_cart_list as $store_id => $store_cart){
			...
			foreach($store_cart as $key => $cart_info){
				...
				$tmp_points += $store_cart[$key]['gift_points'] * $cart_info['goods_num'];
			}
			$store_cart_list[$store_id] = $store_cart;
			$store_goods_total[$store_id] = ncPriceFormat($tmp_amount);
			$store_gift_total[$store_id] = intval($tmp_points);
		}
		return array(
				$store_cart_list, 
				$store_goods_total, 
	...
	public function getMansongRuleCartListByTotal($store_goods_total) {
		if(!C('promotion_allow') || empty($store_goods_total) || !is_array($store_goods_total)) return array(
				array(), 
				array() 
		);
		$model_mansong = Model('p_mansong');
	...
	private function _parseMansongRuleDesc($rule_info) {
		...
		$goods_desc = (!empty($rule_info['mansong_goods_name']) && !empty($rule_info['goods_storage'])) ? " 送<a href='" . urlShop('goods', 'index', array(
				'goods_id' => $rule_info['goods_id'] 
		)) . "' title='{$rule_info['mansong_goods_name']}' target='_blank'>[赠品]</a>" : '';
