...
class groupbuyModel extends Model {
	...
	public function getGroupbuyAvailableList($condition) {
		$condition['state'] = array(
				'in', 
				array(
						self :: GROUPBUY_STATE_REVIEW, 
						self :: GROUPBUY_STATE_NORMAL 
				) 
		);
	 ...
	 * 根据团购编号读取团购信息
	 *
	 * @param array $groupbuy_id
	 *        	团购活动编号
	 * @param int $store_id
	 *        	如果提供店铺编号，判断是否为该店铺活动，如果不是返回null
	 * @return array 团购信息
	 *        
	 */
	...
	/*
	 * 增加 @param array $param @return bool
	 */
	...
	private function _unlockGoods($goods_commonid) {
		...
		if(!empty($groupbuy_list)){
			$model_goods = Model('goods');
			$model_goods -> editGoodsCommonUnlock(array(
					'goods_commonid' => $goods_commonid 
			));
		}
	}
	/*
	 * 更新 @param array $update @param array $condition @return bool
	 */
	public function editGroupbuy($update, $condition) {
		return $this -> where($condition) -> update($update);
	}
	/*
	 * 审核成功 @param int $groupbuy_id @return bool
	 */
	...
	/*
	 * 审核失败 @param int $groupbuy_id @return bool
	 */
	...
	/*
	 * 取消 @param int $groupbuy_id @return bool
	 */
	...
	public function editExpireGroupbuy() {
		...
		if($groupbuy_id_string != ''){
			...
			$condition = array();
			$condition['groupbuy_id'] = array(
					'in', 
					rtrim($groupbuy_id_string, ',') 
			);
			$this -> editGroupbuy($update, $condition);
		}
	}
	/*
	 * 删除团购活动 @param array $condition @return bool
	 */
	...
	public function getGroupbuyExtendInfo($groupbuy_info) {
		$groupbuy_info['groupbuy_url'] = urlShop('show_groupbuy', 'groupbuy_detail', array(
				'group_id' => $groupbuy_info['groupbuy_id'] 
		));
		$groupbuy_info['goods_url'] = urlShop('goods', 'index', array(
