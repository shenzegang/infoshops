...
class evaluate_storeModel extends Model
{
     ...
     *            int @store_id 店铺编号
     * @param
     *            int @sc_id 分类编号，如果传入分类编号同时返回行业对比数据
     */
    public function getEvaluateStoreInfoByStoreID($store_id, $sc_id = 0)
    {
        ...
        if (empty($info)) {
            $info = array();
            $info['store_credit'] = $this->_getEvaluateStore(array(
                'seval_storeid' => $store_id
            ));
    ...
    public function getEvaluateStoreInfoByScID($sc_id)
    {
        ...
        if (empty($info)) {
            ...
            $store_id_string = $model_store->getStoreIDString(array(
                'sc_id' => $sc_id
            ));
            $info = $this->_getEvaluateStore(array(
                'seval_storeid' => array(
                    'in',
                    $store_id_string
                )
