...
class refund_returnModel extends Model
{
    ...
    public function editOrderLock($order_id)
    {
        ...
        if ($order_id > 0) {
            ...
            $data = array();
            $data['lock_state'] = array(
                'exp',
                'lock_state+1'
            );
            $result = $this->table('order')
                ->where($condition)
                ->update($data);
            return $result;
        }
    ...
    public function editRefundReturn($condition, $data)
    {
        ...
        if (is_array($data)) {
            $result = $this->table('refund_return')
                ->where($condition)
                ->update($data);
            return $result;
        } else {
    ...
    public function editOrderRefund($refund)
    {
        ...
        if ($refund_id > 0) {
            ...
            $field = 'order_id,buyer_id,buyer_name,store_id,order_sn,order_amount,payment_code,order_state,refund_amount,is_gift';
            ...
            if ($state) {
                ...
                /**
                 * 2015-9-6 tjz增加 检查订单部分退款
                 */
                $condition = array();
                $condition['order_id'] = $order_id;
                $order_info = $orderGoods->getOrderInfo($condition, array(
                    'order_common',
                    'order_goods'
                ));
                $flag = false;
               if (!empty($order_info)) {
                    foreach ($order_info['extend_order_goods'] as $order_exgoods) {
                        if ($order_exgoods['is_refund'] == '2' && $order_exgoods['goods_id'] == $refund['goods_id']) {
                            $data = array();
                            $data['is_refund'] = 1;
                            $condition = array();
                            $condition['goods_id'] = $refund['goods_id'];
                            $condition['order_id'] = $order_id;
                            $result = $orderGoods->editOrderGoods($data, $condition);
                            if ($result) {
                                //查询当前订单是否还有未退款的商品
                                $conditionGoodsCount = array();
                                $conditionGoodsCount['order_id'] = $order_id;
                                $conditionGoodsCount['is_refund'] = 0;
                                $goodsCount = $orderGoods->getGoodsCountByRefund($conditionGoodsCount);
                                if ($goodsCount == 0) {

                                    //sj 20150924 仅退款不退货 退款成功后 自动确认收货
                                    $cond = array();
                                    $cond['order_id'] = $order_id;
                                    $order = $orderGoods->getOrderInfo($cond);
                                    //退款不退货 卖家同意,平台确认后，自动确认收货
                                    if (intval($order['order_state']) < 40 && $refund['refund_type'] == '1') {
                                        $update_order = array();
                                        $update_order['order_state'] = 40;
                                        $orderGoods->editOrder($update_order, $cond);
                                    }
                                    $flag = true;
                                }
                            }
                ...
                if ($flag == true) {
                    //20150819 tjz增加 将订单状态改成交易关闭
                    $order_array['order_state'] = 50;
                }
                /*************2015-9-7 tjz增加结束*********************/

                $state = $this->table('order')
                    ->where(array(
                        'order_id' => $order_id
                    ))
                    ->update($order_array); // 更新订单退款

                //20150818 tjz修改 判断该笔订单是否是积分商品 是否需要退积分
                if ($order['is_gift'] == 1) {
                    // 是积分兑换 执行退款后给用户退回已经扣掉的积分

                    $conditionOrderGood = array();
                    $conditionOrderGood['order_id'] = $order['order_id'];
                    $order_Goods = $orderGoods->getOrderGoodsInfo($conditionOrderGood);
                    if (empty($order_Goods['gift_points']) || $order_Goods['gift_points'] != 0) {
                        $points_model = Model('points');
                        $insert_arr = array();
                        $insert_arr['pl_memberid'] = $order['buyer_id'];
                        $insert_arr['pl_membername'] = $order['buyer_name'];
                        $insert_arr['pl_points'] = +$order_Goods['gift_points'];
                        $insert_arr['point_ordersn'] = $order['order_sn'];
            ...
            /* require_once BASE_CORE_PATH . DS . 'framework' . DS . 'function' . DS . 'payapi.php';
             $resp = array();
             $req = array('goodCode' => $order['order_id'].$order_exgoods['goods_id']);
             $validResp = trade("http://192.168.0.97:6060/xnpayws/ws/pay/balancePay/refundGood", $req, $resp);
             if (!$validResp || $resp['returnStatus'] != '000') {
                 throw new Exception('退款操作失败');
             }*/
    ...
    public function getGoodsRefundList($order_list = array())
    {
        ...
        $refund_list = $this->table('refund_return')
            ->where($condition)
            ->order('refund_id desc')
            ->select();
