...
;
    ...
    {
        $goods_list = $this->table('order_goods')
            ->field('order_id,goods_num,goods_id')
            ->where(array(
            'order_id' => $order_id
        ))
            ->select(); // 订单商品
        if (! empty($goods_list) && is_array($goods_list)) {
            foreach ($goods_list as $k => $v) {
                ...
                $data['goods_salenum'] = array(
                    'exp',
                    'goods_salenum-' . $goods_num
                ); // 销售记录
                $state = $this->table('goods')
                    ->where($condition)
                    ->update($data);
            }
            ...
            $state = $model_order->editOrder($order_array, array(
                'order_id' => $order_id
            )); // 更新订单
            if ($state) {
    ...
    public function editOrderFinnsh($order_id, $log_array = array())
    {
        ...
        if ($order['order_state'] == $order_shipped) { // 确认已经完成发货
            ...
            $log_array['log_orderstate'] = $order_array['order_state'];
            if ($state)
                $state = $model_order->addOrderLog($log_array); // 订单处理记录信息
            return $state;
        } else {
            return false;
        }
