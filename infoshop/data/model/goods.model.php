...
class goodsModel extends Model
{
 ...
 // 等待审核
    ...
    {
        $condition = $this->_getRecursiveClass($condition);
        return $this->table('goods_common')
            ->field($field)
            ->where($condition)
            ->order($order)
            ->page($page)
    ...
    public function calculateStorage($goods_list, $storage_alarm = 0)
    {
        ...
        if (! empty($goods_list)) {
            ...
            $goods_storage = $this->getGoodsList(array(
                'goods_commonid' => array(
                    'in',
                    $goodscommonid_array
                )
            ), 'goods_storage,goods_commonid,goods_id');
            $storage_array = array();
    ...
    public function getGoodsDetail($goods_id)
    {
        ...
        $result1 = $this->getGoodsAsGoodsShowInfo(array(
            'goods_id' => $goods_id
        ));
        if (empty($result1)) {
            return null;
        }
        $result2 = $this->getGoodeCommonAsGoodsShowInfo(array(
            'goods_commonid' => $result1['goods_commonid']
        ));
        ...
        // 查询所有规格商品
        $spec_array = $this->getGoodsList(array(
            'goods_commonid' => $goods_info['goods_commonid']
        ), 'goods_spec,goods_id,store_id,goods_image,color_id');
        ...
        foreach ($spec_array as $key => $value) {
            ...
            $tpl_spec['url'] = urlShop('goods', 'index', array(
        ...
        // 商品多图
        $image_more = $this->getGoodsImageList(array(
            'goods_commonid' => $goods_info['goods_commonid'],
    ...
    {
        $this->cls();
        return $this->table('goods_images')
            ->field($field)
            ->where($condition)
            ->order($order)
            ->select();
    ...
    public function getViewedGoodsList()
    {
        ...
        if (cookie('viewed_goods')) {
            ...
            $viewed_list = $this->getGoodsList(array(
                'goods_id' => array(
                    'in',
                    $goodsid_array
                )
            ), 'goods_id, goods_name, goods_price, goods_image, store_id');
    ...
    public function delGoodsAll($condition)
    {
        ...
        $this->table('favorites')
            ...
            ->delete();
        // 删除优惠套装商品
        Model('p_bundling')->delBundlingGoods(array(
            'goods_id' => array(
                'in',
                $goodsid_array
            )
        ...
        // 推荐展位商品
        Model('p_booth')->delBoothGoods(array(
            'goods_id' => array(
                'in',
                $goodsid_array
            )
