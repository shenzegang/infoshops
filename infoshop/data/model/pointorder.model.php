...
class pointorderModel
{
 ...
 // 订单编号
    ...
    public function getPointOrderProdList($condition, $page, $field = '*', $type = 'simple')
    {
        ...
        switch ($type) {
            case 'all':
                ...
                $param['join_on'] = array(
                    '`points_ordergoods`.point_orderid = `points_order`.point_orderid'
                );
                break;
            case 'simple':
    ...
    public function getPointOrderInfo($condition, $type = 'all', $field = '*')
    {
        ...
        switch ($type) {
            case 'all':
                ...
                $param['join_type'] = 'left join';
                $param['join_on'] = array(
                    '`points_order`.point_orderid=`points_orderaddress`.point_orderid'
                );
                break;
            case 'simple':
    ...
    public function getPointOrderList($condition, $page, $type = 'all', $field = '*')
    {
        ...
        $param['field'] = $field;
        $param['order'] = $condition['order'] ? $condition['order'] : 'order_goods.order_id desc';
        $param['limit'] = $condition['limit'];
        $param['group'] = $condition['group'];
