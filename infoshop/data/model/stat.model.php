...
class statModel extends Model
{
    ...
    {
        if ($limit) {
            return $this->field('order_id,order_sn,store_name,buyer_name,add_time,order_amount,order_state')
                ->table('order')
                ->where($condition)
                ->order('add_time desc')
                ->page(15)
    ...
    public function getTradeInfo($type, $stime, $etime)
    {
        switch ($type) {
            case 'order_goods_num': // 下单商品数
                $data = $this->field('sum(goods_num) as allnum')
                    ->table('order_goods,order')
                    ->join('left join')
                    ->on('order_goods.order_id=order.order_id')
                    ->where(array(
                    'order.add_time' => array(
                        'between',
                        array(
                            $stime,
                            $etime
                    ...
                    'order.order_state' => array(
                        'neq',
                        ORDER_STATE_NEW
                    ),
                    'refund_state' => array(
                        'exp',
                        "!(order.order_state = '" . ORDER_STATE_CANCEL . "' and order.refund_state = 0)"
                    )
                ))
                    ->select();
                ...
                break;
            case 'order_num': // 下单单量
                $data = $this->field('count(*) as allnum')
                    ->table('order')
                    ->where(array(
                    'add_time' => array(
                        'between',
                    ...
                    'refund_state' => array(
                        'exp',
                        "!(order_state = '" . ORDER_STATE_CANCEL . "' and refund_state = 0)"
                    )
                ))
                    ->select();
                return $data[0]['allnum'];
            ...
            case 'order_buyer_num': // 下单客户数
                $data = $this->field('DISTINCT(buyer_id)')
                    ->table('order')
                    ->where(array(
                    'add_time' => array(
                        'between',
            ...
            case 'order_amount': // 合计金额
                $data = $this->field('sum(order_amount) as allnum')
                    ->table('order')
                    ->where(array(
                    'add_time' => array(
                        'between',
    ...
    public function getGoodsTradeRanking($type, $stime, $etime)
    {
        switch ($type) {
            case 'trade_num': // 按销量
                return $this->field('sum(goods_num) as allnum,goods_name,goods_id')
                    ->table('order_goods,order')
                    ->join('left join')
                    ->on('order_goods.order_id=order.order_id')
                    ->where(array(
                    ...
                    'payment_code' => array(
                        'exp',
                        "!(order.payment_code='offline' and order_state <> '" . ORDER_STATE_SUCCESS . "')"
                    )
                ))
            ...
            case 'trade_amount': // 按销售额
                return $this->field('sum(goods_price*goods_num) as allnum,goods_name,goods_id')
                    ->table('order_goods,order')
                    ->join('left join')
                    ->on('order_goods.order_id=order.order_id')
                    ->where(array(
    ...
    public function statByOrderCommon($where, $field = '*', $page = 0, $limit = 0, $order = '', $group = '')
    {
        if (is_array($page)) {
            if ($page[1] > 0) {
                return $this->table('order,order_common')
                    ->field($field)
                    ->join('left')
    ...
    public function getGoodsTradeDetailList($condition, $page = 15)
    {
        ...
        if (intval($page) > 0) {
            $count_allnum = $this->field('count(DISTINCT goods.goods_id) as countnum')
                ->table('goods,order_goods,goods_common,order')
                ->join('left join')
                ->on('goods.goods_id=order_goods.goods_id,goods.goods_commonid=goods_common.goods_commonid,order_goods.order_id=order.order_id')
                ->where($condition)
                ->select();
            $count_allnum = $count_allnum[0]['countnum'];
            return $this->field('sum(order_goods.goods_num) as gnum,count(*) as onum,sum(order_goods.goods_price*order_goods.goods_num) as pnum,goods.goods_name,goods.store_name,goods_common.goods_addtime,goods_common.goods_selltime,goods.goods_id')
                ->table('goods,order_goods,goods_common,order')
                ->join('left join')
                ->on('goods.goods_id=order_goods.goods_id,goods.goods_commonid=goods_common.goods_commonid,order_goods.order_id=order.order_id')
                ->where($condition)
    ...
    public function getStoreTradeList($condition, $type, $limit = '')
    {
        ...
        switch ($type) {
            case 'goods': // 返回商品销售列表
                return $this->field('sum(order_goods.goods_num) as gnum,count(*) as onum,sum(order_goods.goods_price*order_goods.goods_num) as pnum,goods.goods_name,goods.store_name,goods_class.gc_name,goods.goods_id')
                    ->table('goods,order_goods,order,goods_class')
                    ->join('left join')
                    ->on('goods.goods_id=order_goods.goods_id,order_goods.order_id=order.order_id,goods.gc_id=goods_class.gc_id')
                    ->where($condition)
                    ->group('goods.goods_id')
                    ->order('sum(order_goods.goods_price*order_goods.goods_num) desc')
                    ->limit($limit)
                    ->select();
                break;
            case 'store': // 返回店铺销售列表
                return $this->field('count(*) as onum,sum(order.order_amount) as pnum,store.store_name,store_class.sc_name,store.member_name')
                    ->table('store,order,store_class')
                    ->join('left join')
                    ->on('store.store_id=order.store_id,store.sc_id=store_class.sc_id')
                    ->where($condition)
                    ->group('store.store_id')
                    ->order('sum(order.order_amount) desc')
                    ->limit($limit)
                    ->select();
                break;
        }
    ...
    public function getBillList($condition, $type, $have_page = true)
    {
        switch ($type) {
            case 'os': // 平台
                return $this->field('sum(os_order_totals) as oot,sum(os_order_return_totals) as oort,sum(os_commis_totals-os_commis_return_totals) as oct,sum(os_store_cost_totals) as osct,sum(os_result_totals) as ort')
                    ->table('order_statis')
                    ->where($condition)
                    ->select();
                break;
            case 'ob': // 店铺
                $page = $have_page ? 15 : '';
                return $this->field('order_bill.*,store.member_name')
                    ->table('order_bill,store')
                    ->join('left join')
                    ->on('order_bill.ob_store_id=store.store_id')
                    ->where($condition)
                    ->page($page)
                    ->order('ob_no desc')
                    ->select();
    ...
    public function statByOrderLog($where, $field = '*', $page = 0, $limit = 0, $order = '', $group = '')
    {
        if (is_array($page)) {
            if ($page[1] > 0) {
                return $this->table('order_log,order')
                    ->field($field)
                    ->join('left')
                    ->on('order_log.order_id = order.order_id')
                    ->where($where)
                    ->group($group)
                    ->page($page[0], $page[1])
                    ->limit($limit)
    ...
    public function statByRefundreturn($where, $field = '*', $page = 0, $limit = 0, $order = '', $group = '')
    {
        if (is_array($page)) {
            if ($page[1] > 0) {
                return $this->table('refund_return')
                    ->field($field)
                    ->where($where)
                    ->group($group)
                    ->page($page[0], $page[1])
    ...
    public function statByStoreAndEvaluatestore($where, $field = '*', $page = 0, $limit = 0, $order = '', $group = '')
    {
        if (is_array($page)) {
            if ($page[1] > 0) {
                return $this->table('evaluate_store,store')
                    ->field($field)
                    ->join('left')
                    ->on('evaluate_store.seval_storeid=store.store_id')
                    ->where($where)
                    ->group($group)
                    ->page($page[0], $page[1])
                    ->limit($limit)
    ...
    public function getStoreSaleRank($condition, $type)
    {
        switch ($type) {
            case 'sale_amount': // 按销售额排行
                return $this->field('sum(order_amount) as allnum,store_name')
                    ->table('order')
                    ->where($condition)
                    ->order('allnum desc')
                    ->group('store_id')
            ...
            case 'sale_num': // 按下单量排行
                return $this->field('count(*) as allnum,store_name')
                    ->table('order')
                    ->where($condition)
                    ->order('allnum desc')
                    ->group('store_id')
