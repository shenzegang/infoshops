...
class goods_classModel extends Model
{
    ...
    {
        $result = $this->table('goods_class')
            ->field($field)
            ->where($condition)
            ->order('gc_parent_id asc,gc_sort asc,gc_id asc')
            ->limit(10000)
            ->select();
        return $result;
    }
    ...
    {
        // 读取商品分类
        $gc_list = $this->getGoodsClassList(array(
            'gc_parent_id' => $pid
        ), 'gc_id, gc_name, type_id');
    ...
    public function delGoodsClassByGcIdString($gcids)
    {
        ...
        foreach ($gcids as $gc_id) {
            ...
            $gcid_array = array_merge($gcid_array, array(
                $gc_id
            ), $child, $childchild);
        }
        // 删除商品分类
        $this->delGoodsClass(array(
            'gc_id' => array(
                'in',
                $gcid_array
            )
        ...
        // 删除常用商品分类
        Model('goods_class_staple')->delStaple(array(
            'gc_id_1|gc_id_2|gc_id_3' => array(
                'in',
                $gcid_array
            )
        ));
        // 删除分类tag表
        Model('goods_class_tag')->delGoodsClassTag(array(
            'gc_id_1|gc_id_2|gc_id_3' => array(
                'in',
                $gcid_array
            )
        ...
        // 删除店铺绑定分类
        Model('store_bind_class')->delStoreBindClass(array(
            'class_1|class_2|class_3' => array(
                'in',
                $gcid_array
            )
        ));
        ...
        Model('goods')->editProducesLockUp(array(
            'goods_stateremark' => '商品分类被删除，需要重新选择分类'
        ), array(
            'gc_id' => array(
                'in',
                $gcid_array
                ...
                foreach ($class_list as $key => $value) {
                    ...
                    if ($p_id == 0) { // 第1级分类
                        ...
                        if (file_exists($pic_name)) {
                            $value['pic'] = UPLOAD_SITE_URL . '/' . ATTACH_COMMON . '/category-pic-' . $value['gc_id'] . '.jpg';
                        }
    ...
    public function get_type_brands($type_ids = array())
    {
        ...
        if (is_array($type_ids) && ! empty($type_ids)) {
            ...
            $type_list = $this->table('type_brand')
                ->where(array(
                'type_id' => array(
                    'in',
                    $type_ids
    ...
    public function getGoodsClassNav($id = 0, $sign = 1)
    {
        if (intval($id) > 0) {
            ...
            if ($sign == 1) {
                ...
                $nav_link[] = array(
                    'title' => $data[$id]['gc_name'],
                    'link' => urlShop('search', 'index', array(
                        'cate_id' => $data[$id]['gc_id']
                    ))
                );
            }
            ...
            // 加上 首页 商品分类导航
            $nav_link[] = array(
                'title' => Language::get('goods_class_index_search_results')
            );
        }
        // 首页导航
        $nav_link[] = array(
