...
class storeModel extends Model
{
    ...
    public function getStoreInfo($condition)
    {
        ...
        if (! empty($store_info)) {
            ...
            $store_info['goods_count'] = $model_goods->getGoodsCommonOnlineCount(array(
    ...
    public function getStoreInfoByID($store_id)
    {
        ...
        if (empty($store_info)) {
            $store_info = $this->getStoreInfo(array(
                'store_id' => $store_id
            ));
    ...
    public function getHotSalesList($store_id, $limit = 5)
    {
        ...
        if (empty($hot_sales_list)) {
            $model_goods = Model('goods');
            $hot_sales_list = $model_goods->getGoodsOnlineList(array(
                'store_id' => $store_id
            ), '*', 0, 'goods_salenum desc', $limit);
    ...
    public function getHotCollectList($store_id, $limit = 5)
    {
        ...
        if (empty($hot_collect_list)) {
            $model_goods = Model('goods');
            $hot_collect_list = $model_goods->getGoodsOnlineList(array(
                'store_id' => $store_id
            ), '*', 0, 'goods_collect desc', $limit);
    ...
    /**
     * 2015-9-15 tjz增加 更新店铺信息
     */
    public  function  updateStoreInfo($condition,$update){
        return $this->where($condition)->update($update);
    }
}
