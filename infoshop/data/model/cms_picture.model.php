...
class cms_pictureModel extends Model
{
    ...
    public function getListByTagID($condition, $page = null, $order = '', $field = '*', $limit = '')
    {
        ...
        $result = $this->table('cms_picture,cms_tag_relation')
            ->field($field)
            ->join('left')
            ->on($on)
            ->where($condition)
        ...
        if (! empty($picture_list) && is_array($picture_list)) {
            ...
            foreach ($picture_list as $picture) {
                ...
                $picture_image_list = $model_picture_image->getList(array(
                    'image_picture_id' => $picture['picture_id']
                ), NULL);
            ...
            // 删除画报图片记录
            $model_picture_image->drop(array(
