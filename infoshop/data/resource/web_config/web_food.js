...
function update_recommend() {//更新
	...
	if (get_text=='1') {
		...
		obj.find("img").each(function() {
			...
			var goods_pic = goods.attr("src");
			var goods_name = goods.attr("title");
			text_append += '<li><img title="'+goods_name+'" src="'+goods_pic+'"/></li>';
		});
	  $("dl[recommend_id='"+id+"'] dd ul").html('');
	  $(".list ul").html(text_append);
...
function update_pic(id,pic) {//更新图片
	if (id == 'tit') {
	    ...
	    if (get_type == 'txt') {
            var html = tit_floor + ' ' + tit_title;
    	}else{
            var html = '<img src="'+UPLOAD_SITE_URL+'/'+pic+'" />';
        }
...
function update_top(){
    $('#top_list_cat').val($('#top_cat').val());
    $('#top_list_num').val($('#top_num').val());
    update_data('top_list');
    DialogManager.close("top_list");
}