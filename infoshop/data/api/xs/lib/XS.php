...
class XSFieldScheme implements IteratorAggregate
{
    ...
    public static function logger()
    {
        if (self::$_logger === null) {
            $scheme = new self();
            $scheme->addField('id', array(
                'type' => 'id'
            ));
            $scheme->addField('pinyin');
            $scheme->addField('partial');
            $scheme->addField('total', array(
                'type' => 'numeric',
                'index' => 'self'
            ));
            $scheme->addField('lastnum', array(
                'type' => 'numeric',
                'index' => 'self'
            ));
            $scheme->addField('currnum', array(
                'type' => 'numeric',
                'index' => 'self'
            ));
            $scheme->addField('currtag', array(
                'type' => 'string'
            ));
            $scheme->addField('body', array(
                'type' => 'body'
            ));
            self::$_logger = $scheme;
        }
...
class XSIndex extends XSServer
{
    ...
    public function del($term, $field = null)
    {
        ...
        $cmds = array();
        $terms = is_array($term) ? array_unique($term) : array(
            $term
        );
    ...
    public function setScwsMulti($level)
    {
        ...
        if ($level >= 0 && $level < 16) {
            $cmd = array(
                'cmd' => CMD_SEARCH_SCWS_SET,
                'arg1' => CMD_SCWS_SET_MULTI,
                'arg2' => $level
            );
    ...
    {
        $cmd = array(
            'cmd' => CMD_SEARCH_SCWS_GET,
            'arg1' => CMD_SCWS_GET_MULTI
        );
    ...
    {
        $this->execCommand(array(
            'cmd' => CMD_INDEX_REBUILD,
            'arg1' => 0
        ), CMD_OK_DB_REBUILD);
        $this->_rebuild = true;
        return $this;
...
class XSSearch extends XSServer
{
    ...
    public function getQuery($query = null)
    {
        ...
        if (strpos($res->buf, 'VALUE_RANGE') !== false) {
            ...
            $res->buf = preg_replace_callback($regex, array(
                $this,
                'formatValueRange'
            ), $res->buf);
        }
