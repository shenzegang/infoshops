			...
			<dl>
				<dt>
					<a href="<?php echo urlShop('show_joinin', 'index');?>"
						title="申请商家入驻；已提交申请，可查看当前审核状态。" target="_blank">商家登陆</a>
				</dt>
			</dl>
      <?php
...
<header id="topHeader">
	<div class="warp-all">
		<div class="circle-logo">
			<a href="<?php echo CIRCLE_SITE_URL;?>"><img
				src="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_CIRCLE.'/'.C('circle_logo');?>" /></a>
		</div>
		<div class="circle-search" id="circleSearch">
			<form id="form_search" method="get"
				action="<?php echo CIRCLE_SITE_URL;?>/index.php">
				<input type="hidden" name="act" value="search" />
				<div class="input-box">
					<i class="icon"></i> <input id="keyword" name="keyword" type="text"
						class="input-text"
						value="<?php echo isset($_GET['keyword'])?$_GET['keyword']:'';?>"
						maxlength="60" x-webkit-speech="" lang="zh-CN"
						onwebkitspeechchange="foo()" x-webkit-grammar="builtin:search" />
					<input id="btn_search" type="submit" class="input-btn"
						value="<?php echo $lang['nc_search_nbsp'];?>">
				</div>
				<div class="radio-box">
					<label> <input name="op" value="theme" type="radio"
						<?php if($output['search_sign']=='theme' || !isset($output['search_sign'])){?>
						checked="checked" <?php }?> />
						<h5><?php echo $lang['search_theme'];?></h5></label> <label> <input
						name="op" value="group" type="radio"
						<?php if($output['search_sign']=='group'){?> checked="checked"
						<?php }?> />
		...
		<div class="circle-user">
			<h2>
				<a
					href="<?php echo CIRCLE_SITE_URL;?>/index.php?act=search&op=group"><?php echo $lang['nc_find_fascinating'];?></a>
			</h2>
			<div class="head-portrait">
				<span class="thumb size20"> <i></i><img
					src="<?php  echo getMemberAvatarForID($_SESSION['member_id']);?>" /></span>
			</div>
			<div class="user-login">
        <?php if($_SESSION['is_login']){?>
        <div class="my-group"><?php echo $lang['my_circle'];?><span><i></i></span><span
						class="hidden" nctype="span-mygroup"> </span>
				</div>
        <?php }else{?>
        <a href="Javascript:void(0)" nctype="login"><?php echo $lang['nc_login'];?></a>
				| <a
					href="<?php echo SHOP_SITE_URL.'/';?>index.php?act=login&op=register"><?php echo $lang['nc_register'];?></a>
        <?php }?>
      </div>
		</div>
	</div>
