...
<div class="base-layout">
    ...
    <form id="circle_level" name="circle_level" method="post"
			action="<?php echo CIRCLE_SITE_URL;?>/index.php?act=manage_level&op=level&c_id=<?php echo $output['c_id']; ?>">
			<input type="hidden" value="ok" name="form_submit">
			<table class="base-table-style">
				<thead>
					<tr>
						<th colspan="15" class="tl">
							<ul class="level-set">
								<li><input type="radio" name="levelset" nctype="levelset"
									<?php if($output['ml_info']['mlref_id'] === '0'){?>
									checked="checked" <?php }?> id="levelset0" checked="checked"
									value="0"
									data-param="<?php echo str_replace('"',"'",json_encode($output['mld_array']));?>" />
              ...
              <li><input type="radio" name="levelset" nctype="levelset"
									<?php if($output['ml_info']['mlref_id'] == $key){?>
									checked="checked" <?php }?> id="levelset<?php echo $key;?>"
									value="<?php echo $key;?>"
									data-param="<?php echo str_replace('"',"'",json_encode($val['info']));?>" />
									<label for="levelset<?php echo $key;?>"><?php echo $val['name'];?></label>
								</li>
              <?php }?>
              <?php }?>
              <li><input type="radio" name="levelset" id="user-defined"
									<?php if(!empty($output['ml_info']) && $output['ml_info']['mlref_id'] == null){?>
									checked="checked"
									data-param="<?php echo str_replace('"',"'",json_encode($output['ml_info']['info']));?>"
									<?php }?> value="custom" /> <label for="user-defined"><?php echo $lang['circle_custom'];?></label>
								</li>
							</ul>
						</th>
					</tr>
        ...
        <tr>
						...
						<td nctype="mlname<?php echo $i;?>"><span
							class="member-level-name"><?php echo $output['mld_array'][$i]['name'];?></span></td>
						...
						<td nctype="mlname<?php echo $i+8;?>"><span
							class="member-level-name"><?php echo $output['mld_array'][$i+8]['name'];?></span></td>
					</tr>
        <?php }?>
        <?php }?>
      </tbody>
				<tfoot>
					<tr>
						<td colspan="10" class="bottom tc"><a href="JavaScript:void(0);"
							class="submit-btn" nctype="submitBtn"
							style="display: inline-block; float: none; margin: 10px auto 20px auto;"><?php echo $lang['circle_submit_setting'];?></a></td>
					</tr>
				</tfoot>
			</table>
		</form>
