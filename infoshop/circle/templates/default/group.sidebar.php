...
<div class="sidebar">
  ...
  <div class="my-info">
		<div class="avatar">
			<img
				src="<?php echo getMemberAvatarForID($output['cm_info']['member_id']);?>" /><a
		...
		<dl>
			<dt>
				<h3>
					<a href="index.php?act=p_center" target="_blank"><?php echo $lang['nc_inro_personal_center'];?></a>
				</h3>
			</dt>
			<dd>
        ...
        <div class="cm-exp">
          ...
          <?php }else{?>
          <p style="width: <?php echo intval($output['cm_info']['cm_nextexp']) != 0?sprintf('%.2f%%', intval($output['cm_info']['cm_exp'])/intval($output['cm_info']['cm_nextexp'])*100):0;?>">
					</p>
		...
		<div class="sidebar-circle-info tabs-panel tabs-hide">
          ...
          <?php if($output['class_info']['class_name'] != ''){ echo $output['class_info']['class_name'];}else{echo $lang['nc_default'];}?>
			...
			<dl>
				...
				<dd>
					<a target="_blank"
						href="<?php echo SHOP_SITE_URL;?>/index.php?act=sns_circle&op=theme&mid=<?php echo $output['creator']['member_id'];?>"
						class="master"
						title="<?php echo $output['creator']['member_name'];?>"
						target="_blank"><i></i><?php echo $output['creator']['member_name'];?></a>
				</dd>
			</dl>
			<dl>
          ...
          <a target="_blank"
						...
						class="moderator" title="<?php echo $val['member_name'];?>"
						target="_blank"><i></i><?php echo $val['member_name'];?></a>
          <?php }?>
          <?php }else{?>
	...
	<div class="side-tab-nav">
      ...
      <dl>
				...
				<dd class="member-avatar-s">
					<img src="<?php echo getMemberAvatarForID($val['member_id']);?>" /><em></em>
				</dd>
				<dd class="theme-num"
					title="<?php echo $lang['circle_theme_count'];?>">
					(<b><?php echo $val['cm_thcount'];?></b>)
				</dd>
			</dl>
      <?php }?>
      <?php }else{?>
      <p>
				<span class="no-member"><i></i><?php echo $lang['circle_no_firend'];?></span>
			</p>
      <?php }?>
    </div>
  ...
  <div class="sidebar-box">
        ...
        <li><span class="thumb size32"><i></i><a
						href="<?php echo CIRCLE_SITE_URL;?>/index.php?act=group&c_id=<?php echo $val['friendship_id'];?>"
						title="<?php echo $val['friendship_name'];?>"><img
							src="<?php echo circleLogo($val['friendship_id']);?>" /></a></span></li>
        <?php }?>
      </ul>
		</div>
	</div>
