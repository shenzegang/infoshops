...
<div class="base-layout">
    ...
    <form id="circle_form"
			action="<?php echo CIRCLE_SITE_URL;?>/index.php?act=manage&c_id=<?php echo $output['c_id'];?>"
			method="post" class="base-form-style">
			...
			<dl>
				...
				<dd>
					...
					<span class="count" id="noticecharcount"></span>
				</dd>
			</dl>
			<dl>
				...
				<dd>
					<div class="picture">
						<img id="view_img"
							src="<?php echo circleLogo($output['circle_info']['circle_id']);?>" />
					</div>
					<div class="upload-btn">
						<a href="javascript:void(0);"> <span> <input type="file"
								name="c_img" id="c_img" multiple="" file_id="0" class="file"
								size="1" hidefocus="true" maxlength="0" />
						</span>
							<div class="upload-button">
								<i></i><?php echo $lang['circle_image_upload'];?></div> <input
			...
			<dl>
				...
				<dd>
					<input type="radio" name="c_joinaudit" value="1"
						<?php if($output['circle_info']['circle_joinaudit'] == 1){?>
						checked="checked" <?php }?> />
					<h5 class="mr20"><?php echo $lang['nc_yes'];?></h5>
					<input type="radio" name="c_joinaudit" value="0"
						<?php if($output['circle_info']['circle_joinaudit'] == 0){?>
						checked="checked" <?php }?> />
			...
			<dl style="border-bottom: none;">
				...
				<dd>
					<input type="radio" name="c_mapply" value="1"
						<?php if($output['circle_info']['mapply_open'] == 1){?>
						checked="checked" <?php }?> />
					<h5 class="mr20"><?php echo $lang['nc_yes'];?></h5>
					<input type="radio" name="c_mapply" value="0"
						<?php if($output['circle_info']['mapply_open'] == 0){?>
						checked="checked" <?php }?> />
					<h5><?php echo $lang['nc_no'];?></h5>
				</dd>
			</dl>
			<dl>
				<dt></dt>
				<dd>
					<select name="c_ml" class="mr10">
						<option value="0"><?php echo $lang['circle_no_requirement'];?></option>
           <?php for($i=1; $i<=16; $i++){?>
           <option value="<?php echo $i;?>"
							<?php if($output['circle_info']['mapply_ml'] == $i){?>
							selected="selected" <?php }?>><?php echo $output['ml_info']['ml_'.$i].'&nbsp;LV'.$i;?></option>
           <?php }?>
			...
			<dl class="bottom">
				...
				<dd>
					<a href="Javascript: void(0)" class="submit-btn"
						nctype="submit-btn"><?php echo $lang['circle_submit_setting'];?></a>
				</dd>
			</dl>
		</form>
	</div>
