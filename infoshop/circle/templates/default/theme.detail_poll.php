...
<div class="theme-detail-poll-content">
	<form method="post" id="poll_form"
		...
		action="<?php echo CIRCLE_SITE_URL;?>/index.php?act=theme&op=save_votepoll&c_id=<?php echo $output['c_id'];?>&t_id=<?php echo $output['t_id'];?>"
		<?php }?>>
      ...
      <tr>
          ...
          <input type="checkbox" name="pollopid[]"
					value="<?php echo $val['pollop_id'];?>"
					<?php if($output['vote_end'] || $output['partake'] || !in_array($output['identity'], array(1,2,3))){?>
					disabled="disabled" <?php }?> />
          <?php }else{?>
          <input type="radio" name="pollopid[]"
					value="<?php echo $val['pollop_id'];?>"
					<?php if($output['vote_end'] || $output['partake'] || !in_array($output['identity'], array(1,2,3))){?>
					disabled="disabled" <?php }?> />
          <?php }?></td>
				...
				<td class="w230"><div class="poll-column">
						<p class="c<?php echo $i;?>" style="width: <?php echo intval($output['poll_info']['poll_voters']) != 0?sprintf('%.2f%%', intval($val['pollop_votes'])/intval($output['poll_info']['poll_voters'])*100):0;?>">
						</p>
						<i> <?php echo intval($output['poll_info']['poll_voters']) != 0?sprintf('%.2f%%', intval($val['pollop_votes'])/intval($output['poll_info']['poll_voters'])*100):'0.00%';?></i>
					</div></td>
    ...
    <?php if(!$output['vote_end'] && !$output['partake'] && in_array($output['identity'], array(1,2,3))){?>
    <div class="bottom">
			<a href="javascript:void(0);" nctype="poll_submit" class="btn"><?php echo $lang['nc_submit'];?></a><span
				nctype="poll-warning" class="warning"></span>
		</div>
    <?php }?>
  </form>
</div>
