...
<div class="group warp-all">
  ...
  <div class="theme-editor">
		<form method="post" id="theme_form"
			action="<?php echo CIRCLE_SITE_URL;?>/index.php?act=theme&op=edit_theme&sp=1&c_id=<?php echo $output['c_id'];?>&t_id=<?php echo $output['t_id']; ?>">
			<input type="hidden" name="form_submit" value="ok" />
			<div class="quick-thread">
				<div class="quick-thread-box">
					<div class="title">
						<label class="mr10"><span class="t"><?php echo $lang['circle_type'].$lang['nc_colon'];?></span><span
							class="i"> <select name="thtype" class="select">
                ...
                <option value="<?php echo $val['thclass_id'];?>"
										<?php if($output['theme_info']['thclass_id'] == $val['thclass_id']){?>
										selected="selected" <?php }?>><?php echo $val['thclass_name'];?></option>
                ...
                <?php }?>
						...
						</span></label> <label><span class="t"><?php echo $lang['nc_title'].$lang['nc_colon'];?></span><span
							class="i"> <input name="name" type="text" class="text"
								value="<?php echo $output['theme_info']['theme_name'];?>" />
						</span></label>
					</div>
              ...
              <?php echo $lang['circle_poll_options_max'];?></span> <span
								class="input-text">
								<h4><?php echo $lang['circle_poll_days'].$lang['nc_colon'];?></h4>
								<label> <input type="text" name="days"
									value="<?php if($output['poll_info']['poll_days'] != 0){echo $output['poll_info']['poll_days'];}?>" />
                <?php echo $lang['nc_day'];?></label>
							</span> <span class="input-radio">
								<h4><?php echo $lang['circle_poll_patterns'].$lang['nc_colon'];?></h4>
								<label> <input type="radio" name="multiple" value="0"
									<?php if($output['poll_info']['poll_multiple'] == 0){?>
									checked="checked" <?php }?> />
									<h5><?php echo $lang['circle_poll_radio'];?></h5>
							</label> <label> <input type="radio" name="multiple" value="1"
						...
						<div id="poll_div_1" class="add-poll">
							<div class="titbar">
								<span class="sort"><?php echo $lang['circle_poll_sort'];?></span><span
									class="option"><?php echo $lang['circle_option'];?></span>
							</div>
              <?php if(!empty($output['option_list'])){?>
              <?php foreach ($output['option_list'] as $val){?>
              <p>
								<input type="text"
									name="pollsort[<?php echo $val['pollop_id'];?>]"
									value="<?php echo $val['pollop_sort'];?>" class="sort" /> <input
									type="text" name="polloption[<?php echo $val['pollop_id'];?>]"
									value="<?php echo $val['pollop_option'];?>" class="option">
							</p>
              <?php }?>
              <?php }?>
              <a href="javascript:void(0);" nctype="addpolloption"
								class="btn"><i></i><?php echo $lang['circle_add_new'];?></a>
						</div>
					</div>
          ...
          <div class="bottom">
						<a class="submit-btn" nctype="theme_submit"
							href="Javascript: void(0)"><?php echo $lang['nc_edit_theme'];?></a>
						<a class="cancel-btn" nctype="theme_cancle"
							href="Javascript:history.go(-1);"><?php echo $lang['nc_cancel'];?></a>
						<div id="warning"></div>
					</div>
				</div>
			</div>
