...
<div class="add-group">
	<dl class="top-box">
		<dt>
			<h3><?php echo $lang['circle_create_a_group'];?><i><?php echo $lang['circle_first_from'];?> <a
					href="<?php echo CIRCLE_SITE_URL;?>/index.php?act=search&op=group"><?php echo $lang['nc_find_fascinating'];?></a> <?php echo $lang['circle_find_like_group'];?></i>
			</h3>
			<p><?php echo $lang['nc_welcome_at'].C('circle_name').$lang['nc_welcome_words'];?></p>
		</dt>
		<dd>
			<span><?php echo $lang['circle_allow_create_group_count'].$lang['nc_colon'];?><em><?php echo C('circle_createsum');?></em></span><span><?php echo $lang['circle_yet_create_group_count'].$lang['nc_colon'];?><em><?php echo $output['create_count'];?></em></span>
		</dd>
		<dd>
			<span><?php echo $lang['circle_allow_join_group_count'].$lang['nc_colon'];?><em><?php echo C('circle_joinsum');?></em></span><span><?php echo $lang['circle_yet_join_group_count'].$lang['nc_colon'];?><em><?php echo $output['join_count'];?></em></span>
		</dd>
	</dl>
	<div class="base-form-style">
		<form method="post" id="groupadd_form"
			action="<?php echo CIRCLE_SITE_URL;?>/index.php?act=index&op=add_group">
			<input type="hidden" name="form_submit" value="ok" />
			<dl>
				<dt class="">
					<em></em><?php echo $lang['circle_belong_to_class'].$lang['nc_colon'];?></dt>
				<dd class="group-classes">
          <?php if(!empty($output['class_list'])){?>
          <ul>
            <?php foreach($output['class_list'] as $val){?>
            <li class="fl mr20 mb5"><input type="radio" name="class_id"
							value="<?php echo $val['class_id'];?>" />
          ...
          <p class="field_notice"></p>
			...
			<dl>
				<dt class="required">
					<em></em><?php echo $lang['circle_name'].$lang['nc_colon'];?></dt>
				<dd>
					<p>
						<input type="text" name="c_name" id="c_name" class="text w400"
							value="<?php echo $_GET['kw'];?>" />
					</p>
					<p class="field_notice"></p>
			...
			<dl>
				...
				<dd>
					<p>
						<textarea name="c_pursuereason" id="c_pursuereason"
							class="textarea w400 h100"></textarea>
          ...
          <?php echo $lang['circle_my_read_carefully_agree'];?><a
						target="_blank"
						href="<?php echo SHOP_SITE_URL;?>/index.php?act=document&code=create_circle"><?php echo $lang['circle_notice_for_use']?></a><?php echo $lang['circle_all_terms'];?></dd>
			</dl>
			<dl class="bottom">
				<dt>&nbsp;</dt>
				<dd>
					<a class="submit-btn" nctype="submit-btn"
						href="Javascript: void(0)"><?php echo $lang['circle_submit_applications'];?></a>
				</dd>
			</dl>
		</form>
	</div>
