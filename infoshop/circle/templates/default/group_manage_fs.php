    ...
    <table class="base-table-style">
          ...
          <td class="tc"><input id="all" class="checkall"
					...
					<td colspan="3"><label for="all" class="handle-btn"><i class="ac0"></i><?php echo $lang['nc_check_all'];?></label>
						...
						nctype="batchbutton"
						uri="<?php echo CIRCLE_SITE_URL;?>/index.php?act=manage&op=class_del&c_id=<?php echo $output['c_id'];?>"
						name="thc_id" confirm="<?php echo $lang['nc_ensure_del'];?>"><i
							class="ac5"></i><?php echo $lang['nc_delete'];?></a></td>
          <?php }?>
          <td colspan="20" class="tr"><a href="javascript:void(0);"
						nctype="add_fs" class="add-group-class-btn mr10"><?php echo $lang['fcircle_add'];?></a></td>
				</tr>
			</thead>
			<tbody>
        <?php if(!empty($output['fs_list'])){?>
        ...
        <tr>
					<td><input type="checkbox" class="checkitem"
						value="<?php echo $val['friendship_id'];?>" /></td>
					<td class="tl"><dl class="group-base">
							<dt class="group-name">
								<a
									href="<?php echo CIRCLE_SITE_URL;?>/index.php?act=group&c_id=<?php echo $val['friendship_id'];?>"
									target="_blank"><?php echo $val['friendship_name'];?></a>
							</dt>
							<dd class="group-pic">
								<img src="<?php echo circleLogo($val['friendship_id']);?>">
							</dd>
						</dl></td>
					...
					<td class="handle"><a href="javascript:void(0);"
						onclick="edit_fs(<?php echo $val['friendship_id'];?>)"><?php echo $lang['nc_edit'];?></a>&nbsp;|&nbsp;<a
						href="javascript:void(0);"
						onclick="del_fs(<?php echo $val['friendship_id'];?>)"><?php echo $lang['nc_delete'];?></a></td>
				</tr>
        <?php }?>
        <?php }else{?>
        <tr>
      ...
      <tfoot>
				<tr>
					...
					<th colspan="20"><label for="all" class="handle-btn"><i class="ac0"></i><?php echo $lang['nc_check_all'];?></label>
						<a href="javascript:void(0);" class="handle-btn"
						nctype="batchbutton"
						uri="<?php echo CIRCLE_SITE_URL;?>/index.php?act=manage&op=friendship_del&c_id=<?php echo $output['c_id'];?>"
						name="fs_id" confirm="<?php echo $lang['nc_ensure_del'];?>"><i
							class="ac5"></i><?php echo $lang['nc_delete'];?></a></th>
				</tr>
			</tfoot>
      <?php }?>
    </table>
