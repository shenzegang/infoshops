...
class manageControl extends BaseCircleManageControl
{
    ...
    public function member_manageOp()
    {
        ...
        $cm_list = $model->table('circle_member')
            ->where($where)
            ->order('is_identity asc,cm_jointime desc')
            ->page(15)
            ->select();
    ...
    public function delmemberOp()
    {
        ...
        if (chksubmit()) {
            ...
            // 更新圈子成员数量
            $count = Model()->table('circle_member')
                ->where(array(
                'circle_id' => $this->c_id,
                'cm_state' => 1
            ))
                ->count();
            Model()->table('circle')->update(array(
                'circle_id' => $this->c_id,
                'circle_mcount' => $count
            ));
    ...
    public function setmanageOp()
    {
        ...
        if ($_GET['type'] == 'yes') {
            $cmid_array = $this->removeManager($cmid_array); // 去除圈主和管理
            $manage_count = Model()->table('circle_member')
                ->where(array(
                'circle_id' => $this->c_id,
                'is_identity' => 2
            ))
    ...
    public function classOp()
    {
        ...
        $model = Model();
        $thclass_list = $model->table('circle_thclass')
            ->where(array(
            'circle_id' => $this->c_id
        ))
            ->order('thclass_sort asc')
            ->select();
    ...
    public function class_addOp()
    {
        ...
        if (chksubmit()) {
            // 超过10不能继续添加
            $count = Model()->table('circle_thclass')
                ->where(array(
                'circle_id' => $this->c_id
            ))
                ->count();
            ...
            $obj_validate->validateparam = array(
                array(
                    "input" => $_POST["name"],
                    "require" => "true",
                    "message" => L('circle_tclass_name_not_null')
                ),
                array(
                    "input" => $_POST["sort"],
                    "require" => "true",
    ...
    public function class_editOp()
    {
        ...
        $thclass_info = Model()->table('circle_thclass')
            ->where(array(
            'circle_id' => $this->c_id,
            'thclass_id' => $thc_id
        ))
    ...
    public function friendshipOp()
    {
        ...
        $fs_list = Model()->table('circle_fs')
            ->where(array(
            'circle_id' => $this->c_id
        ))
            ->order('friendship_sort asc')
            ->select();
    ...
    public function search_circleOp()
    {
        ...
        // 查询已加友情圈子
        $circle_list = $model->table('circle_fs')
            ->where(array(
            'circle_id' => $this->c_id
        ))
            ->select();
        ...
        $circle_list = $model->table('circle')
            ->field('circle_id,circle_name')
            ->where($where)
            ->select();
        if (empty($circle_list)) {
            echo 'false';
    ...
    public function friendship_editOp()
    {
        ...
        $friendship_info = Model()->table('circle_fs')
            ->where(array(
            'circle_id' => $this->c_id,
            'friendship_id' => $fs_id
        ))
    ...
    public function edit_themeOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($rs) {
                ...
                $has_affix = 0; // 存在附件标记
                                // 删除原有商品
                $goods_list = Model()->table('circle_thg')
                    ->where(array(
                    'theme_id' => $t_id,
                    'reply_id' => 0
                ))
                ...
                // 更新话题信息
                $affixe_count = $model->table('circle_affix')
                    ->where(array(
                    'affix_type' => 1,
                    'theme_id' => $t_id
                ))
                ...
                if ($_GET['sp'] == 1) {
                    ...
                    if (! empty($_POST['polloption'])) {
                        ...
                        foreach ($_POST['polloption'] as $key => $val) {
                            $option_info = $model->table('circle_thpolloption')
                                ->where(array(
                                'pollop_id' => $key,
                                'theme_id' => $t_id
                            ))
        ...
        // 话题信息
        $this->theme_info = $model->table('circle_theme')
            ->where(array(
            'theme_id' => $t_id,
            'circle_id' => $this->c_id
        ))
        ...
        // 话题附件
        $affix_list = $model->table('circle_affix')
            ->where(array(
            'affix_type' => 1,
            'theme_id' => $t_id
        ))
        ...
        if ($this->theme_info['theme_special'] == 1) {
            ...
            $option_list = $model->table('circle_thpolloption')
                ->where(array(
                'theme_id' => $t_id
            ))
                ->order('pollop_sort asc')
                ->select();
    ...
    public function delimgOp()
    {
        ...
        // 附件详细
        $affix_info = Model()->table('circle_affix')
            ->where(array(
        ...
        // 验证是否为该圈子附件
        $theme_info = Model()->table('circle_theme')
            ->where(array(
            'circle_id' => $this->c_id,
    ...
    public function del_replyOp()
    {
        ...
        $where['reply_id'] = $r_id;
        $where['circle_id'] = $this->c_id;
        $reply_info = $model->table('circle_threply')
            ->where($where)
            ->find();
        if (empty($reply_info)) {
        ...
        $rs = $model->table('circle_threply')
            ...
            ->delete();
        // 更新话题回复数
        $model->table('circle_theme')->update(array(
            'theme_id' => $t_id,
            'theme_commentcount' => array(
                'exp',
    ...
    public function ajax_nospeakOp()
    {
        ...
        // 圈主和管理不能被禁言
        $m_info = Model()->table('circle_member')
            ->where($where)
            ->find();
        if (in_array($m_info['is_identity'], array(
            1,
            2
        ))) {
