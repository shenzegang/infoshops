...
class indexControl extends BaseCircleControl
{
    ...
    public function indexOp()
    {
        ...
        if (! empty($circle_list)) {
            ...
            // 今天发表的主题
            $nowthemecount_array = $model->table('circle_theme')
                ->field('count(circle_id) as count,circle_id')
                ->group('circle_id')
                ->where(array(
                'theme_addtime' => array(
                    'gt',
            ...
            // 今天新加入的成员
            $nowjoincount_array = $model->table('circle_member')
                ->field('count(circle_id) as count,circle_id')
                ->group('circle_id')
                ->where(array(
                'cm_jointime' => array(
        ...
        // 圈子分类
        $class_list = $model->table('circle_class')
            ->where(array(
            'class_status' => 1,
            'is_recommend' => 1
        ))
            ->order('class_sort asc')
            ->select();
        Tpl::output('class_list', $class_list);
        
        // 推荐圈子
        $rcircle_list = $model->table('circle')
            ->field('*, is_recommend*rand() as rand')
            ->where(array(
            'circle_status' => 1,
            'is_recommend' => 1
        ))
        ...
        // 商品话题
        $gtheme_list = $model->table('circle_theme')
            ->where(array(
            'has_goods' => 1,
            'is_closed' => 0
        ))
        ...
        if (! empty($gtheme_list)) {
            ...
            // 圈子商品
            $thg_list = $model->table('circle_thg')
                ->where(array(
                'theme_id' => array(
                    'in',
                    $themeid_array
        ...
        if (! empty($member_list)) {
            ...
            $one_membertheme = $model->table('circle_theme')
                ->where($where)
                ->order('theme_id desc')
                ->limit(4)
                ->select();
            ...
            if (! empty($member_list)) {
                ...
                $more_membertheme = $model->table('circle_member,circle_theme')
                    ->join('inner')
                    ->on('circle_member.member_id = circle_theme.member_id and circle_member.circle_id = circle_theme.circle_id')
                    ->where($where)
                    ->group('circle_member.member_id,circle_member.circle_id')
                    ->select();
    ...
    public function add_groupOp()
    {
        ...
        $create_count = $model->table('circle')
            ->where($where)
            ->count();
        ...
        $join_count = $model->table('circle_member')
            ->where($where)
            ->count();
        ...
        if (chksubmit()) {
            ...
            $obj_validate->validateparam = array(
                array(
                    "input" => $_POST["c_name"],
                    "require" => "true",
                    "message" => L('circle_name_not_null')
                )
            );
    ...
    public function myjoinedcircleOp()
    {
        ...
        $cm_list = $model->table('circle_member')
            ->field('circle_id,circle_name,is_identity')
            ->where(array(
            'member_id' => $_SESSION['member_id']
        ))
            ->order('is_identity asc')
            ->select();
        if (empty($cm_list)) {
            echo false;
            die();
