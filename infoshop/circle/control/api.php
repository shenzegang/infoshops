...
class apiControl extends BaseCircleControl
{
    ...
    public function get_theme_listOp()
    {
        ...
        $theme_list = $model->table('circle_theme')
            ->field('*, is_recommend*rand()*10000 + has_affix*rand() as rand')
            ->where(array(
            'circle_status' => 1,
            'is_closed' => 0
        ))
    ...
    public function get_more_memberthemeOp()
    {
        ...
        $more_membertheme = $model->table('circle_member,circle_theme')
            ->field('circle_member.*,circle_theme.*, circle_member.is_recommend*10000*rand()+(circle_member.cm_thcount)/10000  as rand')
            ->order('rand desc')
            ->join('inner')
