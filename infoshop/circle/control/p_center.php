...
class p_centerControl extends BaseCirclePersonalControl
{
    ...
    public function indexOp()
    {
        ...
        if (! empty($theme_list)) {
            ...
            // like
            $like_list = $model->table('circle_like')
                ->where(array(
                'theme_id' => array(
                    'in',
                    $themeid_array
    ...
    {
        $model = Model();
        $like_array = $model->table('circle_like')
            ->field('circle_id,theme_id')
            ->where(array(
            'member_id' => $this->m_id
        ))
            ->order('theme_id desc')
    ...
    {
        $model = Model();
        $circlemember_array = $model->table('circle_member')
            ->where(array(
            'member_id' => $this->m_id
        ))
            ->select();
    ...
    public function my_informOp()
    {
        ...
        $inform_list = $model->table('circle_inform')
            ->where($where)
            ->page(10)
    ...
    public function delinformOp()
    {
        ...
        $where = array();
        $where['member_id'] = $_SESSION['member_id'];
        $where['inform_id'] = array(
            'in',
            $inform_id
        );
        Model()->table('circle_inform')
            ->where($where)
            ->delete();
    ...
    {
        $model = Model();
        $recycle_list = $model->table('circle_recycle')
            ->where(array(
            'member_id' => $_SESSION['member_id']
        ))
            ->order('recycle_id desc')
            ->page(10)
            ->select();
