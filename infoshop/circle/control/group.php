...
class groupControl extends BaseCircleThemeControl
{
    ...
    public function indexOp()
    {
        ...
        $theme_list = $model->table('circle_theme')
            ->where($where)
        ...
        $todaythcount = $model->table('circle_theme')
            ->where(array(
            'theme_addtime' => array(
                'egt',
                $dayBegin
    ...
    public function applyOp()
    {
        ...
        if (chksubmit()) {
            ...
            $obj_validate->validateparam = array(
                array(
                    "input" => $_POST["apply_content"],
                    "require" => "true",
                    "message" => L('circle_apply_content_null')
                ),
                array(
                    "input" => $_POST["intro"],
                    "require" => "true",
                    "message" => L('circle_introduction_not_null')
                )
            );
    ...
    public function quitOp()
    {
        ...
        if (in_array($this->identity, array(
            ...
            $update['circle_mcount'] = array(
                'exp',
                'circle_mcount-1'
            );
            
            // Whether to apply for management
            $rs = Model()->table('circle_mapply')
                ->where(array(
                'circle_id' => $this->c_id,
                'member_id' => $_SESSION['member_id']
            ))
    ...
    public function group_membereditOp()
    {
        ...
        $member_info = $model->table('circle_member')
            ->where($where)
            ->find();
    ...
    public function group_goodsOp()
    {
        ...
        // 成员商品列表
        $model = Model();
        $cmid_list = $model->table('circle_member')
            ->field('member_id')
            ->where(array(
            'circle_id' => $this->c_id,
            'cm_state' => 1
        ...
        $count = $model->table('sns_sharegoods')
            ->where(array(
            'share_memberid' => array(
                'in',
                $cmid_array
            )
        ))
            ->count();
        $goods_list = $model->table('sns_sharegoods,sns_goods')
            ->join('left')
            ->on('sns_sharegoods.share_goodsid=sns_goods.snsgoods_goodsid')
            ->where(array(
            'sns_sharegoods.share_memberid' => array(
                'in',
                $cmid_array
            ),
    ...
    public function manage_applyOp()
    {
        ...
        $this->circleInfo();
        // Verify membership
        $cm_info = $model->table('circle_member')
            ->where(array(
            'circle_id' => $this->c_id,
            'cm_state' => 1,
            'is_identity' => 3
        ...
        $mapply_info = $model->table('circle_mapply')
            ->where(array(
            'circle_id' => $this->c_id,
            'member_id' => $_SESSION['member_id']
        ))
