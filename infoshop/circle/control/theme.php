...
class themeControl extends BaseCircleThemeControl
{
 ...
 // 身份 0游客 1圈主 2管理 3成员 4申请中 5申请失败
    ...
    public function ajax_themeinfoOp()
    {
        ...
        // 是否赞过话题
        $like_info = $model->table('circle_like')
            ->where(array(
            'theme_id' => $this->t_id,
    ...
    public function ajax_quickreplyOp()
    {
        ...
        $data['member_avatar'] = getMemberAvatarForID($_SESSION['member_id']); // 头像
                                                                               // 回复
        $reply_list = Model()->table('circle_threply')
            ->where(array(
            'theme_id' => $this->t_id,
            'circle_id' => $this->c_id
        ))
    ...
    public function save_themeOp()
    {
        if (chksubmit()) {
            ...
            // 不是圈子成员不能发帖
            if (! in_array($this->identity, array(
                1,
                2,
                3
            ))) {
    ...
    public function save_votepollOp()
    {
        if (chksubmit()) {
            ...
            $voter_info = $model->table('circle_thpollvoter')
                ->where(array(
                'theme_id' => $this->t_id,
                'member_id' => $_SESSION['member_id']
            ))
            ...
            foreach ($option_list as $val) {
                ...
                $update['pollop_votes'] = array(
                    'exp',
                    'pollop_votes+1'
                );
                $update['pollop_votername'] = array(
                    'exp',
            ...
            $update['poll_voters'] = array(
                'exp',
                'poll_voters+' . count($option_list)
            );
    ...
    public function choose_imageOp()
    {
        ...
        $pic_list = $model->table('sns_albumpic')
            ->where($where)
            ->page(6)
            ->select();
        ...
        $class_list = $model->table('sns_albumclass')
            ->where(array(
            'member_id' => $_SESSION['member_id']
        ))
            ->select();
    ...
    public function image_uploadOp()
    {
        ...
        $count = Model()->table('circle_affix')
            ->where($where)
            ->count();
        if ($count < 10) {
    ...
    {
        $affix_list = Model()->table('circle_affix')
            ->field('affix_id,affix_filename,affix_filethumb')
            ->where(array(
            'member_id' => $_SESSION['member_id'],
            'affix_type' => 1,
            'theme_id' => 0
    ...
    public function theme_detailOp()
    {
        ...
        // goods
        $goods_array = $model->table('circle_thg')
            ->where($where)
            ->select();
        ...
        // affix
        $affix_array = $model->table('circle_affix')
            ->where($where)
            ->select();
        ...
        $member_list = $model->table('circle_member')
            ->field('member_id,cm_level,cm_levelname')
            ->where(array(
            'circle_id' => $this->c_id,
            'member_id' => array(
                'in',
    ...
    public function ajax_likeyesOp()
    {
        ...
        if (empty($like_info)) {
            // 插入话题赞表
            Model()->table('circle_like')->insert(array(
                'theme_id' => $this->t_id,
                'member_id' => $_SESSION['member_id'],
                'circle_id' => $this->c_id
            ));
    ...
    public function edit_replyOp()
    {
        ...
        if (chksubmit()) {
            ...
            $obj_validate->validateparam = array(
                array(
                    "input" => $_POST["replycontent"],
                    "require" => "true",
                    "message" => L('circle_reply_not_null')
                )
            );
            $error = $obj_validate->validate();
            if ($error != '') {
        ...
        if (chksubmit()) {
            ...
            if ($error != '') {
                ...
                if ($reply_id) {
                    ...
                    $update = array();
                    $update['theme_id'] = $this->t_id;
                    $update['theme_commentcount'] = array(
                        'exp',
