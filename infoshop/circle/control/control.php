...
class BaseCircleControl
{
 ...
 // Members read permissions
    ...
    {
        $manager_list = Model()->table('circle_member')
            ->where(array(
            'circle_id' => $this->c_id,
            'is_identity' => array(
                'in',
    ...
    protected function memberInfo()
    {
        if ($_SESSION['is_login']) {
            $this->cm_info = Model()->table('circle_member')
                ->where(array(
                'circle_id' => $this->c_id,
    ...
    protected function sidebar()
    {
        ...
        // 明星圈友
        $star_member = Model()->table('circle_member')
            ->where(array(
            'cm_state' => 1,
            'circle_id' => $this->c_id,
            'is_star' => 1
        ...
        // 最新加入
        $newest_member = Model()->table('circle_member')
            ->where(array(
            'cm_state' => 1,
            'circle_id' => $this->c_id
        ))
        ...
        // 友情圈子
        $friendship_list = Model()->table('circle_fs')
            ->where(array(
            'circle_id' => $this->c_id,
            'friendship_status' => 1
        ))
    ...
    protected function themeTop()
    {
        $model = Model();
        // 最新话题
        $new_themelist = $model->table('circle_theme')
            ->where(array(
            'is_closed' => 0
        ))
            ->order('theme_id desc')
        ...
        // 热门话题
        $hot_themelist = $model->table('circle_theme')
            ->where(array(
            'is_closed' => 0
        ))
            ->order('theme_browsecount desc')
            ->limit(10)
            ->select();
        Tpl::output('hot_themelist', $hot_themelist);
        // 人气回复
        $reply_themelist = $model->table('circle_theme')
            ->where(array(
            'is_closed' => 0
        ))
            ->order('theme_commentcount desc')
            ->limit(10)
            ->select();
...
class BaseCircleThemeControl extends BaseCircleControl
{
 ...
 // Members of the information
    ...
    protected function checkReplySelf()
    {
        ...
        $this->reply_info = Model()->table('circle_threply')
            ->where(array(
            'theme_id' => $this->t_id,
            'reply_id' => $this->r_id,
            'member_id' => $_SESSION['member_id']
...
class BaseCircleManageControl extends BaseCircleControl
{
 ...
 // 会员信息
     ...
     * 身份验证
     */
    protected function checkIdentity($type)
    { // c圈主 m管理 cm圈主和管理
        $this->cm_info = Model()->table('circle_member')
            ->where(array(
            'circle_id' => $this->c_id,
    ...
    {
        // 所属圈子信息
        $circle_array = Model()->table('circle,circle_member')
            ->field('circle.*,circle_member.is_identity')
            ->join('inner')
            ->on('circle.circle_id=circle_member.circle_id')
            ->where(array(
            'circle_member.member_id' => $_SESSION['member_id']
        ))
            ->select();
...
class BaseCirclePersonalControl extends BaseCircleControl
{
 ...
 // memeber ID
    ...
    protected function circleMemberInfo()
    {
        ...
        $circlemember_list = Model()->table('circle_member')
            ->where(array(
            'member_id' => $this->m_id
        ))
            ->select();
