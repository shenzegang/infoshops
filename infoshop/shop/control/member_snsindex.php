...
class member_snsindexControl extends BaseMemberControl
{
 ...
 // 允许插入新记录的最大条数(注意在sns中该常量是一样的，注意与member_snshome中的该常量一致)
    ...
    public function indexOp()
    {
        ...
        $visitme_list = $visitor_model->getVisitorList(array(
            'v_ownermid' => "{$_SESSION['member_id']}",
            'limit' => 9
        ));
        ...
        // 查询我访问过的人
        $visitother_list = $visitor_model->getVisitorList(array(
            'v_mid' => "{$_SESSION['member_id']}",
            'limit' => 9
        ));
    ...
    public function sharegoodsOp()
    {
        if ($_POST['form_submit'] == 'ok') {
            ...
            // 判断该商品是否已经存在分享或者喜欢记录
            $sharegoods_info = $sharegoods_model->getSharegoodsInfo(array(
            ...
            if (empty($sharegoods_info)) {
                ...
                $result = $sharegoods_model->editSharegoods($update_arr, array(
                    'share_id' => "{$sharegoods_info['share_id']}"
                ));
                unset($update_arr);
            }
            if ($result) {
                ...
                $snsgoods_info = $snsgoods_model->getGoodsInfo(array(
                    'snsgoods_goodsid' => "{$goods_info['goods_id']}"
                ));
                if (empty($snsgoods_info)) {
                    ...
                    $snsgoods_model->editGoods($update_arr, array(
                        'snsgoods_goodsid' => "{$goods_info['goods_id']}"
                    ));
                }
                // 添加分享动态
                ...
                $content_str .= "<div class=\"fd-media\">
					<div class=\"goodsimg\"><a target=\"_blank\" href=\"" . urlShop('goods', 'index', array(
                    'goods_id' => $goods_info['goods_id']
                )) . "\"><img src=\"" . thumb($goods_info, 240) . "\" onload=\"javascript:DrawImage(this,120,120);\" alt=\"{$goods_info['goods_name']}\"></a></div>
					<div class=\"goodsinfo\">
						<dl>
							<dt><a target=\"_blank\" href=\"" . urlShop('goods', 'index', array(
                    'goods_id' => $goods_info['goods_id']
                )) . "\">" . $goods_info['goods_name'] . "</a></dt>
                ...
                if (C('share_isuse') == 1) {
                    ...
                    $params['url'] = urlShop('goods', 'index', array(
                        'goods_id' => $goods_info['goods_id']
                    ));
            ...
            // 查询收藏商品
            $favorites_list = Model()->table('favorites')
                ->field('fav_id')
                ->where(array(
                'member_id' => $_SESSION['member_id'],
                'fav_type' => 'goods'
    ...
    public function sharestoreOp()
    {
        if ($_POST['form_submit'] == 'ok') {
            ...
            // 判断该商品是否已经分享过
            $sharestore_info = $sharestore_model->getSharestoreInfo(array(
                'share_memberid' => "{$_SESSION['member_id']}",
            ...
            if (empty($sharestore_info)) {
                ...
                $result = $sharestore_model->editSharestore($update_arr, array(
                    'share_id' => "{$sharestore_info['share_id']}"
                ));
                unset($update_arr);
            }
            if ($result) {
                ...
                $store_info['store_url'] = urlShop('show_store', 'index', array(
    ...
    public function tracelistOp()
    {
        ...
        if (! empty($tracelist)) {
            foreach ($tracelist as $k => $v) {
                ...
                $info = $member_model->infoMember(array(
                    'member_id' => $v['trace_memberid']
                ));
                
                $v['trace_membername'] = $info['member_name'];
                $v['trace_memberavatar'] = $info['member_avatar'];
    ...
    public function editprivacyOp()
    {
        ...
        $privacy = in_array($_GET['privacy'], array(
            0,
            1,
            2
        )) ? $_GET['privacy'] : 0;
        $result = $sharegoods_model->editSharegoods(array(
    ...
    public function delgoodsOp()
    {
        ...
        if ($result) {
            ...
            if ($_GET['type'] == 'like') {
                $snsgoods_model = Model('sns_goods');
                $snsgoods_info = $snsgoods_model->getGoodsInfo(array(
                    'snsgoods_goodsid' => "{$sharegoods_info['share_goodsid']}"
                ));
                if (! empty($snsgoods_info)) {
                    $update_arr = array();
                    ...
                    $snsgoods_model->editGoods($update_arr, array(
                        'snsgoods_goodsid' => "{$snsgoods_info['snsgoods_goodsid']}"
                    ));
                }
            }
            $js = "location.reload();";
    ...
    public function storeprivacyOp()
    {
        ...
        $result = $sharestore_model->editSharestore(array(
            'share_privacy' => "$privacy"
        ), $condition);
        if ($result) {
            $privacy_item = $privacy + 1;
    ...
    public function addcommentOp()
    {
        ...
        if ($originaltype == 1) {
            ...
            // 查询原帖信息
            $tracelog_model = Model('sns_tracelog');
            $tracelog_info = $tracelog_model->getTracelogRow(array(
                'trace_id' => "{$originalid}",
                'trace_state' => '0'
            ));
        ...
        if ($result) {
            if ($originaltype == 1) {
                // 更新商品的评论数
                $update_arr = array();
                $update_arr['share_commentcount'] = array(
                    'sign' => 'increase',
                    'value' => '1'
                );
                $sharegoods_model->editSharegoods($update_arr, array(
                    'share_id' => "{$originalid}"
                ));
            } else {
                // 更新动态统计信息
                $update_arr = array();
                $update_arr['trace_commentcount'] = array(
                    'sign' => 'increase',
                    'value' => '1'
                );
                if (intval($tracelog_info['trace_originalid']) == 0) {
                    $update_arr['trace_orgcommentcount'] = array(
                        'sign' => 'increase',
                        'value' => '1'
                    );
                }
                ...
                if (intval($tracelog_info['trace_originalid']) == 0) {
                    $tracelog_model->tracelogEdit(array(
                        'trace_orgcommentcount' => $tracelog_info['trace_orgcommentcount'] + 1
                    ), array(
                        'trace_originalid' => "$originalid"
                    ));
                }
            }
            // 建立cookie
    ...
    public function delcommentOp()
    {
        ...
        $comment_model = Model('sns_comment');
        // 查询评论信息
        $comment_info = $comment_model->getCommentRow(array(
            'comment_id' => "$id",
        ...
        if ($result) {
            if ($comment_info['comment_originaltype'] == 1) {
                ...
                $sharegoods_model->editSharegoods($update_arr, array(
                    'share_id' => "{$comment_info['comment_originalid']}"
                ));
            } else {
                // 更新动态统计信息
                $tracelog_model = Model('sns_tracelog');
                ...
                $tracelog_model->tracelogEdit($update_arr, array(
                    'trace_id' => "{$comment_info['comment_originalid']}"
                ));
            }
    ...
    public function addforwardOp()
    {
        ...
        if ($result) {
            ...
            $tracelog_model = Model('sns_tracelog');
            $update_arr = array();
            $update_arr['trace_copycount'] = array(
                'sign' => 'increase',
                'value' => '1'
            );
            $update_arr['trace_orgcopycount'] = array(
                'sign' => 'increase',
                'value' => '1'
            );
            $condition = array();
            ...
            $tracelog_model->tracelogEdit(array(
                'trace_orgcopycount' => $tracelog_info['trace_orgcopycount'] + 1
            ), $condition);
            if ($_GET['irefresh']) {
                // 建立cookie
