...
class statistics_probabilityControl extends BaseSellerControl
{
    ...
    public function probability_statisticsOp()
    {
        ...
        $result_array = $model->table($table)
            ->field($field)
            ->join('left')
            ->on($on)
            ->where($where)
    ...
    private function profile_menu($menu_key = '')
    {
        $menu_array = array(
            1 => array(
                'menu_key' => 'probability_statistics',
                'menu_name' => Language::get('stat_probability_statistics'),
                'menu_url' => 'index.php?act=statistics_probability&op=probability_statistics'
            )
        );
