...
class Control
{
        ...
        switch ($mail_tpl['type']) {
            case '0' :
                ...
                if ($GLOBALS['setting_config']['email_enabled'] == '1') {
                    //1、发送邮件
                   $email_result = $email->send_sys_email($receiver['member_email'], $subject, $message);
                   //sj 20150828 新增发送邮件并发送站内信功能
                    //2、同时发送站内信
                    ...
                    //处理商品列表字符串
                    $msg1 = explode("<table", $message);
                    $msg2 = explode("table>", $message);
                    $msg = $msg1[0]. $msg2[1];
                    $msg = str_replace("订单商品详细：","",$msg);
                    $param = array(
                        'member_id' => $receiver_id,
                    ...
                    $message_result = $model_message->saveMessage($param);
                    if($message_result < 1 || $message_result == null){
                        $message_result == false;
                    }else{
                        $message_result == true;
                    }
                   $result = $email_result && $message_result;
                }else{
                    //邮箱配置关闭
                    $result = false;
                }
    ...
    protected function send_notice_e($mail, $tpl_code, $param, $flag = true)
    {
        $mail_tpl_model = Model('mail_templates');
        ...
        if ($GLOBALS['setting_config']['email_enabled'] == '1') {
            //1、发送邮件
            $email_result = $email->send_sys_email($mail, $subject, $message);
            $result = $email_result;
        }else{
            //邮箱配置关闭
            $result = false;
    ...
    protected function send_sms($receiver_id, $tpl_code, $param, $flag = true, $logarray = array())
    {
        ...
        $mail_tpl_model = Model('mail_templates');
        $mail_tpl = $mail_tpl_model->getOneTemplates($tpl_code);
        if (intval($logarray['store_id']) < 1) {
            return false;
        }
        $smsconf = Model('store_smsconf')->getOne($logarray['store_id']);
        $smsarr = json_decode($smsconf['opensend']);
        if ($smsarr->bl_quota_starttime > time() || $smsarr->bl_quota_endtime < time()) {
            return false;
        }
        if (empty($mail_tpl) || $mail_tpl['mail_switch'] == 0) return false;
        $member_model = Model('member');
    ...
    protected function queryCart()
    {
        if (cookie('cart_goods_num') != null && intval(cookie('cart_goods_num')) >= 0) {
            ...
            $cart_num = Model('cart')->getCartNum($save_type, array(
...
class BaseMemberControl extends Control
{
    ...
    public function get_member_info()
    {
        ...
        if (false) {
            ...
            $member_info['order_nopay'] = $model_order->getOrderStateNewCount(array(
                'buyer_id' => $_SESSION['member_id']
            ));
            $member_info['order_noreceiving'] = $model_order->getOrderStateSendCount(array(
                'buyer_id' => $_SESSION['member_id']
            ));
            $member_info['order_noeval'] = $model_order->getOrderStateEvalCount(array(
                'buyer_id' => $_SESSION['member_id']
            ));
            ...
            if ($resp['returnStatus'] == '000' && !empty($userInfo)) {
                $member_info['available_predeposit'] = ncPriceFormat(floatval($userInfo['accountBalance']) + floatval($userInfo['blockedBalance']));
            }else{
...
class BaseSNSControl extends Control
{
    ...
    // 浏览者与主人的关系：0 表示游客 1 表示一般普通会员 2表示朋友 3表示自己4表示已关注主人
    ...
    public function get_member_info()
    {
        ...
        if ($_cache = rcache($hash_key, 'sns_member')) {
            ...
            $mtag_list = $model->table('sns_membertag,sns_mtagmember')->field('mtag_name')->on('sns_membertag.mtag_id = sns_mtagmember.mtag_id')->join('inner')->where(array(
                'sns_mtagmember.member_id' => $this->master_id
            ))->select();
            $tagname_array = array();
            if (!empty($mtag_list)) {
        ...
        if ($ownid == $uid) {
            ...
            $friend_info = $model->table('sns_friend')
                ->where($where)
                ->find();
        ...
        $member_info['member_areainfo'] = ($followed >= intval($member_info['member_privacy']['area'])) ? $member_info['member_areainfo'] : L('home_member_privary');
        Tpl:: output('member_info', $member_info);
        return $member_info;
    }
    ...
    protected function get_visitor()
    {
        $model = Model();
        // 查询谁来看过我
        $visitme_list = $model->table('sns_visitor')->where(array(
            'v_ownermid' => $this->master_id
        ))->limit(9)->order('v_addtime desc')->select();
        ...
        if ($this->relation == 3) { // 主人自己才有我访问过的人
            // 查询我访问过的人
            $visitother_list = $model->table('sns_visitor')->where(array(
                'v_mid' => $this->master_id
            ))->limit(9)->order('v_addtime desc')->select();
    ...
    private function check_relation()
    {
        ...
        if ($_SESSION['is_login'] == '1') {
            if ($this->master_id == $_SESSION['member_id']) { // 主人自己
                ...
                $this->relation = 1;
                // 查询好友表
                $friend_arr = $model->table('sns_friend')->where(array(
    ...
    private function add_visit()
    {
        ...
        if ($_SESSION['is_login'] == '1' && $this->relation != 3) {
            ...
            if (!empty($visitor_info)) {
                // 查询访客记录是否存在
                $existevisitor_info = $model->table('sns_visitor')->where(array(
                    'v_ownermid' => $this->master_id,
                ...
                if (!empty($existevisitor_info)) { // 访问记录存在则更新访问时间
                    ...
                    $model->table('sns_visitor')->update(array(
        ...
        if ($is_increase == true) {
            // 增加访问次数
            $model->table('member')->update(array(
                'member_id' => $this->master_id,
                'member_snsvisitnum' => array(
                    'exp',
                    'member_snsvisitnum+1'
                )
            ));
            // 设置cookie，24小时之内不再累加
    ...
    private function my_attention()
    {
        if (intval($_SESSION['member_id']) > 0) {
            $my_attention = Model()->table('sns_friend')->where(array(
                'friend_frommid' => $_SESSION['member_id']
            ))->order('friend_addtime desc')->limit(4)->select();
...
class BaseStoreControl extends Control
{
    ...
    protected function outputStoreInfo($store_info)
    {
        ...
        // 卖家列表
        $seller_list = $model_seller->getSellerList(array(
         ...
         * 查看商家是否已申请保证金
         */
        $model_sellerdeposit = Model('seller_deposit');
        $seller_deposit = $model_sellerdeposit->getOneSellerDeposit($store_info['store_id']);
        if($seller_deposit != null)
        {
            Tpl::output('seller_deposit', $seller_deposit);
        }
    }
    ...
    protected function getStoreNavigation($store_id)
    {
        ...
        $store_navigation_list = $model_store_navigation->getStoreNavigationList(array(
            'sn_store_id' => $store_id
        ));
