...
class store_goods_classControl extends BaseSellerControl
{
    ...
    public function indexOp()
    {
        ...
        if ($_GET['type'] == 'ok') {
            if (intval($_GET['class_id']) != 0) {
                $class_info = $model_class->getClassInfo(array(
                    'stc_id' => intval($_GET['class_id'])
                ));
                Tpl::output('class_info', $class_info);
            }
            if (intval($_GET['top_class_id']) != 0) {
                Tpl::output('class_info', array(
                    'stc_parent_id' => intval($_GET['top_class_id'])
                ));
            }
            ...
            $goods_class = $model_class->getTreeClassList(array(
                'store_id' => $_SESSION['store_id']
            ), 2);
            $str = '';
    ...
    public function goods_class_saveOp()
    {
        $model_class = Model('my_goods_class');
        if ($_POST['stc_id'] != '') {
            $choeck_class = $model_class->getClassInfo(array(
            ...
            case 'store_goods_class':
                $menu_array = array(
                    1 => array(
                        ...
                        'menu_name' => Language::get('nc_member_path_goods_class'),
