...
class member_snshomeControl extends BaseSNSControl
{
    ...
    public function indexOp()
    {
        ...
        if (! empty($storelist)) {
            ...
            $goodslist = $goods_model->getGoodsOnlineList(array(
                'store_id' => array(
                    'in',
                    $storeid_arr
                )
            ...
            if (! empty($goodslist)) {
                foreach ($goodslist as $k => $v) {
                    $v['goodsurl'] = urlShop('goods', 'index', array(
                        'goods_id' => $v['goods_id']
                    ));
        ...
        $tracelist = $model->table('sns_tracelog')
            ->where($where)
            ->order('trace_id desc')
            ->limit(4)
            ->select();
    ...
    public function shareglistOp()
    {
        ...
        if ($_GET['type'] != 'like' && ! empty($goodslist)) {
            ...
            $pic_array = Model()->table('sns_albumpic')
                ->field('count(item_id) as count,item_id,ap_cover')
                ->where(array(
                'ap_type' => 1,
                'item_id' => array(
                    'in',
    ...
    public function goodsinfoOp()
    {
        ...
        $sharegoods_info['snsgoods_goodsurl'] = urlShop('goods', 'index', array(
            'goods_id' => $sharegoods_info['snsgoods_goodsid']
        ));
        ...
        // 更多分享/喜欢商品
        $sharegoods_list = $model->table('sns_sharegoods,sns_goods')
            ->join('inner')
            ->on('sns_sharegoods.share_goodsid=sns_goods.snsgoods_goodsid')
            ->where($where)
            ->limit(9)
    ...
    public function storelistOp()
    {
        ...
        if (! empty($storelist)) {
            ...
            foreach ($storeid_arr as $val) {
                $storelist_new[$val]['goods_count'] = $goods_model->getGoodsCount(array(
                    'store_id' => $val
                ));
            }
        }
    ...
    public function image_uploadOp()
    {
         ...
         */
        $model = Model();
        $default_class = $model->table('sns_albumclass')
            ->where(array(
        ...
        // 验证图片数量
        $count = $model->table('sns_albumpic')
            ->where(array(
            'member_id' => $_SESSION['member_id']
        ))
            ->count();
        ...
        if ($ap_id > 0) {
            ...
            if (! empty($pic_info))
                $upload->set('file_name', $pic_info['ap_cover']); // 原图存在设置图片名称为原图名称
        }
    ...
    private function sns_messageboard()
    {
        $model = Model();
        $where = array();
        $where['from_member_id'] = array(
            'neq',
            0
        );
        $where['to_member_id'] = $this->master_id;
        $where['message_state'] = array(
            'neq',
            2
        );
        $where['message_parent_id'] = 0;
        $where['message_type'] = 2;
        $message_list = $model->table('message')
            ->where($where)
        ...
        if (! empty($message_list)) {
            ...
            $where = array();
            $where['message_parent_id'] = array(
                'in',
                $pmsg_id
            );
            $rmessage_array = $model->table('message')
                ->where($where)
                ->select();
            $rmessage_list = array();
