...
class store_deliverControl extends BaseSellerControl
{
    ...
    public function indexOp()
    {
        ...
        if (! in_array($_GET['state'], array(
            'deliverno',
            'delivering',
            'delivered'
        )))
            $_GET['state'] = 'deliverno';
        $order_state = str_replace(array(
            'deliverno',
            'delivering',
            'delivered'
        ), array(
        ...
        $order_list = $model_order->getOrderList($condition, 5, '*', 'order_id desc', '', array(
            'order_goods',
            'order_common',
            'member'
        ));
    ...
    public function sendOp()
    {
        ...
        if (chksubmit()) {
            try {
                ...
                $data['reciver_info'] = serialize(array(
                    'address' => $_POST['reciver_address'],
                    'phone' => $_POST['reciver_phone']
                ));
                ...
                $resp = array();
                $req = array('orderCode' => $order_info['order_sn'], 'orderStatus' => '3');
                $validResp = trade(C('order.sellerDelivery'), $req, $resp);
            ...
            // 发送邮件
            $this->send_notice($order_info['buyer_id'], 'email_tobuyer_shipped_notify', array(
                
                'site_name' => $GLOBALS['setting_config']['site_name'],
                
                'site_url' => SHOP_SITE_URL,
                'store_name' => $order_info['store_name'],
                'invoice_no' => $data['shipping_code'],
            ...
            , false);
            // 发送短信
            $this->send_sms($order_info['buyer_id'], 'sms_tobuyer_shipped_notify', array(
            ...
            , false, array(
                ...
                'dateline' => mktime(),
                'tomember_id' => $order_info['buyer_id'],
                'tomember_name' => $order_info['buyer_name']
            ));
        ...
        if ($order_info['extend_order_common']['daddress_id'] > 0) {
            $daddess_info = $model_daddress->getAddressInfo(array(
                'address_id' => $order_info['extend_order_common']['daddress_id']
            ));
        } else {
            // 取默认地址
            $daddess_info = $model_daddress->getAddressList(array(
    ...
    public function search_deliverOp()
    {
        ...
        //订单状态为已发货或已收货才能查看物流信息
        if (!($order_info['order_state'] == ORDER_STATE_SEND || $order_info['order_state'] == ORDER_STATE_SUCCESS)) {
            showMessage('未找到信息', '', 'html', 'error');
        }
    ...
    public function shipping_codeOp(){
        ...
        Tpl::showpage('shipping_code.edit', 'null_layout');
    }

    /**
     * 修改修改物流单号
     */
    public function shipping_code_editOp(){
        $model_order = Model('order');
        $order_id = intval($_GET['order_id']);
        $order_sn = $_GET['order_sn'];
        $update_order = array();
        $update_order['shipping_code'] = $_POST['shipping_code'];
        $result = $model_order->editOrder($update_order, array(
            'order_id' => $order_id
        ));
        if ($result) {
            showDialog('修改成功', '', 'succ');
        } else {
    ...
    public function delay_receiveOp()
    {
        ...
        if (chksubmit()) {
            ...
            $update = $model_order->editOrder(array(
                'delay_time' => array(
                    'exp',
