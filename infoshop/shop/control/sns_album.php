...
class sns_albumControl extends BaseSNSControl
{
    ...
    public function album_cateOp()
    {
        ...
        $where['is_default'] = 1;
        $class_info = $model->table('sns_albumclass')
            ->where($where)
            ->find();
        if (empty($class_info)) {
            $insert = array();
        ...
        $order = 'ac_sort asc';
        // 相册
        $ac_list = $model->table('sns_albumclass')
            ->where($where)
            ->order($order)
            ->select();
        $count = 0; // 图片总数量
        if (! empty($ac_list)) {
            // 相册中商品数量
            $ap_count = $model->table('sns_albumpic')
                ->field('count(ap_id) as count,ac_id')
                ->where($where)
                ->group('ac_id')
                ->select();
    ...
    public function album_addOp()
    {
         ...
         */
        $model = Model();
        $class_count = $model->table('sns_albumclass')
            ->where(array(
    ...
    public function album_delOp()
    {
         ...
         * 删除分类
         */
        $return = $model->table('sns_albumclass')
            ->where(array(
            'ac_id' => $id,
            'member_id' => $_SESSION['member_id']
        ))
    ...
    public function album_pic_listOp()
    {
        ...
        $where = array();
        $where['member_id'] = $this->master_id;
        $class_array = $model->table('sns_albumclass')
            ->where($where)
            ->select();
    ...
    public function change_album_coverOp()
    {
        ...
        $where['ap_id'] = $id;
        $where['member_id'] = $_SESSION['member_id'];
        $pic_info = $model->table('sns_albumpic')
            ->where($where)
            ->find();
        $update = array();
    ...
    public function album_pic_delOp()
    {
        ...
        foreach ($id as $v) {
            ...
            if ($v <= 0)
                continue;
            $ap_info = $model->table('sns_albumpic')
                ->where(array(
                'ap_id' => $v,
                'member_id' => $_SESSION['member_id']
            ))
