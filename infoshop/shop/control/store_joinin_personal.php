...
class store_joinin_personalControl extends BaseHomeControl
{
    ...
    public function __construct()
    {
        ...
        $seller_info = $model_seller->getSellerInfo(array(
            ...
            switch (intval($joinin_detail['joinin_state'])) {
                ...
                case STORE_JOIN_STATE_VERIFY_SUCCESS:
                    if (! in_array($_GET['op'], array(
                        ...
                        $this->show_join_message('审核成功，请完成付款，付款后点击下一步提交付款凭证', SHOP_SITE_URL . DS . 'index.php?act=store_joinin_personal&op=pay');
                    }
                    break;
                case STORE_JOIN_STATE_VERIFY_FAIL:
                    if (! in_array($_GET['op'], array(
                        ...
                        $this->show_join_message('审核失败:' . $joinin_detail['joinin_message'], SHOP_SITE_URL . DS . 'index.php?act=store_joinin_personal&op=step1');
                    }
                    break;
                case STORE_JOIN_STATE_PAY_FAIL:
                    if (! in_array($_GET['op'], array(
                        ...
                        $this->show_join_message('付款审核失败:' . $joinin_detail['joinin_message'], SHOP_SITE_URL . DS . 'index.php?act=store_joinin_personal&op=pay');
                    }
                    break;
    ...
    public function step0Op()
    {
        ...
        Tpl::showpage('store_joinin_personal_apply');
    }
...
//            $param['business_licence_number_electronic'] = $this->upload_image('business_licence_number_electronic');
            ...
            $param['idcard_number'] = $_POST['idcard_number'];
            $param['idcard_electronic'] = $this->upload_image('idcard_electronic');
            ...
            $model_store_joinin = Model('store_joinin');
            $joinin_info = $model_store_joinin->getOne(array(
                'member_id' => $_SESSION['member_id']
            ));
    ...
    private function step2_save_valid($param)
    {
        ...
        $obj_validate->validateparam = array(
            ...
            array(
                "input" => $param['idcard_number'],
                "require" => "true",
                "validator" => "Length",
                "min" => "1",
                "max" => "20",
                "message" => "法人身份证号不能为空且必须小于20个字"
            ),
            array(
                "input" => $param['idcard_electronic'],
                "require" => "true",
                "message" => "法人身份证电子版不能为空"
            )
        );
    ...
    private function step3_save_valid($param)
    {
        ...
        $obj_validate->validateparam = array(
            ...
            array(
                "input" => $input['settlement_bank_address'],
                "require" => "true",
                "开户行所在地不能为空"
            )
        );
    ...
    public function step4Op()
    {
        ...
        @header('location: index.php?act=store_joinin_personal');
    }
    ...
    public function payOp()
    {
        ...
        $agreement_template_model = Model('agreement_template');
        //sj 20150826 根据模板类型查询
        $agreement_template = $agreement_template_model->getAgreementTemplateList(array(
            'type' => '2'
        ), null,null
        );
        Tpl::output('file_name', $agreement_template[0]['file_name']);
        Tpl::showpage('store_joinin_personal_apply');
    }
    ...
    public function pay_saveOp()
    {
        ...
        $param['agreement_name'] = $this->upload_agreement('agreement_name');

        if (empty($param['agreement_name'])) {
            showMessage('请上传文件大小20M以下的PDF或doc格式的合同电子档', '', '', 'error');
        }
        if (empty($param['paying_money_certificate'])) {
            showMessage('请上传图片格式（bmp,jpg,tiff,gif等）的付款凭证', '', '', 'error');
        }
    ...
    private function upload_agreement($file)
    {
        //文件大小限制20M
        $file_size = 20;
        $file_name = '';
        $upload = new UploadFile();
        $uploaddir = ATTACH_PATH . DS . 'agreement' . DS;
        $upload->set('default_dir', $uploaddir);
        $upload->set('allow_type', array(
            'pdf'
        ));
