...
class store_specControl extends BaseSellerControl
{
    ...
    public function add_specOp()
    {
        ...
        // 分类信息
        $gc_info = Model('goods_class')->getGoodsClassInfo(array(
            'gc_id' => $gc_id
        ), 'gc_id,gc_name');
    ...
    public function ajax_delspecOp()
    {
        ...
        $rs = Model('spec')->delSpecValue(array(
    ...
    public function ajax_classOp()
    {
        ...
        // 验证分类是否存在
        $gc_info = $model_goodsclass->getGoodsClassInfo(array(
            'gc_id' => $id
        ), 'type_id');
        if (empty($gc_info)) {
            echo 'false';
        ...
        if (! empty($gc_list)) {
            ...
            $spec_list = $model_type->getSpecByType(array(
                'type_id' => $gc_info['type_id']
            ), 'type_id, spec.*');
