...
class store_billControl extends BaseSellerControl
{
    ...
    public function show_billOp()
    {
        ...
        $model_bill = Model('bill');
        $bill_info = $model_bill->getOrderBillInfo(array(
            'ob_no' => $_GET['ob_no']
        ));
        if (! $bill_info) {
            showMessage('参数错误', '', 'html', 'error');
        ...
        if ($if_end_date || $if_end_date) {
            ...
            $order_condition['finnshed_time'] = array(
                'time',
                array(
                    $bill_info['ob_start_date'],
                    $bill_info['ob_end_date']
                )
            );
        }
        if ($_GET['type'] == 'refund') {
    ...
    public function confirm_billOp()
    {
        ...
        $update = $model_bill->editOrderBill(array(
            'ob_state' => BILL_STATE_STORE_COFIRM
        ), $condition);
        if ($update) {
    ...
    public function export_orderOp()
    {
        ...
        if (! is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {
                ...
                // 如果数量小，直接下载
                $data = $model_order->getOrderList($condition, '', '*', 'order_id desc', self::EXPORT_SIZE, array(
                    'order_goods'
                ));
            }
        } else {
            ...
            $data = $model_order->getOrderList($condition, '', '*', 'order_id desc', "{$limit1},{$limit2}", array(
                'order_goods'
            ));
        }
    ...
    public function export_refund_orderOp()
    {
        ...
        foreach ($data as $v) {
            ...
            $export_data[$k + 1][] = str_replace(array(
                1,
                2
            ), array(
                '退款',
        ...
        switch ($menu_type) {
            case 'list':
                $menu_array = array(
                    1 => array(
                        'menu_key' => 'list',
                        'menu_name' => Language::get('nc_member_path_bill_manage'),
            ...
            case 'show':
                $menu_array = array(
                    array(
                        'menu_key' => 'order_list',
                        'menu_name' => '订单列表',
                        'menu_url' => 'index.php?act=store_bill&op=show_bill&ob_no=' . $_GET['ob_no']
                    ),
                    array(
                        'menu_key' => 'refund_list',
                        'menu_name' => '退款订单',
                        'menu_url' => 'index.php?act=store_bill&op=show_bill&type=refund&ob_no=' . $_GET['ob_no']
                    ),
                    array(
                        'menu_key' => 'cost_list',
                        'menu_name' => '促销费用',
                        'menu_url' => 'index.php?act=store_bill&op=show_bill&type=cost&ob_no=' . $_GET['ob_no']
                    )
                );
                break;
        }
