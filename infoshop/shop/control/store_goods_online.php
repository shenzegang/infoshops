...
class store_goods_onlineControl extends BaseSellerControl
{
    ...
    public function edit_save_goodsOp()
    {
        ...
        $update_common['is_gift'] = intval($_POST['is_gift']);
        $update_common['gift_points'] = intval($_POST['gift_points']);
        
        if ($update_common['is_gift'] > 0 && $update_common['gift_points'] <= 0) {
            showMessage('所需积分不能为0', '', '', 'error');
        }
            ...
            if (is_array($_POST['spec'])) {
                foreach ($_POST['spec'] as $value) {
                    ...
                    if (! empty($goods_info)) {
                        ...
                        $update['is_gift'] = $update_common['is_gift'];
                        $update['gift_points'] = $update_common['gift_points'];
                        
                        $model_goods->editGoods($update, array(
                            'goods_id' => $goods_id
                        ));
                        // 生成商品二维码
                        $PhpQRCode->set('date', urlShop('goods', 'index', array(
                            'goods_id' => $goods_id
                        )));
                        ...
                        $insert['is_gift'] = $update_common['is_gift'];
                        $insert['gift_points'] = $update_common['gift_points'];
    ...
    public function edit_imageOp()
    {
        ...
        $image_list = $model_goods->getGoodsImageList(array(
            'goods_commonid' => $common_id
        ));
        ...
        $common_list = $model_goods->getGoodeCommonInfo(array(
            'goods_commonid' => $common_id
        ), 'spec_value');
    ...
    public function edit_jingleOp()
    {
        if (chksubmit()) {
            ...
            $update = array(
                'goods_jingle' => trim($_POST['g_jingle'])
            );
    ...
    public function get_goods_list_ajaxOp()
    {
        ...
        $model_goods = Model('goods');
        $goodscommon_list = $model_goods->getGoodeCommonInfo(array(
            'store_id' => $_SESSION['store_id'],
            'goods_commonid' => $common_id
        ), 'spec_name');
        ...
        $goods_list = $model_goods->getGoodsList(array(
            'store_id' => $_SESSION['store_id'],
            'goods_commonid' => $common_id
        ), 'goods_id,goods_spec,store_id,goods_price,goods_serial,goods_storage,goods_image');
        if (empty($goods_list)) {
            echo 'false';
            exit();
        }
        ...
        foreach ($goods_list as $key => $val) {
            ...
            $goods_list[$key]['url'] = urlShop('goods', 'index', array(
