...
class indexControl extends BaseHomeControl
{
    ...
    /**
     * 20150827 tjz增加 输入店铺能发布的分类集合
     */
    public function josn_classByIdOp()
    {
        $gc_id = intval($_GET['gc_id']);
        $deep = $_GET['deep_id'];
        if ($gc_id <= 0 || $deep <= 0 || $deep >= 4) {
            echo $_GET['callback'] . '(' . json_encode("end") . ')';
            exit();
        }
    ...
    public function viewed_infoOp()
    {
        ...
        if ($_SESSION['is_login'] == '1') {
            ...
            if (C('voucher_allow') == 1) {
                $time_to = time(); // 当前日期
                $info['voucher'] = Model()->table('voucher')
                    ->where(array(
                    'voucher_owner_id' => $member_id,
                    'voucher_state' => 1,
                    'voucher_start_date' => array(
                        'elt',
                        $time_to
                    ),
            ...
            $info['consult'] = Model()->table('consult')
                ->where(array(
                'member_id' => $member_id,
                'consult_reply_time' => array(
                    array(
                        'gt',
                        strtotime($time_from)
                    ),
        ...
        if (is_array($goods_list) && ! empty($goods_list)) {
            ...
            foreach ($goods_list as $key => $val) {
                ...
                $val['url'] = urlShop('goods', 'index', array(
                    'goods_id' => $goods_id
                ));
