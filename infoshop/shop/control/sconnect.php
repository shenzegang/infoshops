...
class sconnectControl extends BaseHomeControl
{
    ...
    public function registerOp()
    {
        ...
        $obj_validate->validateparam = array(
            array(
                ...
                "require" => "true",
                "message" => Language::get('login_usersave_username_isnull')
            ),
            array(
                "input" => $_POST["password"],
                "require" => "true",
                "message" => Language::get('login_usersave_password_isnull')
            ),
            array(
                "input" => $_POST["password_confirm"],
                "require" => "true",
                "validator" => "Compare",
                "operator" => "==",
                "to" => $_POST["password"],
                "message" => Language::get('login_usersave_password_not_the_same')
            ),
            array(
                "input" => $_POST["email"],
                "require" => "true",
                "validator" => "email",
                "message" => Language::get('login_usersave_wrong_format_email')
            ),
            array(
                "input" => $_POST["agree"],
                "require" => "true",
                "message" => Language::get('login_usersave_you_must_agree')
            )
        );
        ...
        if ($result) {
            $member_info = $model_member->infoMember(array(
                'member_name' => trim($user_array['member_name'])
            ));
    ...
    public function loginOp()
    {
        ...
        $obj_validate->validateparam = array(
            array(
                "input" => $_POST["user_name"],
                "require" => "true",
                "message" => Language::get('login_index_username_isnull')
            ),
            array(
                "input" => $_POST["password"],
                "require" => "true",
                "message" => Language::get('login_index_password_isnull')
            )
        );
        $error = $obj_validate->validate();
        if ($error != '') {
    ...
    public function bindsinaOp()
    {
        ...
        $edit_state = $model_member->updateMember(array(
            'member_sinaopenid' => $_SESSION['slast_key']['uid']
        ), $_SESSION['member_id']);
        if ($edit_state) {
