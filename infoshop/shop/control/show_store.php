...
class show_storeControl extends BaseStoreControl
{
    ...
    public function show_articleOp()
    {
        ...
        $store_navigation_info = $model_store_navigation->getStoreNavigationInfo(array(
    ...
    {
        $count = Model('store_sns_tracelog')->getStoreSnsTracelogCount(array(
            'strace_storeid' => $this->store_info['store_id']
        ));
    ...
    public function ajax_flowstat_recordOp()
    {
        if ($_GET['store_id'] != '' && $_SESSION['store_id'] != $_GET['store_id']) {
            ...
            if ($_GET['act_param'] == 'show_store' && ($_GET['op_param'] == 'index' || $_GET['op_param'] == 'credit' || $_GET['op_param'] == 'store_info')) {
                $flow_date_array = $model->table($flow_tablename)
                    ->where(array(
                    'date' => $date,
                ...
                if ($_GET['act_param'] == 'goods' && $_GET['op_param'] == 'index') {
                    $flow_date_array = $model->table($flow_tablename)
                        ->where(array(
                        'date' => $date,
                        ...
                        ->find();
                    $flow_date_array_sub = $model->table($flow_tablename)
                        ->where(array(
                        'date' => $date,
                        'store_id' => intval($_GET['store_id'])
                    ))
            ...
            if (is_array($flow_date_array) && ! empty($flow_date_array)) { // 已经存在数据则更新
                ...
                if ($_GET['act_param'] == 'show_store' && ($_GET['op_param'] == 'index' || $_GET['op_param'] == 'credit' || $_GET['op_param'] == 'store_info')) {
                    $model->table($flow_tablename)->insert(array(
                        'date' => $date,
                        'clicknum' => 1,
                        'store_id' => intval($_GET['store_id']),
                        'type' => 'sum',
