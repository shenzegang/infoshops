    ...
    public function import_pic_uploadOp()
    {
        ...
        if ($_POST['Filename'] == '') {
            echo json_encode(array(
                'state' => 'false',
                'message' => $lang['iswfupload_miss_file']
            ));
            exit();
        }
        
        $upload_list = $model_upload->getUploadList(array(
            'file_name' => substr($_POST['Filename'], 0, strlen($_POST['Filename']) - 4)
        ));
        
        if (empty($upload_list) or ! is_array($upload_list)) {
            echo json_encode(array(
                'state' => 'false',
                'message' => $lang['iswfupload_image'] . $_POST['Filename'] . $lang['iswfupload_not_goods_image']
            ));
            exit();
        }
        ...
        if (empty($upload_info['store_id'])) {
            echo json_encode(array(
                'state' => 'false',
                'message' => $lang['iswfupload_upload_pictext'] . $_POST['Filename'] . $lang['iswfupload_upload_noprodpic']
            ));
            exit();
        }
        ...
        foreach ($upload_list as $val) {
            ...
            if ($result) {
                if ($result) {
                    ...
                    echo json_encode(array(
                        'state' => 'false',
                        'message' => $lang['iswfupload_update_fail']
                    ));
                    exit();
                }
            } else {
                echo json_encode(array(
                    'state' => 'false',
                    'message' => $lang['iswfupload_upload_fail']
                ));
                exit();
            }
        }
