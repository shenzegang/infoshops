...
class store_settingControl extends BaseSellerControl
{
     ...
     * 卖家申请经营类目
     */
    public function apply_business_category(){
        /**
         * 输出店铺信息
         */
        self::profile_menu('store_setting');
        Tpl::output('menu_sign', 'store_setting');
        Tpl::output('menu_sign_url', 'index.php?act=store_setting&op=apply_business_category');
        ...
        if (chksubmit()) {
            ...
            if (! empty($store_domain) && $store_domain != $store_info['store_domain']) {
                ...
                if ($subdomain_times > $store_domain_times) { // 可继续修改
                    ...
                    $model_class->editStore($param, array(
                        'store_id' => $store_id
                    ));
                }
    ...
    public function store_slideOp()
    {
        ...
        if ($_POST['form_submit'] == 'ok') {
            ...
            $model_store->editStore($update, array(
                'store_id' => $_SESSION['store_id']
            ));
            
            // 删除upload表中数据
            $model_upload->delByWhere(array(
                'upload_type' => 7,
                'store_id' => $_SESSION['store_id']
            ));
    ...
    public function dorp_imgOp()
    {
        ...
        echo json_encode(array(
            'succeed' => Language::get('nc_common_save_succ', 'UTF-8')
        ));
        die();
    }
    ...
    public function set_themeOp()
    {
        ...
        if (! empty($style) && file_exists(BASE_TPL_PATH . DS . '/store/style/' . $style . '/images/preview.jpg')) {
            $store_class = Model('store');
            $rs = $store_class->editStore(array(
                'store_theme' => $style
            ), array(
                'store_id' => $_SESSION['store_id']
            ));
