...
class seller_centerControl extends BaseSellerControl
{
    ...
    public function indexOp()
    {
        ...
        ), 'goods_id,goods_name,goods_image,store_id,goods_salenum', '', 'goods_salenum desc', 8);
        ...
        if (intval(C('promotion_allow')) == 1) {
            ...
            // 优惠套装套餐
            $binglingquota_info = Model('p_bundling')->getBundlingQuotaInfoCurrent(array(
                'store_id' => $_SESSION['store_id']
            ));
            Tpl::output('binglingquota_info', $binglingquota_info);
            // 推荐展位套餐
            $boothquota_info = Model('p_booth')->getBoothQuotaInfoCurrent(array(
                'store_id' => $_SESSION['store_id']
            ));
    ...
    public function statisticsOp()
    {
        ...
        $model_order = Model('order');
        // 交易中的订单
        $progressing = $model_order->getOrderCount(array(
            'store_id' => $_SESSION['store_id'],
            'order_state' => array(
                array(
                    'neq',
        ...
        // 待付款
        $no_payment = $model_order->getOrderStateNewCount(array(
            'store_id' => $_SESSION['store_id']
        ));
        // 待发货
        $no_delivery = $model_order->getOrderStatePayCount(array(
            'store_id' => $_SESSION['store_id']
        ));
        ...
        $condition['complain_state'] = array(
            array(
                'gt',
                10
            ),
