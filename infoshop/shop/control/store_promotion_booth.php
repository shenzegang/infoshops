...
class store_promotion_boothControl extends BaseSellerControl
{
    ...
    public function booth_goods_listOp()
    {
        ...
        $where['booth_quota_endtime'] = array(
            'lt',
            TIMESTAMP
        );
        ...
        if (! empty($booth_quota)) {
            // 查询已选择商品
            $boothgoods_list = $model_booth->getBoothGoodsList(array(
            ...
            if (! empty($boothgoods_list)) {
                ...
                if (! empty($goods_list)) {
                    ...
                    $goodsclass_list = Model('goods_class')->getGoodsClassList(array(
                        'gc_id' => array(
                            'in',
                            $gcid_array
                        )
    ...
    public function choosed_goodsOp()
    {
        ...
        $bootgoods_info = $model_booth->getBoothGoodsList(array(
            'store_id' => $_SESSION['store_id']
        ), 'goods_id');
        // 已添加商品总数
        if (count($bootgoods_info) >= C('promotion_booth_goods_sum')) {
            $data = array(
                'result' => 'false',
                'msg' => '只能添加' . C('promotion_booth_goods_sum') . '个商品'
            );
            $this->_echoJson($data);
        }
        // 商品是否已经被添加
        ...
        $goods_info['url'] = urlShop('goods', 'index', array(
            'goods_id' => $goods_info['goods_id']
        ));
        $goods_class = Model('goods_class')->getGoodsClassInfo(array(
    ...
    public function del_choosed_goodsOp()
    {
        ...
        $result = Model('p_booth')->delBoothGoods(array(
            'goods_id' => $gid,
            'store_id' => $_SESSION['store_id']
        ));
        if ($result) {
