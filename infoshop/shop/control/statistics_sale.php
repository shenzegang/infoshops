...
class statistics_saleControl extends BaseSellerControl
{
    ...
    public function sale_statisticsOp()
    {
        if ($_GET['type'] == 'month') {
            ...
            $model = Model();
            $flow_array = $model->table('salenum')
                ->field('date,sum(salenum) as sum')
                ->where(array(
                'date' => array(
                    'in',
                    $request_date_str
    ...
    private function profile_menu($menu_key = '')
    {
        $menu_array = array(
            1 => array(
                'menu_key' => 'store_sale',
                'menu_name' => Language::get('stat_store_sale'),
                'menu_url' => 'index.php?act=statistics_sale&op=sale_statistics'
            ),
            2 => array(
                'menu_key' => 'goods_sale',
                'menu_name' => Language::get('stat_goods_sale'),
                'menu_url' => 'index.php?act=statistics_sale&op=goods_sale_statistics'
            )
        );
