...
class goodsControl extends BaseGoodsControl
{
    ...
    public function indexOp()
    {
        ...
        if ($goods_info['is_gift']) {
            @header('Location: ' . urlShop('pointprod', 'pinfo', array(
        ...
        if ($_cache = rcache($hash_key, 'product')) {
            ...
            // 查询SNS中该商品的信息
            $snsgoodsinfo = Model('sns_goods')->getSNSGoodsInfo(array(
                'snsgoods_goodsid' => $goods_info['goods_id']
            ), 'snsgoods_likenum,snsgoods_sharenum');
        ...
        if ($goods_info['transport_id'] > 0) {
            ...
            $transport = $model_transport->getExtendList(array(
        ...
        if (! empty($plateid_array)) {
            $plate_array = Model('store_plate')->getPlateList(array(
                'plate_id' => array(
                    'in',
                    $plateid_array
                ),
        ...
        // 优先得到推荐商品
        $goods_commend_list = $model_goods->getGoodsOnlineList(array(
            'store_id' => $goods_info['store_id'],
            'goods_commend' => 1
        ), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_price', 0, 'rand()', 5, 'goods_commonid');
    ...
    public function comments_listOp()
    {
        ...
        $nav_link_list[] = array(
            'title' => $goods_info['goods_name'],
            'link' => urlShop('goods', 'index', array(
                'goods_id' => $goods_id
            ))
        );
        $nav_link_list[] = array(
...
//        foreach ($goodsevallist as $eval) {
//            //匿名购买
//            if($eval['geval_isanonymous'] == '1'){
//
//            }
...
//        }
    ...
    public function salelogOp()
    {
        ...
        $sales = $order_class->getOrderAndOrderGoodsSalesRecordList(array(
            'order_goods.goods_id' => $goods_id
        ), 'order_goods.*, order.buyer_name, order.add_time', 10);
        Tpl::output('show_page', $order_class->showpage());
        Tpl::output('sales', $sales);
        
        Tpl::output('order_type', array(
            2 => '团',
            3 => '折',
            '4' => '套装'
        ));
    ...
    public function cosultingOp()
    {
        ...
        $consult_list = $consult->getConsultList(array(
            'goods_id' => $goods_id
        ), $page, 'simple');
    ...
    public function save_consultajaxOp()
    {
        ...
        if (! C('guest_comment') && ! $_SESSION['member_id']) {
            echo json_encode(array(
                'done' => 'false',
                'msg' => Language::get('goods_index_goods_noallow')
            ));
            die();
        }
        $goods_id = intval($_GET['goods_id']);
        ...
        if (trim($_GET['goods_content']) == "") {
            echo json_encode(array(
                'done' => 'false',
                'msg' => Language::get('goods_index_input_consult')
            ));
            die();
        }
        $_POST = $_GET;
        ...
        if (! $result) {
            echo json_encode(array(
                'done' => 'false',
                'msg' => Language::get('invalid_request')
            ));
            die();
        } elseif ($result === - 11) {
            echo json_encode(array(
            ...
            echo json_encode(array(
                'done' => 'false',
                'msg' => Language::get('wrong_checkcode')
            ));
            die();
        }
        if (processClass::islock('commit')) {
            echo json_encode(array(
                'done' => 'false',
                'msg' => Language::get('nc_common_op_repeat')
            ));
            die();
        } else {
        ...
        $goods = Model('goods');
        $goods_info = array();
        $goods_info = $goods->getGoodsInfo(array(
            'goods_id' => $goods_id
        ));
        if (empty($goods_info)) {
            echo json_encode(array(
                'done' => 'false',
                'msg' => Language::get('goods_index_goods_not_exists')
            ));
            die();
        }
        // 判断是否是店主本人
        if ($_SESSION['store_id'] && $goods_info['store_id'] == $_SESSION['store_id']) {
            echo json_encode(array(
                'done' => 'false',
                'msg' => Language::get('goods_index_consult_store_error')
            ));
            die();
        }
        // 检查店铺状态
        ...
        if ($store_info['store_state'] == '0' || intval($store_info['store_state']) == '2' || (intval($store_info['store_end_time']) != 0 && $store_info['store_end_time'] <= time())) {
            echo json_encode(array(
                'done' => 'false',
                'msg' => Language::get('goods_index_goods_store_closed')
            ));
            die();
        }
        // 接收数据并保存
        ...
        if ($consult_model->addConsult($input)) {
            ...
            echo json_encode(array(
                'done' => 'false',
                'msg' => Language::get('goods_index_consult_fail')
            ));
            die();
        }
    }
    ...
    public function get_bundlingOp()
    {
        ...
        // 更新优惠套装状态
        $model_bundling->editBundlingTimeout(array(
            'store_id' => $store_id
        ));
        
        // 查询店铺套餐活动是否开启
        $quota_list = $model_bundling->getBundlingQuotaOpenList(array(
            'store_id' => $store_id
        ), 0, 1);
        if (! empty($quota_list)) {
            // 根据商品id查询bl_id
            $b_g_list = $model_bundling->getBundlingGoodsList(array(
                'goods_id' => $goods_id,
                'bl_appoint' => 1
            ), 'bl_id');
            if (! empty($b_g_list) && is_array($b_g_list)) {
                ...
                // 查询套餐列表
                $bundling_list = $model_bundling->getBundlingOpenList(array(
                    'bl_id' => array(
                        'in',
                        $b_id_array
                    )
                ...
                if (! empty($bundling_list) && is_array($bundling_list)) {
                    ...
                    $b_goods_list = $model_bundling->getBundlingGoodsList(array(
                        'bl_id' => array(
                            'in',
                            $blid_array
                        )
                    ...
                    if (! empty($b_goods_list)) {
                        ...
                        $goods_list = Model('goods')->getGoodsAsGoodsShowList(array(
                            'goods_id' => array(
                                'in',
                                $goodsid_array
                            )
    ...
    function calcOp()
    {
        ...
        $model_transport = Model('transport');
        $extend = $model_transport->getExtendList(array(
            'transport_id' => array(
                intval($_GET['tid'])
            )
        ));
