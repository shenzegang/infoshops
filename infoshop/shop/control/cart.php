...
class cartControl extends BaseBuyControl
{
    ...
    public function indexOp()
    {
        ...
        // 取出购物车信息
        $cart_list = $model_cart->listCart('db', array(
        ...
        if ($_SESSION['member_id']) {
            ...
            if (!empty($cart_list)) {
                ...
                foreach ($cart_list as $cart) {
                    ...
                    $cart_array['list'][$k]['goods_url'] = urlShop('goods', 'index', array(
            ...
            foreach ($cart_list as $key => $cart) {
                ...
                $value['goods_url'] = urlShop('goods', 'index', array(
                    'goods_id' => $cart['goods_id']
                ));
    ...
    public function addOp()
    {
        ...
        if (is_numeric($_GET['goods_id'])) {
            ...
            if ($goods_id <= 0)
                return;
            $goods_info = $model_goods->getGoodsOnlineInfo(array(
                'goods_id' => $goods_id
            ));

            if ($goods_info['is_gift']) {
                exit(json_encode(array(
                    'msg' => '该商品为积分兑换礼品，禁止加入购物车',
                    'UTF-8'
                )));
            }
            ...
            if (empty($bl_info) || $bl_info['bl_state'] == '0') {
                exit(json_encode(array(
                    'msg' => '该优惠套装已不存在，建议您单独购买',
                    'UTF-8'
                )));
            }

            // 检查每个商品是否符合条件,并重新计算套装总价
            $bl_goods_list = $model_bl->getBundlingGoodsList(array(
                'bl_id' => $bl_id
            ));
            ...
            $model_goods = Model('goods');
            $goods_list = $model_goods->getGoodsOnlineList(array(
                'goods_id' => array(
                    in,
                    $goods_id_array
                )
        ...
        $model_cart = Model('cart');
        //判断购物车是否存在，如果不存在，就是当前数量
        //如果存在就是数量相加
        $result = $model_cart->getCartInfo(array(
            'goods_id' => $goods_id,
            'buyer_id' => $_SESSION['member_id']
        ));
        if($result != null){
            $total = $quantity + $result["goods_num"];
            //更新数量
            $flag = $model_cart->editCart(array(
                'goods_num' => $total
            ), array(
                'cart_id' => $result["cart_id"],
            ));
        }
        $flag = $model_cart->addCart($goods_info, $save_type, $quantity);
    ...
    private function _check_goods($goods_info, $quantity)
    {
        if (empty($quantity)) {
            exit(json_encode(array(
                'msg' => Language::get('wrong_argument', 'UTF-8')
            )));
        }
        if (empty($goods_info)) {
            exit(json_encode(array(
                'msg' => Language::get('cart_add_goods_not_exists', 'UTF-8')
            )));
        }
        if ($goods_info['store_id'] == $_SESSION['store_id']) {
            exit(json_encode(array(
                'msg' => Language::get('cart_add_cannot_buy', 'UTF-8')
            )));
        }
        if (intval($goods_info['goods_storage']) < 1) {
            exit(json_encode(array(
                'msg' => Language::get('cart_add_stock_shortage', 'UTF-8')
            )));
        }
        if (intval($goods_info['goods_storage']) < $quantity) {
            exit(json_encode(array(
                'msg' => Language::get('cart_add_too_much', 'UTF-8')
            )));
        }
    }
    ...
    public function updateOp()
    {
        ...
        if (empty($cart_id) || empty($quantity)) {
            exit(json_encode(array(
                'msg' => Language::get('cart_update_buy_fail', 'UTF-8')
            )));
        }
        ...
        $cart_info = $model_cart->getCartInfo(array(
            'cart_id' => $cart_id,
            'buyer_id' => $_SESSION['member_id']
        ));
        if ($cart_info['bl_id'] == '0') {
            ...
            if (intval($goods_info['goods_storage']) < $quantity) {
                ...
                $model_cart->editCart(array(
                    'goods_num' => $goods_info['goods_storage']
                ), array(
                    'cart_id' => $cart_id,
                    'buyer_id' => $_SESSION['member_id']
                ));
            ...
            $model_bl = Model('p_bundling');
            $bl_goods_list = $model_bl->getBundlingGoodsList(array(
                'bl_id' => $cart_info['bl_id']
            ));
            $goods_id_array = array();
            foreach ($bl_goods_list as $goods) {
                $goods_id_array[] = $goods['goods_id'];
            }
            $cart_list[$key]['bl_goods_list'] = $model_goods->getGoodsOnlineList(array(
                'goods_id' => array(
                    in,
                    $goods_id_array
                )
        ...
        $update = $model_cart->editCart($data, array(
            'cart_id' => $cart_id,
            'buyer_id' => $_SESSION['member_id']
        ));
        if ($update) {
    ...
    public function delOp()
    {
        ...
        $goods_id = intval($_GET['goods_id']);
        if ($cart_id < 0 || $goods_id < 0)
            return;
        $model_cart = Model('cart');
        $data = array();
        if ($_SESSION['member_id']) {
            // 登录状态下删除数据库内容
            $delete = $model_cart->delCart('db', array(
                'cart_id' => $cart_id,
                'buyer_id' => $_SESSION['member_id']
            ));
            if ($delete) {
            ...
            $save_type = C('cache.type') == 'file' ? 'cookie' : 'cache';
            $delete = $model_cart->delCart($save_type, array(
                'goods_id' => $goods_id
            ));
            if ($delete) {
                $data['state'] = 'true';
 ...
 * **/
    public function delSelectedOp()
    {
        ...
        if ($_SESSION['member_id']) {
            // 登录状态下删除数据库内容
            $condition = array();
            $condition['cart_id'] = array(
                'in',
                $cart_ids
            );
            $condition['buyer_id'] = $_SESSION['member_id'];

            $delete = $model_cart->delSelectedCart('db', $condition
            );
            if ($delete) {
                $data['state'] = 'true';
                $data['quantity'] = $model_cart->cart_goods_num;
        ...
        if (isset($_POST['callback'])) {
            $json_data = $_POST['callback'] == '?' ? '(' . $json_data . ')' : $_POST['callback'] . "($json_data);";
        }
        exit($json_data);
    }
     ...
     * 清除购物车失效宝贝
     * */
    public function clearExpiredOp()
    {
        $model_cart = Model('cart');
        $data = array();
        //清除失效goods
        ...
        $state[1] = "10";
        $cond = array();
        $cond['goods_state'] = array(
            'in',
            $state
        );
        //获取购物车里已失效的goods的id数组
        $ids = array();
        $goods = $model_goods->getGoodsList($cond);
        foreach ($goods as $good) {
            array_push($ids, $good['goods_id']);
        }
        //
        $cart_ids = array();
        if ($_SESSION['member_id']) {
            ...
            $condition['buyer_id'] = $_SESSION['member_id'];
            $carts = $model_cart ->listCart('db', $condition);
            foreach ($carts as $cart) {
                array_push($cart_ids, $cart["cart_id"]);
            }
            $delete = $model_cart->delSelectedCart('db', $condition
            );
            $data["expired_cart_ids"] = $cart_ids;
            if ($delete) {
                $data['state'] = 'true';
                $data['quantity'] = $model_cart->cart_goods_num;
                $data['amount'] = $model_cart->cart_all_price;
