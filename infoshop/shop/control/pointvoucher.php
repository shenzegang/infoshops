...
class pointvoucherControl extends BaseHomeControl
{
    ...
    public function pointvoucherOp()
    {
        ...
        $voucher = $model->table('voucher_template,store')
            ->field($field)
            ->join('left')
            ->on($on)
            ->where(array(
    ...
    public function voucherexchangeOp()
    {
        ...
        if ($result) {
            ...
            $field = 'voucher_template.*,store.store_id,store.store_label,store.store_name,store.store_domain';
            $on = 'voucher_template.voucher_t_store_id=store.store_id';
            $voucher_info = $model->table('voucher_template,store')
                ->field($field)
                ->join('left')
                ->on($on)
                ->where(array(
    ...
    public function voucherexchange_saveOp()
    {
        ...
        $voucher_list = $model->table('voucher')
            ->where(array(
            'voucher_owner_id' => $_SESSION['member_id'],
            'voucher_store_id' => $voucher_info['voucher_t_store_id'],
            'voucher_end_date' => array(
                'gt',
                time()
            )
        ))
        ...
        if ($result) {
            ...
            showDialog(Language::get('home_voucher_exchangsucc'), 'index.php?act=pointprod&op=index', 'succ', $js);
        } else {
            showDialog(Language::get('home_voucher_exchangfail'), 'index.php?act=pointprod&op=index', 'error', $js);
        }
    }
}
