...
class store_orderControl extends BaseSellerControl
{
    ...
    public function indexOp()
    {
        ...
        if (in_array($_GET['state_type'], $allow_state_array)) {
            $condition['order_state'] = str_replace($allow_state_array, array(
                ORDER_STATE_NEW,
                ORDER_STATE_PAY,
                ORDER_STATE_SEND,
                ORDER_STATE_SUCCESS,
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            // 显示调整商品费用
            $order_list[$key]['if_goods_price'] = $model_order->getOrderOperateState('goods_price', $order_info);
    ...
    private function _change_state_order_cancel($order_info)
    {
        ...
        if (chksubmit()) {
            ...
            $goods_list = $model_order->getOrderGoodsList(array(
                'order_id' => $order_id
            ));
            ...
            // 邮箱
            $this->send_notice($order_info['buyer_id'], 'email_tobuyer_cancel_order_notify', array(
            ...
            , false);
            // 短信
            $this->send_sms($order_info['buyer_id'], 'sms_tobuyer_cancel_order_notify', array(
    ...
    private function _change_state_modify_price($order_info)
    {
        ...
        if (chksubmit()) {
            ...
            $data['order_amount'] = array(
                'exp',
                'goods_amount+' . $data['shipping_fee']
            );
            $update = $model_order->editOrder($data, array(
                'order_id' => $order_id
            ));
            ...
            $resp = array();
            $req = array('orderCode' => $order_info['order_sn'], 'orderPostage' => abs(floatval($_POST['shipping_fee'])));
            $validResp = trade(C('order.editOrderPostage'), $req, $resp);
            if (!$validResp || $resp['returnStatus'] != '000') {
                throw new Exception('修改邮费操作失败');
            }
        } else {
            Tpl::output('order_id', $order_id);
    ...
    private function _change_state_goods_price($order_info)
    {
        ...
        if (chksubmit()) {
            ...
            $data['order_amount'] = array(
                'exp',
                'shipping_fee+' . $data['goods_amount']
            );
            $update = $model_order->editOrder($data, array(
                'order_id' => $order_id
            ));
            $data2['goods_pay_price'] = abs(floatval($_POST['goods_amount']));
            $model_order->editOrderGoods($data2, array(
                'order_id' => $order_id
            ));
            if (! $update) {
                throw new Exception(L('nc_common_save_fail'));
            ...
            case 'show':
                $menu_array = array(
                    array(
                        'menu_key' => 'all_order',
                        'menu_name' => Language::get('nc_member_path_all_order'),
                        'menu_url' => 'index.php?act=store_order&op=index'
                    ),
                    array(
                        'menu_key' => 'show_order',
                        'menu_name' => Language::get('nc_member_path_show_order'),
                        'menu_url' => ''
                    )
                );
                break;
