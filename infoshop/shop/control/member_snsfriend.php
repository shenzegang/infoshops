...
class member_snsfriendControl extends BaseMemberControl
{
    ...
    public function findOp()
    {
        if (chksubmit()) {
            ...
            if (! empty($mtag_list)) {
                // 查询已关注好友 ，不显示已关注好友
                $friend_array = $model->table('sns_friend')
                    ->field('friend_tomid')
                    ->where(array(
                ...
                // 查询会员
                $where['sns_mtagmember.mtag_id'] = array(
                    'in',
                    $mtagid_array
                ); // 查询条件
                $where['sns_mtagmember.member_id'] = array(
                    'not in',
                    $friendid_array
                );
                ...
                $tagmember_list = $model->table('sns_mtagmember,member')
                    ...
                    ->join('left')
                    ->on('sns_mtagmember.member_id=member.member_id')
                    ->where($where)
                    ->order('sns_mtagmember.recommend desc, sns_mtagmember.member_id asc')
                    ->limit(count($mtagid_array) * 20)
                    ->select();
    ...
    public function findlistOp()
    {
        if (trim($_POST['searchname']) != '') {
            ...
            // 查询关注会员id
            $followlist = $model->table('sns_friend')
                ->field('friend_tomid, friend_followstate')
                ->where(array(
                'friend_frommid' => $_SESSION['member_id']
            ))
                ->select();
    ...
    public function addfollowOp()
    {
        ...
        $friend_count = $friend_model->countFriend(array(
            'friend_frommid' => "{$_SESSION['member_id']}",
            'friend_tomid' => "$mid"
        ));
        if ($friend_count > 0) {
        ...
        $friend_info = $friend_model->getFriendRow(array(
            'friend_frommid' => "{$mid}",
        ...
        if ($result) {
            ...
            $return = json_encode(array(
                'state' => $insert_arr['friend_followstate']
            ));
        } else {
            $return = 'false';
        }
    ...
    public function delfollowOp()
    {
        ...
        // 取消关注
        $friend_model = Model('sns_friend');
        $result = $friend_model->delFriend(array(
            'friend_frommid' => "{$_SESSION['member_id']}",
            'friend_tomid' => "$mid"
        ));
        if ($result) {
    ...
    public function batch_addfollowOp()
    {
        ...
        // 查询是否已经关注
        $gz_array = $model->table('sns_friend')
            ->where(array(
            'friend_frommid' => $_SESSION['member_id'],
            'friend_tomid' => array(
                'in',
        ...
        // 查询对方是否关注我
        $bgz_array = $model->table('sns_friend')
            ->where(array(
            'friend_frommid' => array(
                'in',
                $ids
            ),
            'friend_tomid' => $_SESSION['member_id']
        ...
        if (! empty($pm_array)) {
            ...
            foreach ($pm_array as $val) {
                ...
                if (isset($bgz_array[$val['member_id']])) {
                    ...
                    $model->table('sns_friend')->update(array(
                        'friend_followstate' => 2,
    ...
    public function followOp()
    {
        ...
        $page->setEachNum(15);
        $page->setStyle('admin');
        $follow_list = $friend_model->listFriend(array(
            'friend_frommid' => "{$_SESSION['member_id']}"
        ), '*', $page, 'detail');
        if (! empty($follow_list)) {
            foreach ($follow_list as $k => $v) {
                ...
                $uid = $follow_list[$k]['member_id'];

                $model = Model();
                if ($v['member_privacy'] != '') {
                    $v['member_privacy'] = unserialize($v['member_privacy']);
                }
                ...
                //sj 20150915 设定地址是否对当前用户显示
                $follow_list[$k]['member_areainfo'] = ($followed >= intval($v['member_privacy']['area'])) ? $v['member_areainfo'] : L('home_member_privary');
            }
        }
        //将保密的个人信息 不显示出来
    ...
    public function fanOp()
    {
        ...
        $page->setEachNum(15);
        $page->setStyle('admin');
        $fan_list = $friend_model->listFriend(array(
            'friend_tomid' => "{$_SESSION['member_id']}"
        ), '*', $page, 'fromdetail');
        if (! empty($fan_list)) {
