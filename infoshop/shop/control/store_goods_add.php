...
class store_goods_addControl extends BaseSellerControl
{
    ...
    public function save_goodsOp()
    {
        if (chksubmit()) {
            ...
            $common_array['is_gift'] = intval($_POST['is_gift']);
            $common_array['gift_points'] = intval($_POST['gift_points']);

            if ($common_array['is_gift'] > 0 && $common_array['gift_points'] <= 0) {
            ...
            if ($common_id) {
                ...
                if (is_array($_POST['spec'])) {
                    foreach ($_POST['spec'] as $value) {
                        ...
                        $goods['is_gift'] = $common_array['is_gift'];
                        $goods['gift_points'] = $common_array['gift_points'];
                ...
                if (isset($_POST['starttime'])) {
                    ...
                    if ($selltime > TIMESTAMP) {
                        ...
                        showMessage("上架时间不能小于当前时间", getReferer(), 'html', 'error');
                        die();
                    }
                }
                // 记录日志
                $this->recordSellerLog('添加商品，平台货号:' . $common_id);
                redirect(urlShop('store_goods_add', 'add_step_three', array(
                    'commonid' => $common_id
                )));
            } else {
