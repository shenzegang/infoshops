...
class member_favoritesControl extends BaseMemberControl
{
    ...
    public function favoritesgoodsOp()
    {
        ...
        if ($fav_id <= 0) {
            echo json_encode(array(
                'done' => false,
                'msg' => Language::get('favorite_collect_fail', 'UTF-8')
            ));
            die();
        }
        ...
        $goods_info = $goods_model->getGoodsInfo(array(
            'goods_id' => $fav_id
        ));
        if ($goods_info['store_id'] == $_SESSION['store_id']) {
            echo json_encode(array(
                'done' => false,
                'msg' => Language::get('favorite_no_my_product', 'UTF-8')
            ));
            die();
        }
        // 添加收藏
        ...
        if ($result) {
            ...
            echo json_encode(array(
                'done' => true,
                'msg' => Language::get('favorite_collect_success', 'UTF-8')
            ));
            die();
        } else {
            echo json_encode(array(
     ...
     * 批量增加商品收藏
     */
    public function favoritesselectedgoodsOp()
    {
        ...
        foreach ($goods_ids_array as $goods_id) {
            ...
            $insert_arr['fav_type'] = 'goods';
            $insert_arr['fav_time'] = time();
            $flag = $favorites_model->addFavorites($insert_arr);
            if ($flag) {
                // 增加收藏数量
                $goods_model->editGoods(array(
                    'goods_collect' => array(
    ...
    public function favoritesstoreOp()
    {
        ...
        if ($fav_id == $_SESSION['store_id']) {
            echo json_encode(array(
                'done' => false,
                'msg' => Language::get('favorite_no_my_store', 'UTF-8')
            ));
            die();
        }
        // 添加收藏
    ...
    public function fglistOp()
    {
        ...
        $favorites_list = $favorites_model->getGoodsFavoritesList(array(
            'member_id' => $_SESSION['member_id']
        ), '*', 20);
        ...
        if (! empty($favorites_list) && is_array($favorites_list)) {
            ...
            $goods_list = $goods_model->getGoodsStoreList(array(
                'goods_id' => array(
                    'in',
                    $favorites_id
                )
    ...
    public function fslistOp()
    {
        ...
        if (! empty($favorites_list) && is_array($favorites_list)) {
            ...
            $store_model = Model('store');
            $store_list = $store_model->getStoreList(array(
                'store_id' => array(
                    'in',
                    $favorites_id
                )
    ...
    public function store_goodsOp()
    {
        ...
        if ($store_id > 0) {
            ...
            $condition['goods_addtime'] = array(
                'between',
                $add_time_from . ',' . TIMESTAMP
            );
            $goods_model = Model('goods');
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        $menu_array = array(
            1 => array(
                ...
                'menu_url' => 'index.php?act=member_favorites&op=fglist'
            ),
            2 => array(
                'menu_key' => 'fav_store',
                'menu_name' => Language::get('nc_member_path_collect_store'),
                'menu_url' => 'index.php?act=member_favorites&op=fslist'
            )
        );
