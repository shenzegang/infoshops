...
class taobao_importControl extends BaseSellerControl
{
    ...
    public function indexOp()
    {
        ...
        if (! $_POST) {
            //************************************20150827  tjz修改 根据卖家当前可发布分类展示导入分类********************************
            ...
            $store_goods_class = $model_store_class->getClassTree(array(
                'store_id' => $_SESSION['store_id'],
                'stc_state' => '1'
            ));
            ...
            $gc_sub_list = $gc->getClassList(array(
                'gc_parent_id' => intval($_POST['gc_id']),
                'gc_show' => 1
            ));
            ...
            // 商品数量
            $goods_num = $model_store_goods->getGoodsCommonCount(array(
                'store_id' => $_SESSION['store_id']
            ));
            ...
            foreach ($record as $k => $col) {
                ...
                switch ($k) {
                    case $fields_cols['goods_body']:
                        $return[$key]['goods_body'] = str_replace(array(
                            "\\\"\\\"",
                            "\"\""
                        ), array(
                            "\\\"",
            ...
            if (is_array($gid_arr) && ! empty($gid_arr)) {
                ...
                foreach ($gid_arr as $v1) {
                    ...
                    $upload_list = $upload_model->getUploadList(array(
                        'item_id' => $v1
                    ), 'upload_id,file_name,file_thumb');
                    $goods_image = ''; // 商品默认图
                    $i = 1;
                    foreach ($upload_list as $k2 => $v2) {
                        if ($k2 == 0)
                            $goods_image = $v2['file_name'];
                            // 上传多图
                        $tmp_insert = array();
