...
class homeControl extends BaseMemberControl{
    ...
    public function memberOp()
    {
        ...
        if (chksubmit()) {
            ...
            $member_array['member_tel'] = $_POST['member_tel'];
            ...
            $mobile_phone = $_POST['member_tel'];
            if($member_info['member_tel'] != $_POST['member_tel']) {
                $code = $model_member->getOneSMS($mobile_phone);
                if ($code[0]['code'] == $_POST['code'] && time() - strtotime($code[0]['send_date']) < C('sms_invalid')) {
    ...
    public function moreOp()
    {
        ...
        // 用户标签列表
        $mtag_array = $model->table('sns_membertag')
            ->order('mtag_sort asc')
            ->limit(1000)
            ->select();
        
        // 用户已添加标签列表。
        $mtm_array = $model->table('sns_mtagmember')
            ->where(array(
            'member_id' => $_SESSION['member_id']
        ))
            ->select();
    ...
    public function passwdOp()
    {
        ...
        if (chksubmit()) {
            ...
            $obj_validate->validateparam = array(
                array(
                    "input" => $_POST["orig_password"],
                    "require" => "true",
                    "message" => $lang['home_member_old_password_null']
                ),
                array(
                    "input" => $_POST["new_password"],
                    "require" => "true",
                    "message" => $lang['home_member_new_password_null']
                ),
                array(
                    ...
                    "message" => $lang['home_member_input_two_password_again']
                )
            );
            ...
            $member_info = $model_member->where(array(
                'member_name' => trim($_SESSION['member_name']),
                'member_passwd' => md5(trim($_POST['orig_password']))
            ))->find();
            if (empty($member_info)) {
                showDialog($lang['home_member_old_password_wrong']);
            }
            $update = $model_member->update(array(
                'member_passwd' => md5(trim($_POST['new_password'])),
                'member_id' => $_SESSION['member_id']
            ));
            if ($update) {
        ...
        if (chksubmit()) {
            ...
            $obj_validate->validateparam = array(
                array(
                    "input" => $_POST["orig_password"],
                    "require" => "true",
                    "message" => $lang['home_member_password_null']
                ),
                array(
                    "input" => $_POST["email"],
                    "require" => "true",
                    "validator" => "email",
                    "message" => $lang['home_member_input_email_again_format']
                )
            );
            $error = $obj_validate->validate();
            if ($error != '') {
    ...
    public function messageOp()
    {
        ...
        $page->setEachNum(10);
        $page->setStyle('admin');
        $message_array = $model_message->listMessage(array(
            'message_type' => '2',
    ...
    {
        $model_message = Model('message');
        $countnum = $model_message->countMessage(array(
            'message_type' => '2',
            'to_member_id_common' => $_SESSION['member_id'],
            'no_message_state' => '2',
    ...
    public function sendmsgOp()
    {
        ...
        if ($member_id > 0) {
            ...
            $friend_list = $friend_model->listFriend(array(
    ...
    public function savemsgOp()
    {
        ...
        if (chksubmit()) {
            ...
            // 查询有效会员
            $member_model = Model('member');
            $member_list = $member_model->getMemberList(array(
                'member_name' => array(
                    'in',
                    $membername_arr
                )
    ...
    public function showmsgcommonOp()
    {
        ...
        if ($message_info['message_parent_id'] > 0) {
            // 查询该站内信的父站内信
            $parent_array = $model_message->getRowMessage(array(
                'message_id' => "{$message_info['message_parent_id']}",
                'message_type' => '0',
                'no_message_state' => '2'
            ));
            // 查询该站内信的回复站内信
            $reply_array = $model_message->listMessage(array(
        ...
        if (! empty($messageid_arr)) {
            ...
            $model_message->updateCommonMessage(array(
                'message_open' => '1'
            ), array(
                'message_id_in' => "$messageid_str"
            ));
        }
        // 更新未读站内信数量cookie值
        $cookie_name = 'msgnewnum' . $_SESSION['member_id'];
    ...
    public function showmsgbatchOp()
    {
        ...
        if ($drop_type == 'msg_seller') {
            ...
            $store_info = $model_store->getStoreInfo(array(
                'member_id' => "{$message_info['from_member_id']}"
            ));
    ...
    public function savereplyOp()
    {
        ...
        if ($_POST['form_submit'] == 'ok') {
            ...
            if ($insert_state) {
                ...
                $model_message->updateCommonMessage($update_arr, array(
                    'message_id' => "{$insert_arr['message_parent_id']}"
                ));
            }
            if ($_GET['inajax'] == 1) {
    ...
    public function upload_idcardOp()
    {
    	...
    	$ext = strtolower(pathinfo($_FILES['pic']['name'], PATHINFO_EXTENSION));
    	$upload->set('file_name', "idcard_$member_id.$ext");
    	$upload->set('thumb_ext', '_new');
    	$upload->set('ifremove', true);
    	$upload->set('default_dir', ATTACH_IDCARD);
    	if (! empty($_FILES['pic']['tmp_name'])){
    		$result = $upload -> upfile('pic');
    		if (! $result) {
    			showMessage($upload->error, '', 'html', 'error');
    	...
    	Model()->table('member') -> where(array('member_id' => $_SESSION['member_id'])) -> update(array('idcard' => $_POST['idcard'],'member_truename' => $_POST['member_truename'], 'idcard_chk' => 1,'idcard_photo' => 'idcard_' . $_SESSION['member_id'] . '_new.jpg'));
    	self::profile_menu('member', 'idcard');
    	Tpl::output('menu_sign', 'profile');
    	Tpl::output('menu_sign_url', 'index.php?act=home&op=member');
    	Tpl::output('menu_sign1', 'avatar');
    	Tpl::output('member_idcard', 'idcard_' . $_SESSION['member_id'] . '_new.jpg');
    	Tpl::output('newfile', $upload->thumb_image);
    	Tpl::output('height', get_height(BASE_UPLOAD_PATH . '/' . ATTACH_AVATAR . '/' . $upload->thumb_image));
    	Tpl::output('width', get_width(BASE_UPLOAD_PATH . '/' . ATTACH_AVATAR . '/' . $upload->thumb_image));
    	Tpl::setLayout('member_pub_layout');
    	//Tpl::showpage('member_idcard');
    	showDialog('身份证上传成功，等待审核', 'index.php?act=home&op=idcard', 'succ');
    }
    ...
    public function cutOp()
    {
        if (chksubmit()) {
            ...
            Model()->table('member')
                ...
                ->update(array(
                'member_avatar' => 'avatar_' . $_SESSION['member_id'] . '.jpg'
            ));
     ...
     * 身份认证
     */
    public function idcardOp(){
    	/**
    	 * 加载语言包
    	 */
    	Language::read('member_home_member');
    	self::profile_menu('member', 'idcard');
    	$mem_info = Model('member') -> getMemberInfo(array('member_id' => $_SESSION['member_id']), 'idcard,member_truename, idcard_photo, idcard_chk');
    	
    	Tpl::output('mem_info', $mem_info);
    	Tpl::output('menu_sign', 'profile');
    	Tpl::output('menu_sign_url', 'index.php?act=home&op=member');
    	Tpl::output('menu_sign1', 'idcard');
    	Tpl::setLayout('member_pub_layout');
    ...
    {
        Language::read('member_home_member');
        Model()->table('store')->update(array(
            ...
            case 'member':
                $menu_array = array(
                    1 => array(
                        ...
                        'menu_url' => 'index.php?act=home&op=member'
                    ),
                    2 => array(
                        'menu_key' => 'more',
                        'menu_name' => Language::get('home_member_more'),
                        'menu_url' => 'index.php?act=home&op=more'
                    ),
                    3 => array(
                        'menu_key' => 'passwd',
                        'menu_name' => Language::get('home_member_modify_password'),
                        'menu_url' => 'index.php?act=home&op=passwd'
                    ),
                    4 => array(
                        'menu_key' => 'email',
                        'menu_name' => Language::get('home_member_modify_email'),
                        'menu_url' => 'index.php?act=home&op=email'
                    ),
                    5 => array(
                        'menu_key' => 'avatar',
                        'menu_name' => Language::get('home_member_modify_avatar'),
                        'menu_url' => 'index.php?act=home&op=avatar'
                    ),
                    6 => array(
	                    'menu_key' => 'idcard',
	                    'menu_name' => Language::get('home_member_idcardauth'),
	                    'menu_url' => 'index.php?act=home&op=idcard'
                    )
                );
                break;
            case 'message':
                $menu_array = array(
                    1 => array(
                        ...
                        'menu_url' => 'index.php?act=home&op=message'
                    ),
                    2 => array(
                        'menu_key' => 'private',
                        'menu_name' => Language::get('home_message_private_message'),
                        'menu_url' => 'index.php?act=home&op=privatemsg'
                    ),
                    3 => array(
                        'menu_key' => 'system',
                        'menu_name' => Language::get('home_message_system_message'),
                        'menu_url' => 'index.php?act=home&op=systemmsg'
                    ),
                    4 => array(
                        'menu_key' => 'close',
                        'menu_name' => Language::get('home_message_close'),
                        'menu_url' => 'index.php?act=home&op=personalmsg'
                    )
                );
                if ($menu_key == 'sendmsg') {
                    $menu_array[5] = array(
                        ...
                        'menu_url' => 'index.php?act=home&op=sendmsg'
                    );
