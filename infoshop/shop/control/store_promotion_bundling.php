...
class store_promotion_bundlingControl extends BaseSellerControl
{
    ...
    public function bundling_listOp()
    {
        ...
        if (! empty($bundling_quota)) {
            // 计算已经发布活动、剩余活动数量
            $bundling_published = $model_bundling->getBundlingCount(array(
            ...
            // 状态数组
            $state_array = array(
                0 => Language::get('bundling_status_0'),
                1 => Language::get('bundling_status_1')
            );
    ...
    public function bundling_addOp()
    {
        ...
        if (intval(C('promotion_bundling_sum')) != 0 && ! isset($_REQUEST['bundling_id'])) {
            $count = $model_bundling->getBundlingCount(array(
                'store_id' => $_SESSION['store_id']
            ));
        ...
        if (chksubmit()) {
            ...
            $model_bundling->delBundlingGoods(array(
