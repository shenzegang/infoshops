...
<div class="ncc-receipt-info" id="paymentCon">
	...
	<div id="payment_list" class="ncc-candidate-items"
		style="display: none">
		<ul>
			<li><input type="radio" value="online" name="payment_type"
				id="payment_type_online"> <label for="payment_type_online">在线支付</label>
			</li>
			<li>
        <?php if ($output['ifshow_offpay'] === true) {?>
        <input type="radio" value="offline" name="payment_type"
				id="payment_type_offline"> <label for="payment_type_offline">货到付款</label>
        <?php if (count($output['pay_goods_list']['online']) > 0) {?>
        <a id="show_goods_list" style="display: none"
				class="ncc-payment-showgoods" href="javascript:void(0);"><i
					class="icon-truck"></i>货到付款 (<?php echo count($output['pay_goods_list']['offline']);?>样商品) + <i
					class="icon-credit-card"></i>在线支付 (<?php echo count($output['pay_goods_list']['online']);?>样商品)</a>
        <?php } ?>
        <?php } ?>
      </li>
		</ul>
		<div class="hr16">
			<a href="javascript:void(0);" class="ncc-btn ncc-btn-red"
				id="hide_payment_list">保存支付方式</a>
		</div>
	</div>
  <?php if ($output['ifshow_offpay']) {?>
  <div id="ncc-payment-showgoods-list"
		class="ncc-payment-showgoods-list">
		<dl>
      ...
      <dt><?php echo $type == 'offline' ? '货到付款' : '在线支付';?></dt>
        ...
        <div class="goods-thumb">
					<span><img src="<?php echo thumb($value,60);?>"></span>
				</div>
        <?php } ?>
      </dd>
      <?php } ?>
...
<div id="confirm_offpay_goods_list" style="display: none;">
  ...
  <dl class="ncc-offpay-list">
		<dt>
			以下商品支持<strong><?php echo $type == 'offline' ? '货到付款' : '在线支付';?></strong>
		</dt>
		<dd>
			<ul>
        <?php foreach($data as $value) {?>
        <li><span title="<?php echo $value['goods_name'];?>"><img
						src="<?php echo thumb($value,60);?>"></span></li>
        <?php } ?>
      </ul>
			<label> <input type="radio" value="" checked="checked">
  ...
  <div class="tc mt10 mb10">
		<a href="javascript:void(0);" class="ncc-btn ncc-btn-orange"
			id="close_confirm_button">确认支付方式</a>
	</div>
</div>
<?php } ?>
