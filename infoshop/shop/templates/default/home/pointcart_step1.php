...
<div class="wrapper pr">
	...
	<div class="ncc-main">
		...
		<form method="post" id="porder_form" name="porder_form"
			action="index.php?act=pointcart&op=step2">
			<div class="ncc-receipt-info">
				<div class="ncc-receipt-info-title">
					<h3><?php echo $lang['pointcart_step1_receiver_address'];?><a
							href="index.php?act=member&op=address" target="_blank">[管理]</a>
					</h3>
				</div>
				<!-- 已经存在的收获地址start -->
          ...
          <ul
						class="receive_add address_item <?php if ($k == 0) echo 'selected_address'; ?>">
						<li class="goto">
              <?php if ($k == 0) echo $lang['cart_step1_receiver_jsz'];else echo '&nbsp;';?>
            </li>
						<li
							address="<?php echo $val['area_info']; ?>&nbsp;&nbsp;<?php echo $val['address']; ?>"
							buyer="<?php echo $val['true_name']; ?>&nbsp;&nbsp;<?php if($val['mob_phone']) echo $val['mob_phone']; else echo $val['tel_phone']; ?>">
							<input id="address_<?php echo $val['address_id']; ?>"
							type="radio" city_id="<?php echo $val['city_id']?>"
							name="address_options" value="<?php echo $val['address_id']; ?>"
          ...
          <?php } ?>
          <input type="hidden" id="chooseaddressid"
						name="chooseaddressid"
						value='<?php echo $output['address_list'][0]['address_id'];?>' />
				</div>
				<!-- 已经存在的收获地址end -->
			</div>
            ...
            <tr class="shop-list ">
							<td></td>
							<td class="w60"><a
								href="<?php echo urlShop('pointprod', 'pinfo', array('id' => $val['pgoods_id']));?>"
								class="ncc-goods-thumb" target="_blank"><img
									src="<?php echo pointprodThumb($val['pgoods_image']); ?>"
									alt="<?php echo $val['pgoods_name']; ?>" /></a></td>
							<td class="tl"><dl class="ncc-goods-info">
									<dt>
										<a target="_blank"
											href="<?php echo urlShop('pointprod', 'pinfo', array('id' => $val['pgoods_id']));?>"><?php echo $val['pgoods_name']; ?></a>
									</dt>
								</dl></td>
            ...
            <tr>
                  ...
                  <input type="text" class="w400 text"
									onclick="pcart_messageclear(this);"
									value="<?php echo $lang['pointcart_step1_message_advice'];?>" />
							</label></td>
						</tr>
					</tbody>
					<tfoot>
			...
			<div class="ncc-bottom">
				<a href="index.php?act=pointcart" class="ncc-btn"><?php echo $lang['pointcart_step1_return_list'];?></a><a
					href="javascript:void($('#porder_form').submit());"
					class="ncc-btn ncc-btn-acidblue fr"><?php echo $lang['pointcart_step1_submit'];?></a>
			</div>
		</form>
	</div>
</div>
