...
<div class="nc-login-layout" style="width: 1000px;">
	<div id="login_ol">
		<ol>
			...
			<li<?php if($_GET['mobile'] !="" || $_GET['cur'] ==2) {?> class="active"<?php } ?>><i>2</i><span><?php echo $lang['login_usersave_regist_info'];?></span></li>
	...
	<div class="nc-login-content" style="border: none; padding: 50px 0; width: 358px; margin: 0 auto;">
		<form id="register_form" method="post" action="<?php if($_GET['mobile'] != ""){?><?php echo SHOP_SITE_URL;?>/index.php?act=login&op=usersave&cur=1&mobile=<?php echo $_GET['mobile'];?><?php } ?>">
			<?php if($_GET['mobile'] == ""){?>
			<dl>
				<dt><?php echo $lang['login_register_member_tel'];?></dt>
				<dd style="min-height: 54px;">
					<input type="text" id="member_tel" name="member_tel" class="text tip"
						   onblur="telCheck(this.value)" onfocus=" telFocus();
						 "title="<?php echo $lang['login_register_input_valid_member_tel'];?>" value="<?php echo $_POST['member_tel'];?>" maxlength="11"/>
			...
			<dl>
				...
				<dl>
					<dt></dt>
					<dd>
						<a href="index.php?act=login&op=usersave&cur=email" style="color: #c53801;"><?php echo $lang['login_register_regist_email'];?></a>
					</dd>
				</dl>
			</div>
			<?php }else{?>
...
<script>
	...
	$("#register_form").validate({
		...
		rules : {
			member_tel : {
				required: true,
				remote   : {
					url :'index.php?act=login&op=check_tel',
					type:'get',
					data:{
						member_tel : function(){
		...
		messages : {
			member_tel : {
				required: '<?php echo $lang['login_register_input_tel'];?>',
		...
		$('#Submit_next').click(function(){
			...
			if(!codeCheck($("#code").val())){
				...
				$.ajax({
					...
					data: {mobile_phone:Member_tphone_val ,code:Member_code_val},
					success: function(data){
						if(data == 1){
							location.href = "<?php echo SHOP_SITE_URL;?>/index.php?act=login&op=usersave&mobile="+Member_tphone_val;
						}
						else{
	...
	function settime(val) {
		...
		if(document.getElementById("member_tel").value == ""){
			$("#error_tel").html('<label for="company_registered_capital" class="error">手机号码不能为空</label>');
		}
		else if($("#error_tel").html()!= ""){
			$("#error_tel").html('<label for="company_registered_capital" class="error">请输入正确的手机号码</label>');
		}
		else if($("label[for='member_tel']").css("display") == 'block'){
		}
		else {
			if (countdown == 0) {
	...
	function telCheck(tel) {
		...
		if($("#member_tel").val() == '' && $("label[for='member_tel']").css("display") == 'none'){
			$("#error_tel").html('<label for="company_registered_capital" class="error"><?php echo $lang['login_register_input_tel'];?></label>');
		}
		else if (!pattern.test(tel) && $("#member_tel").val() != '') {
			$("#error_tel").html('<label for="company_registered_capital" class="error">请输入正确的手机号码</label>');
		}
		else{
			$("#error_tel").html('');
