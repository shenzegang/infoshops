...
<div class="side-message">
  ...
  <div class="guest-form">
		<form id="send_form"
			action='index.php?act=home&op=savemsg&type=sns_board' method="post">
			<input type="hidden" value="ok" name="form_submit"> <input
				type="hidden" name="msg_type" value="2" /> <input type="hidden"
				value="<?php echo $output['member_info']['member_name'];?>"
				name="to_member_name">
			<textarea id="content_msg" name="msg_content"
				placeholder="<?php echo $lang['sns_message_content_placeholder'];?>@<?php echo !empty($output['member_info']['member_truename'])?$output['member_info']['member_truename']:$output['member_info']['member_name'];?><?php echo $lang['sns_talk'];?>~"
				class="msg-content"></textarea>
			<div class="action">
				<a class="face" href="javascript:void(0);"
					data-param='{"txtid":"msg"}' nc_type="smiliesbtn"><?php echo $lang['sns_smiles'];?></a><a
					href="javascript:void(0);" nctype="commentbtn" class="btn"><?php echo $lang['sns_speak'];?></a><span
					class="charcount"><em id="commentcharcount"></em>/140</span>
			</div>
		</form>
	</div>
  <?php }?>
  <div nctype="message_list" class="message-list">
    ...
    <dl nctype="dl<?php echo $val['message_id'];?>">
			<dt>
				<a
					href="index.php?act=member_snshome&mid=<?php echo $val['from_member_id'];?>"
					data-param="{'id':<?php echo $val['from_member_id'];?>}"
					nctype="mcard"><?php echo $val['from_member_name'].$lang['nc_colon'];?></a><span><?php echo parsesmiles($val['message_body']);?></span>
			</dt>
			<dd
				data-param="{'msgid':'<?php echo $val['message_id'];?>','fname':'<?php echo $val['from_member_name'];?>'}">
				<span class="time"><?php echo $val['message_time'];?></span><span
					class="handle">
        <?php if($output['relation'] == 3){?>
        ...
        <a
					onclick="ajax_get_confirm('<?php echo $lang['nc_common_op_confirm'];?>', 'index.php?act=home&op=dropcommonmsg&drop_type=sns_msg&message_id=<?php echo $val['message_id'];?>');"
      ...
      <dd>
        ...
        <dl class="re-content">
					<dt>
						<a
							href="index.php?act=member_snshome&mid=<?php echo $v['from_member_id'];?>"><?php echo $v['from_member_name'];?><?php echo $lang['sns_reply'].$lang['nc_colon'];?></a><span><?php echo parsesmiles($v['message_body']);?></span>
					</dt>
					<dd data-param="{'msgid':'<?php echo $v['message_id'];?>'}">
						<span class="time"><?php echo $v['message_time'];?></span>
					</dd>
				</dl>
        <?php }?>
      </dd>
