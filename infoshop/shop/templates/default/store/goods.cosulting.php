...
<div class="ncs-cosult-list">
	<dl class="asker">
	  ...
	  <?php if($v['member_id']== '0') echo $lang['nc_guest']; else if($v['isanonymous'] == 1){?>
        ...
        <a
				...
				target="_blank" data-param="{'id':<?php echo $v['member_id'];?>}"
				nctype="mcard"><?php echo str_cut($v['cmember_name'],8);?></a>
        <?php }?>        
        <time
				datetime="<?php echo date("Y-m-d H:i:s",$v['consult_addtime']);?>"
				pubdate="pubdate" class="ml20"><?php echo date("Y-m-d H:i:s",$v['consult_addtime']);?></time>
		</dd>
	</dl>
	<dl class="ask-con">
    ...
    <dl class="reply">
		...
		<dd>
			...
			<time
				datetime="<?php echo date("Y-m-d H:i:s",$v['consult_reply_time']);?>"
				pubdate="pubdate">[<?php echo date("Y-m-d H:i:s",$v['consult_reply_time']);?>]</time>
		</dd>
	</dl>
    <?php }?>
  </div>
...
<form method="post" id="message"
	action="index.php?act=goods&op=save_consult&id=<?php echo $_GET['id']; ?>">
...
<input type="hidden" name="form_submit" value="ok" /> <input
  ...
  <input type="hidden" name="goods_id"
		value="<?php echo $_GET['goods_id']; ?>" />
  <?php }?>
  <div class="ncs-consult-form">
		<div class="asker-info">
      <?php if($_SESSION['member_id']){ ?>
      <label><strong><?php echo $lang['goods_index_member_name'].$lang['nc_colon'];?></strong><?php echo $_SESSION['member_name'];?><input
				type="hidden" name="email" id="gbTextfield"
				value="<?php echo $_SESSION['member_email']; ?>" /></label> <label
				for="gbCheckbox"><input type="checkbox" class="checkbox"
				name="hide_name" value="hide" id="gbCheckbox"><?php echo $lang['goods_index_anonymous_publish'];?></label>
      <?php }else{ ?>
      <label for="gbTextfield"><strong><?php echo $lang['goods_index_email'];?></strong>
				<input type="text" name="email" id="gbTextfield" class="text w300"
				placeholder="非会员可输入邮件进行咨询，以便客服人员给您回执。" value="" /><span></span> </label>
      <?php }?>
    </div>
		<div class="ask-content">
			<strong>咨询内容：</strong>
			<textarea name="goods_content" id="textfield3"
				class="textarea w700 h60"></textarea>
			<span></span>
		</div>
		<div class="bottom">
			<a href="JavaScript:void(0);" nc_type="consult_submit"
				title="<?php echo $lang['goods_index_publish_consult'];?>"
				class="ncs-btn ncs-btn-red"><?php echo $lang['goods_index_publish_consult'];?></a><span
				id="consultcharcount"></span>
		</div>
	</div>
</form>
<script type="text/javascript">
