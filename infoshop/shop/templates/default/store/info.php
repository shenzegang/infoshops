...
<div class="ncs-info">
	...
	<div class="content">
		...
		<div class="detail-rate">
			<h5>
				<strong><?php echo $lang['nc_dynamic_evaluation'];?></strong>与行业相比
			</h5>
			<ul>
        <?php  foreach ($output['store_info']['store_credit'] as $value) {?>
        <li> <?php echo $value['text'];?><span class="credit"><?php echo $value['credit'];?> 分</span>
		...
		<div class="store-score">
			<strong>信用等级：</strong>
			<p>
      <?php for($i = 1; $i <= intval($output['store_info']['store_score']); $i++){ ?>
      <span></span>
      <?php } ?>
      </p>
		</div>
    ...
    <?php } ?>
		<?php if($output['seller_deposit']['is_show'] == 1 && $output['seller_deposit']['paid'] == 'Y' ) {?>
		<dl class="no-border">
			<dt>店铺保证金：</dt>
			<dd><?php echo $output['seller_deposit']['deposit_amount'];?>元</dd>
		</dl>
		<?php }?>
    <dl class="no-border">
			<dt>公司名称：</dt>
		...
		<div class="goto">
			<a
				...
				target="_blank">进入商家店铺</a><a
				href="javascript:collect_store('<?php echo $output['store_info']['store_id'];?>','count','store_collect')">收藏店铺<em
		...
		<div class="qrcode">
			...
			<div class="qrcode-img">
				<img
					src="/shop/qrcode/<?php echo $output['store_info']['store_id'];?>.png">
			</div>
		</div>
	</div>
</div>
