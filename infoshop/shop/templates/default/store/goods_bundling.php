...
<div class="ncs-bundling-container">
  ...
  <div class="ncs-bundling-list">
    ...
    <li>
					<dl>
						<dt class="goods-name" title="<?php echo $v['name'];?>">
							<a
								href="<?php echo urlShop('goods', 'index', array('goods_id' => $v['id']));?>"
								target="block"><?php echo $v['name'];?></a>
						</dt>
						<dd class="goods-thumb">
							<a
								href="<?php echo urlShop('goods', 'index', array('goods_id' => $v['id']));?>"
								target="block"><img src="<?php echo $v['image'];?>"
								title="<?php echo $v['name'];?>" alt="<?php echo $v['name'];?>"
    ...
    <?php }?>
		...
		<div class="ncs-bundling-price">
			<p class="tcj">
				套装价：<span><?php echo $lang['currency'].$val['price'];?></span>
			</p>
			<p class="js"><?php echo $lang['bundling_save'];?><span><?php echo $lang['currency'].ncPriceFormat(floatval($val['cost_price'])-floatval($val['price']));?></span>
			</p>
    <?php if ($val['freight'] > 0) {?>
    <p class="">
				运&emsp;费：<span><?php echo $lang['currency'].$val['freight'];?></span>
			</p>
    <?php }?>
    <p class="mt5">
				<a href="javascript:void(0);" nctype="addblcart_submit"
					bl_id="<?php echo $val['id']?>" class="ncs-btn ncs-btn-red"><i
					class="icon-th-large"></i><?php echo $lang['bundling_buy'];?></a>
			</p>
		</div>
  <?php }?>
  <?php }?>
