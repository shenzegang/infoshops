...
.share-widget .s-app i {
	background:
		url("<?php echo SHOP_TEMPLATES_URL;?>/images/member/shareicon/shareicons.gif")
		no-repeat;
...
.feededitor ul li a:hover, .feededitor ul li a.selected {
	border-color: #80B8D2;
}

.feededitor ul li .extra {
	display: none;
}

.feededitor ul li a.selected .extra {
	font-size: 12px;
	line-height: 20px;
	color: #fff;
	background-color: rgba(128, 184, 210, 0.85) !important;
	background-color: #80B8D2\9 /* IE6-8 */;
	text-align: center;
	display: block;
	width: 90px;
	height: 20px;
	border-top: solid 1px #80B8D2;
	position: absolute;
	z-index: 1;
	left: 1px;
	bottom: 1px;
...
.feededitor .handle input.button {
...
.privacytab .menu-bd li {
	line-height: 20px;
	color: #666666;
	background: none repea0t scroll 0 0 white;
	height: 20px;
...
.privacytab .menu-bd li .selected {
	background: url(<?php echo SHOP_TEMPLATES_URL;?>/images/right.gif)
...
<div class="feededitor">
	<form method="post"
		action="index.php?act=member_snsindex&op=sharestore"
		id="sharestore_form">
		<input type="hidden" name="form_submit" value="ok" /> <input
			type="hidden" id="choosestoreid" name="choosestoreid"
			value="<?php echo intval($output['store_info']['store_id']);?>" />
		<div class="goods">
			<div class="pic">
				<span class="thumb size90"><i></i><a target="_blank"
					href="<?php echo $output['store_info']['store_url'];?>"><img
						title="<?php echo $output['store_info']['store_name']; ?>"
						src="<?php echo getStoreLogo($output['store_info']['store_label']);?>"
						onload="javascript:DrawImage(this,90,90);" /></a></span>
			</div>
			<dl class="intro">
				<dt>
					<a target="_blank"
						href="<?php echo $output['store_info']['store_url'];?>"><?php echo $output['store_info']['store_name']; ?></a>
				</dt>
...
with(document)0[(getElementsByTagName('head')[0]||body).appendChild(createElement('script')).src='http://bdimg.share.baidu.com/static/api/js/share.js?v=89860593.js?cdnversion='+~(-new Date()/36e5)];
    ...
    <div class="p10">
			<textarea
				placeholder="<?php echo $lang['sns_sharestore_contenttip'];?>"
				name="comment" id="content_ssweibo" resize="none"></textarea>
			...
			<div class="handle">
				...
				<div nc_type="formprivacydiv"
					...
					<span class="privacybtn" nc_type="formprivacybtn"><?php echo $lang['sns_weiboprivacy_default'];?><i></i></span>
					<div class="privacytab" nc_type="formprivacytab"
						style="display: none;">
						<ul class="menu-bd">
							<li nc_type="formprivacyoption"
								data-param='{"v":"0","hiddenid":"sprivacy"}'><span
								class="selected"><?php echo $lang['sns_weiboprivacy_all'];?></span></li>
							<li nc_type="formprivacyoption"
								data-param='{"v":"1","hiddenid":"sprivacy"}'><span><?php echo $lang['sns_weiboprivacy_friend'];?></span></li>
							<li nc_type="formprivacyoption"
								data-param='{"v":"2","hiddenid":"sprivacy"}'><span><?php echo $lang['sns_weiboprivacy_self'];?></span></li>
						</ul>
					</div>
