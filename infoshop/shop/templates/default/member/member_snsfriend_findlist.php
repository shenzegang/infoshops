...
<div class="wrap">
	...
	<div class="friendfind">
		...
		<form method="post" id="search_form"
			action="index.php?act=member_snsfriend&op=findlist">
			<div class="search-form">
				<div class="pt5 pb5">
					<span class="pl50"></span> <input type="text" class="text"
						style="width: 340px;" name="searchname" id="searchname"
						value="<?php echo $_GET['searchname'];?>"
						placeholder="<?php echo $lang['snsfriend_find_keytip'];?>"> <a
						class="submit" nctype="search_submit"><?php echo $lang['snsfriend_search'];?>"</a>
				...
				<div class="pb5" style="display: none;" nctype="advanced_search">
					<span class="pl50"></span> <select nctype="area" name="provinceid"
						id="provinceid">
					</select> <select nctype="area" name="cityid" id="cityid">
						<option><?php echo $lang['snsfriend_city'];?></option>
					</select> <select nctype="area" name="areaid" id="areaid">
				      ...
				      <tr>
				            ...
				            <li id="recordone_<?php echo $v['member_id']; ?>">
														<div class="friend-img">
															<span class="thumb size100"> <a
																href="index.php?act=member_snshome&mid=<?php echo $v['member_id'];?>"
																target="_blank"><img
																	src="<?php if ($v['member_avatar']!='') { echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.DS.$v['member_avatar']; } else { echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('default_user_portrait'); } ?>"
																	alt="<?php echo $v['member_name']; ?>"
																	onload="javascript:DrawImage(this,100,100);"
																	data-param="{'id':<?php echo $v['member_id'];?>}"
																	nctype="mcard" /></a>
															</span>
														</div>
														<dl>
															<dt>
																<a
																	href="index.php?act=member_snshome&mid=<?php echo $v['member_id'];?>"
																	class="friend-name"
																	title="<?php echo $v['friend_tomname']; ?>"
																	target="_blank"
																	data-param="{'id':<?php echo $v['member_id'];?>}"
																	nctype="mcard"><?php echo $v['member_name']; ?></a> <a
																	href="index.php?act=home&op=sendmsg&member_id=<?php echo $v['member_id']; ?>"
																	target="_blank" class="message"
																	title="<?php echo $lang['nc_message'] ;?>">&nbsp;</a> <em
																	class="<?php echo $v['sex_class']; ?>"></em> </a>
															</dt>
															<dd class="area"><?php echo $v['member_areainfo'];?></dd>
															<dd nc_type="signmodule">
																<span nc_type="mutualsign" style="<?php echo $v['followstate']!=2?'display:none;':'';?>"><?php echo $lang['snsfriend_follow_eachother'];?></span>
																<span nc_type="followsign" style="<?php echo $v['followstate']!=1?'display:none;':'';?>"><?php echo $lang['snsfriend_followed'];?></span>
																<a href="javascript:void(0)" class="ncu-btn2" nc_type="followbtn" data-param='{"mid":"<?php echo $v['member_id'];?>"}'style="<?php echo $v['followstate']!=0?'display:none;':'';?>"><?php echo $lang['snsfriend_followbtn'];?></a>
															</dd>
														</dl>
													</li>
				            <?php } ?>
