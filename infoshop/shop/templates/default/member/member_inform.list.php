...
<div class="wrap">
	...
	<form id="list_form" method="get">
		<table class="search-form">
			...
			<input type="hidden" id='op' name='op'
				value='voucher_template_inform_detail' />
			<tr>
				<td></td>
				<td class="w100 tr"><select name="select_inform_state">
						<option value="0"
							<?php if (!$_GET['select_inform_state'] == '0'){echo 'selected=true';}?>> <?php echo $lang['inform_state_all']; ?> </option>
						<option value="1"
							<?php if ($_GET['select_inform_state'] == '1'){echo 'selected=true';}?>> <?php echo $lang['inform_state_unhandle'];?> </option>
						<option value="2"
							<?php if ($_GET['select_inform_state'] == '2'){echo 'selected=true';}?>> <?php echo $lang['inform_state_handled'];?> </option>
				</select></td>
    ...
    <tbody>
      ...
      <tr class="bd-line">
				<td></td>
				<td class="tl"><a
					href="<?php echo urlShop('goods', 'index', array('goods_id'=>$val['inform_goods_id']));?>"
					target="_blank"> <?php echo $val['inform_goods_name']; ?> </a></td>
          ...
          <?php } ?></td>
            ...
            if ($val['inform_state'] === '1')
                echo $lang['inform_state_unhandle'];
            if ($val['inform_state'] === '2')
                echo $lang['inform_state_handled'];
            ?></td>
				<td><?php
            if ($val['inform_handle_type'] === '1')
                echo $lang['inform_handle_type_unuse'];
            if ($val['inform_handle_type'] === '2')
                echo $lang['inform_handle_type_venom'];
            if ($val['inform_handle_type'] === '3')
          ...
          <a class="ncu-btn2 mt5" href="javascript:void(0)"
					onclick="ajax_get_confirm('<?php echo $lang['inform_cancel_confirm'];?>', 'index.php?act=member_inform&op=inform_cancel&inform_id=<?php echo $val['inform_id']; ?>');"><?php echo $lang['nc_cancel']; ?></a>
          <?php } ?></td>
			</tr>
