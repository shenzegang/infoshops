...
<div class="wrap">
	...
	<div class="ncu-form-style">
		<form method="post" id="profile_more"
			action="index.php?act=home&op=more">
			...
			<h4 class="w230 mb10 tip"
				title="<?php echo $lang['home_memner_more_tag_list_title'];?>"><?php echo $lang['home_member_more_tag_list'];?></h4>
			<div nctype="list" class="user-tag-optional">
        <?php
        
if (! empty($output['mtag_list']) && is_array($output['mtag_list'])) {
            foreach ($output['mtag_list'] as $val) {
                ?>
        <span nctype="able"
					data-param='{"id":"<?php echo $val['mtag_id'];?>"}'><?php echo $val['mtag_name'];?></span>
        <?php
        ...
        ?>
			...
			<h4 class="w90 mt20 mb10 tip"
				title="<?php echo $lang['home_member_more_my_tag_title'];?>"><?php echo $lang['home_member_more_my_tag'];?></h4>
			<div nctype="choose" class="user-tag-selected">
        <?php
        
if (! empty($output['mtm_list']) && is_array($output['mtm_list'])) {
            foreach ($output['mtm_list'] as $val) {
                ?>
        <span nctype="able"
					data-param='{"id":"<?php echo $val['mtag_id'];?>"}'><?php echo $val['mtag_name'];?><a
					href="javascript:void(0)" nctype="delTag"> <input type="hidden"
						name="mid[]" value="<?php echo $val['mtag_id'];?>" /> X
				</a></span>
        <?php
