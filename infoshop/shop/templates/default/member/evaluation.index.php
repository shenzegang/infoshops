...
<div class="wrap">
	<div class="tabmenu">
		<ul id="listpj" class="tab">
			<li class="active"><a
				href="index.php?act=<?php echo $output['pj_act'];?>&op=list"><?php echo $lang['member_evaluation_toother'];?></a></li>
		</ul>
	</div>
	<form id="goodsevalform" method="get">
		<input type="hidden" name="act"
			value="<?php echo $output['pj_act'];?>" /> <input type="hidden"
			name="op" value="list" /> <input type="hidden" name="type"
        ...
        <?php foreach ((array)$output['goodsevallist'] as $k=>$v){?><tr>
				...
				<tr>
					<th colspan="20"><span class="ml10"><?php echo $lang['member_evaluation_goodsinfo_title'];?>
            <a target="_blank"
							href="<?php echo urlShop('goods', 'index', array('goods_id'=>$v['geval_goodsid'])); ?>"><?php echo $v['geval_goodsname']?></a>
					</span>
						<div class="raty" style="display: inline-block;"
							data-score="<?php echo $v['geval_scores'];?>"></div>
        <?php if(empty($v['geval_image'])) {?>
        <a
						href="<?php echo urlShop('member_evaluate', 'add_image', array('geval_id' => $v['geval_id']));?>">晒单</a>
        <?php } ?>
    </th>
				</tr>
				<tr>
