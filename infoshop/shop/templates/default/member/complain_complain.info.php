...
<dl>
    ...
    if (empty($output['complain_info']['complain_pic1']) && empty($output['complain_info']['complain_pic2']) && empty($output['complain_info']['complain_pic3'])) {
        ...
        if ((intval($output['complain_info']['complain_state']) < 99) && ($output['complain_info']['accuser_id'] == $_SESSION['member_id'])) {
			...
			nc_type="dialog"
			dialog_title="<?php echo $lang['complain_add_pic'];?>"
			dialog_id="complain_add_pic" dialog_width="400"
			uri="index.php?act=member_complain&op=complain_add_pic&complain_id=<?php echo $output['complain_info']['complain_id']; ?>"
			id="complain_add_pic"><?php echo $lang['complain_add_pic'];?></a>
                            <?php
        }
    } else {
...
<table class="order ncu-table-style">
    ...
    <tr>
		...
		<tr>
			...
			<td><a
				href="<?php echo urlShop('goods', 'index', array('goods_id'=>$complain_goods['goods_id']));?>"
				target="_blank"> <?php echo $complain_goods['goods_name'];?> </a></td>
