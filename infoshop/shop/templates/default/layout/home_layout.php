...
if (($ua == '' || preg_match($uachar, $ua)) && !strpos(strtolower($_SERVER['REQUEST_URI']), 'wap')) {
    if (empty($_GET['goods_id'])) {
        ...
        $Loaction = '/wap/tmpl/product_detail.html?goods_id=' . $_GET['goods_id'];
    }
    if (!empty($Loaction)) {
...
<script>
    window.onload = function () {
        ...
        var ms = document.getElementById("qh");
                ...
                el.height(curHeight).animate({height: autoHeight}, 1000, function () {
                    setTimeout(function () {
                        el.animate({height: 0}, function () {
                            $('.top_ad').hide().eq(1).show();
                            el.animate({height: 80});
                        });
                    }, 2000);
                });
            }, 1000);
...
<div class="header-wrap">
    <header class="public-head-layout wrapper">
        ...
        <div class="head-search-bar">
            <form action="<?php echo SHOP_SITE_URL; ?>" method="get"
                  class="search-form">
                <input name="act" id="search_act" value="search" type="hidden">
                <ul id="qh" onMouseOver="return(qiehuan())">
                    <li id='bb'>宝贝</li>
                    <li id="dp" style="display:none">店铺</li>
                ...
                <input name="keyword" id="keyword" type="text" class="input-text"
                       value="<?php echo $_GET['keyword']; ?>" maxlength="60"
                       x-webkit-speech lang="zh-CN" onwebkitspeechchange="foo()"
                       placeholder="请输入您要搜索的商品关键字" x-webkit-grammar="builtin:search"/> <input
                    type="submit" id="button" value="搜　索" class="input-submit">
            </form>
                ...
                <ul>
                    <?php if (is_array($output['hot_search']) && !empty($output['hot_search'])) {
                        foreach ($output['hot_search'] as $val) { ?>
                            <li><a
                                    href="<?php echo urlShop('search', 'index', array('keyword' => $val)); ?>"><?php echo $val; ?></a>
                            </li>
                        <?php }
                    } ?>
                </ul>
            ...
            <dl class="my-mall">
                ...
                <dd>
                    <div class="sub-title">
                        ...
                        <a
                            href="<?php echo SHOP_SITE_URL; ?>/index.php?act=member_snsindex"
                            class="arrow">我的用户中心<i></i></a>
                    </div>
                    <div class="user-centent-menu">
                        <ul>
                            <li><a
                                    href="<?php echo SHOP_SITE_URL; ?>/index.php?act=home&op=message">站内消息(<span><?php echo $output['message_num'] > 0 ? $output['message_num'] : '0'; ?></span>)
                                </a></li>
                            <li><a
                                    href="<?php echo SHOP_SITE_URL; ?>/index.php?act=member_order"
                                    class="arrow">我的订单<i></i></a></li>
                            <li><a
                                    href="<?php echo SHOP_SITE_URL; ?>/index.php?act=member_consult&op=my_consult">咨询回复(<span
                                        id="member_consult">0</span>)
                                </a></li>
                            <li><a
                                    href="<?php echo SHOP_SITE_URL; ?>/index.php?act=member_favorites&op=fglist"
                                    class="arrow">我的收藏<i></i></a></li>
                            <?php if (C('voucher_allow') == 1) { ?>
                                <li><a
                                        href="<?php echo SHOP_SITE_URL; ?>/index.php?act=member_voucher">代金券(<span
                                            id="member_voucher">0</span>)
                                    </a></li>
                            <?php } ?>
                            <?php if (C('points_isuse') == 1) { ?>
                                <li><a
                                        href="<?php echo SHOP_SITE_URL; ?>/index.php?act=member_points"
                ...
                <dd>
                    ...
                    <div class="checkout">
                        <span
                            ...
                            href="<?php echo SHOP_SITE_URL; ?>/index.php?act=cart"
