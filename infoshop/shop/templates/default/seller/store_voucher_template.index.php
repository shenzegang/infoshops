...
<div class="tabmenu">
    ...
    <a class="ncsc-btn ncsc-btn-green" style="right: 100px"
		href="<?php echo urlShop('store_voucher', 'templateadd');?>"><i
		class="icon-plus-sign"></i><?php echo $lang['voucher_templateadd']; ?></a>
	<a class="ncsc-btn ncsc-btn-acidblue"
		href="<?php echo urlShop('store_voucher', 'quotaadd');?>" title=""><i
		class="icon-money"></i>套餐续费</a>
    <?php } else { ?>
    <a class="ncsc-btn ncsc-btn-acidblue"
		href="<?php echo urlShop('store_voucher', 'quotaadd');?>" title=""><i
		class="icon-money"></i>购买套餐</a>
    <?php } ?>
</div>
<div class="alert alert-block mt10 mb10">
	<h4><?php echo $lang['nc_explain'].$lang['nc_colon'];?></h4>
      ...
      <strong>套餐过期时间<?php echo $lang['nc_colon'];?></strong><strong
		style="color: #F00;"><?php echo date('Y-m-d H:i:s', $output['current_quota']['quota_endtime']);?></strong>
      <?php } else { ?>
...
if ($val['voucher_t_state'] == $output['templatestate_arr']['usable'][0])
                echo $output['templatestate_arr']['usable'][1];
            if ($val['voucher_t_state'] == $output['templatestate_arr']['disabled'][0])
                echo $output['templatestate_arr']['disabled'][1];
            ?></td>
			<td>
        	<?php if($val['voucher_t_state']==$output['templatestate_arr']['usable'][0] && !$val['voucher_t_giveout']) {//代金券模板有效并且没有领取时可以编辑?>
        		<a
				href="index.php?act=store_voucher&op=templateedit&tid=<?php echo $val['voucher_t_id'];?>"><?php echo $lang['nc_edit'];?></a>
        	<?php }else {//代金券模板失效或者有领取时可以查看?>
        		<a
				href="index.php?act=store_voucher&op=templateinfo&tid=<?php echo $val['voucher_t_id'];?>"><?php echo $lang['nc_view'];?></a>
        	<?php }?>
        	<?php if (!$val['voucher_t_giveout']){//该模板没有发放过代金券时可以删除?>
        	<br />
			<a href="javascript:void(0)"
				onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>','index.php?act=store_voucher&op=templatedel&tid=<?php echo $val['voucher_t_id'];?>');"
				class="ncsc-btn"><?php echo $lang['nc_del'];?></a>
        	<?php }?>
        </td>
		</tr>
      <?php }?>
