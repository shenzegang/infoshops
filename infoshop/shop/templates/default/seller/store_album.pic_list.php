...
<div class="tabmenu">
  ...
  <a id="open_uploader" href="JavaScript:void(0);"
		class="ncsc-btn ncsc-btn-acidblue"><i class="icon-cloud-upload"></i><?php echo $lang['album_class_list_img_upload'];?></a>
	<div class="upload-con" id="uploader" style="display: none;">
		<form method="post" action="" id="fileupload"
			enctype="multipart/form-data">
			<input type="hidden" name="category_id" id="category_id"
				value="<?php echo $output['class_info']['aclass_id']?>">
			<div class="upload-con-div">
				...
				<div class="ncsc-upload-btn">
					<a href="javascript:void(0);"><span> <input type="file"
						...
						<p>
							<i class="icon-upload-alt"></i><?php echo $lang['album_class_list_img_upload'];?></p>
					</a>
				</div>
			</div>
			...
			<div class="upload-txt">
				<span><?php echo $lang['album_batch_upload_description'].$GLOBALS['setting_config']['image_max_filesize'].'KB'.$lang['album_batch_upload_description_1'];?></span>
			</div>
		</form>
	</div>
</div>
<div id="pictureFolder" class="ncsc-picture-folder">
	<dl class="ncsc-album-intro">
      ...
      <img id="aclass_cover"
				src="<?php echo cthumb($output['class_info']['aclass_cover'], 60,$_SESSION['store_id']);?>">
      <?php }else{?>
	...
	<table class="search-form">
		<tbody>
          ...
          <a href="JavaScript:void(0);" class="ncsc-btn-mini"
					onClick="checkAll()"><i class="icon-check"></i><?php echo $lang['album_plist_check_all'];?></a><a
					href="JavaScript:void(0);" class="ncsc-btn-mini"
					onClick="uncheckAll()"><i class="icon-check-sign"></i><?php echo $lang['album_plist_cancel'];?></a><a
					href="JavaScript:void(0);" class="ncsc-btn-mini"
					onClick="switchAll()"><i class="icon-check-empty"></i><?php echo $lang['album_plist_inverse'];?></a><a
					href="JavaScript:void(0);" class="ncsc-btn-mini"
					onClick="submit_form('del')"><i class="icon-trash"></i><?php echo $lang['album_class_delete'];?></a><a
					href="JavaScript:void(0);" class="ncsc-btn-mini" id="img_move"><i
						class="icon-move"></i><?php echo $lang['album_plist_move_album'];?></a>

					<a href="JavaScript:void(0);" class="ncsc-btn-mini"
					onClick="submit_form('watermark')"><i class=" icon-paste"></i><?php echo $lang['album_plist_add_watermark'];?></a>
            ...
            <span><?php echo $lang['album_plist_move_album_change'].$lang['nc_colon'];?></span>
              ...
              <?php }?>
            </select> <input type="button" onClick="submit_form('move')"
							value="<?php echo $lang['album_plist_move_album_begin'];?>" />
            <?php }else{?>
            <span><?php echo $lang['album_plist_move_album_only_one'];?><a
							href="JavaScript:void(0);"
							uri="index.php?act=store_album&op=album_add" nc_type="dialog"
							dialog_title="<?php echo $lang['album_class_add'];?>"><?php echo $lang['album_class_add'];?></a><?php echo $lang['album_plist_move_album_only_two'];?></span>
            <?php }?>
          </dd>
					</dl>
          <?php }?></td>
          ...
          <form name="select_sort" id="select_sort">
						<input type="hidden" name="act" value="store_album" /> <input
							type="hidden" name="op" value="album_pic_list" /> <input
							type="hidden" name="id"
							value="<?php echo $output['class_info']['aclass_id']?>" /> <select
							name="sort" id="img_sort">
							<option value="0" <?php if($_GET['sort'] == '0'){?> selected
								<?php }?>><?php echo $lang['album_sort_upload_time_desc'];?></option>
							<option value="1" <?php if($_GET['sort'] == '1'){?> selected
								<?php }?>><?php echo $lang['album_sort_upload_time_asc'];?></option>
							<option value="2" <?php if($_GET['sort'] == '2'){?> selected
								<?php }?>><?php echo $lang['album_sort_img_size_desc'];?></option>
							<option value="3" <?php if($_GET['sort'] == '3'){?> selected
								<?php }?>><?php echo $lang['album_sort_img_siza_asc'];?></option>
							<option value="4" <?php if($_GET['sort'] == '4'){?> selected
								<?php }?>><?php echo $lang['album_sort_img_name_desc'];?></option>
							<option value="5" <?php if($_GET['sort'] == '5'){?> selected
								<?php }?>><?php echo $lang['album_sort_img_name_asc'];?></option>
						</select>
					</form>
          <?php }?></td>
			</tr>
  ...
  <form name="checkboxform" id="checkboxform" method="POST" action="">
		<div class="ncsc-picture-list">
			<div class="alert alert-info">
				<strong><?php echo $lang['album_plist_replace_same_type'];?></strong>
			</div>
			<ul>
        <?php $ii=0;?>
        ...
        <li>
					<dl>
						<dt>
							<div class="picture">
								<a
									href="index.php?act=store_album&op=album_pic_info&id=<?php echo $v['apic_id'];?>&class_id=<?php echo $v['aclass_id']?><?php if(!empty($_GET['sort'])){?>&sort=<?php echo $_GET['sort']; }?>&curpage=<?php echo ceil((12*($curpage-1)+$ii)/7);?>&start_index=<?php echo ($tmp = (12*($curpage-1)+$ii)%7) ? $tmp-1 :6;?>">
									<img id="img_<?php echo $v['apic_id'];?>"
									src="<?php echo thumb($v, 240);?>">
								</a>
							</div>
							<input id="C<?php echo $v['apic_id'];?>" name="id[]"
								value="<?php echo $v['apic_id'];?>" type="checkbox"
								class="checkbox" /> <input id="<?php echo $v['apic_id'];?>"
								class="editInput1" readonly
								onDblClick="$(this).unbind();_focus($(this));"
								value="<?php echo $v['apic_name']?>"
								title="<?php echo $lang['album_plist_double_click_edit'];?>"
								style="cursor: pointer;"> <span
								onDblClick="_focus($(this).prev());"
								title="<?php echo $lang['album_plist_double_click_edit'];?>"><i
						...
						<dd class="buttons">
							<div class="upload-btn">
								<a href="javascript:void(0);"> <span> <input type="file"
										name="file_<?php echo $v['apic_id'];?>"
										id="file_<?php echo $v['apic_id'];?>" class="input-file"
										size="1" hidefocus="true" nctype="replace_image" />
								</span>
									<div class="upload-button">
										<i class="icon-upload-alt"></i><?php echo $lang['album_plist_replace_upload'];?></div>
									<input id="submit_button" style="display: none" type="button"
									value="<?php echo $lang['store_slide_image_upload'];?>"
									onClick="submit_form($(this))" />
								</a>
							</div>
							<a href="JavaScript:void(0);" nc_type="dialog"
								dialog_title="<?php echo $lang['album_plist_move_album'];?>"
								uri="index.php?act=store_album&op=album_pic_move&cid=<?php echo $output['class_info']['aclass_id']?>&id=<?php echo $v['apic_id']?>"><i
								class="icon-move"></i><?php echo $lang['album_plist_move_album'];?></a>
							<a href="JavaScript:void(0);"
								onclick="cover(<?php echo $v['apic_id'];?>)"><i
								class="icon-picture"></i><?php echo $lang['album_plist_set_to_cover'];?></a>
							<a href="javascript:void(0)"
								onclick="ajax_get_confirm('<?php echo $lang['album_plist_delete_confirm_message'];?>','index.php?act=store_album&op=album_pic_del&id=<?php echo $v['apic_id'];?>');"><i
								class="icon-trash"></i><?php echo $lang['album_plist_delete_img'];?></a>
						</dd>
					</dl>
				</li>
        <?php }?>
