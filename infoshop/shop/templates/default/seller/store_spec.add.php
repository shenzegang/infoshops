...
<body>
	...
	<form id="spec_form" method="post"
		action="<?php echo SHOP_SITE_URL;?>/index.php?act=store_spec&op=save_spec">
		<input type="hidden" name="form_submit" value="ok" /> <input
			type="hidden" name="sp_id"
			value="<?php echo $output['sp_info']['sp_id'];?>" /> <input
			type="hidden" name="gc_id"
			value="<?php echo $output['gc_info']['gc_id'];?>" />
		<div class="ncsc-spec-info" style="width: 960px;">
			<span>规格名称：<?php echo $output['sp_info']['sp_name'];?></span><span>所属分类：<?php echo $output['gc_info']['gc_name'];?></span><a
				href="javascript:void(0);" nctype="add_sv" class="ncsc-btn"><i
				class="icon-th-list"></i>添加规格值</a>
		</div>
		...
		<div id="iframe" class="iframe-box" style="width: 960px;">
          ...
          <tr class="bd-line">
							<td class="w80"><input type="text" class="text w40"
								name="sv[old][<?php echo $value['sp_value_id'];?>][sort]"
								value="<?php echo $value['sp_value_sort'];?>" /></td>
							<td class="tl"><input type="text" class="text w250"
								name="sv[old][<?php echo $value['sp_value_id'];?>][name]"
								value="<?php echo $value['sp_value_name'];?>" /></td>
            <?php if ($output['sp_info']['sp_id'] == DEFAULT_SPEC_COLOR_ID) {?>
            <td class="w300 tl"><input type="text" class="text w100"
								name="sv[old][<?php echo $value['sp_value_id'];?>][color]"
								value="<?php echo $value['sp_value_color'];?>"
								nctype="spec_color" /></td>
            <?php }?>
            <td class="w110 nscs-table-handle"><span><a
									href="javascript:void(0);" class="btn-red" nctype="spec_del"
									data-param="{id:<?php echo $value['sp_value_id'];?>}"><i
										class="icon-trash"></i>
										<p>删除</p> </a></span></td>
						</tr>
          <?php }?>
          ...
          <tr class="bd-line">
							<td class="w80"><input type="text" class="text w40"
								name="sv[new][1][sort]" value="0" /></td>
							<td class="tl"><input type="text" class="text w250"
								name="sv[new][1][name]" /></td>
            <?php if ($output['sp_info']['sp_id'] == DEFAULT_SPEC_COLOR_ID) {?>
            <td class="w300 tl"><input type="text" class="text w100"
								name="sv[new][1][color]" nctype="spec_color" /></td>
            <?php }?>
            <td class="w110 nscs-table-handle"><span><a
									href="javascript:void(0);" class="btn-orange"
									nctype="tr_remove"><i class="icon-minus-sign"></i>
										<p>移除</p> </a></span></td>
						</tr>
          <?php }?>
        </tbody>
					<tfoot>
						<tr>
							<td colspan="20" class="bottom"><a href="javascript:void(0);"
								nctype="submit-btn" class="submit mt20">提交保存规格值</a></td>
						</tr>
						<tr>
