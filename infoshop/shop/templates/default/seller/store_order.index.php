...
<form method="get" action="index.php" target="_self">
    ...
    <tr>
			...
			<td class="w240"><input type="text" class="text w70"
				...
				id="query_end_date" class="text w70" type="text"
			...
			<td class="w100"><input type="text" class="text w80"
				name="buyer_name" value="<?php echo $_GET['buyer_name']; ?>" /></td>
...
<table class="ncsc-table-style order">
  ...
  <tbody>
		...
		<tr>
			<th colspan="20"><span class="fl ml10"><?php echo $lang['store_order_order_sn'].$lang['nc_colon'];?><span
					class="goods-num"><em><?php echo $order['order_sn']; ?></em>
        <?php if ($order['order_from'] == 2){?><i
						class="icon-mobile-phone"></i><?php }?>
        </span></span> <span class="fl ml20"><?php echo $lang['store_order_add_time'].$lang['nc_colon'];?><em
...
<?php if ($order['if_deliver']) { ?>
        <a
					...
					class="ncsc-btn-mini"><i class="icon-compass"></i><?php echo $lang['store_order_show_deliver'];?>
        </a>
        <?php } ?>
        <a
					href="index.php?act=store_order&op=show_order&order_id=<?php echo $order_id;?>"
					target="_blank" class="ncsc-btn-mini"><i class="icon-file-text-alt"></i><?php echo $lang['store_order_view_order'];?></a>
					<a
					href="index.php?act=store_order_print&order_id=<?php echo $order_id;?>"
					class="ncsc-btn-mini" target="_blank" title="打印发货单" /><i
					class="icon-print"></i>打印发货单</a>
			</span></th>
		</tr>
    ...
    <tr>
			...
			<td class="w50"><div class="pic-thumb">
					<a
						href="<?php echo urlShop('goods','index',array('goods_id'=>$goods['goods_id']));?>"
						target="_blank"><img src="<?php echo thumb($goods,60);?>" /></a>
				</div></td>
			<td class="tl">
				<dl class="goods-name">
					<dt><?php if(!empty($goods['is_gift'])){ ?><span
							title="该商品为积分兑换礼品，消耗<?php echo $goods['gift_points']; ?>积分">礼</span><?php } ?><a
							target="_blank"
							href="<?php echo urlShop('goods','index',array('goods_id'=>$goods['goods_id']));?>"><?php echo $goods['goods_name']; ?></a>
					</dt>
          <?php if ($goods['goods_type'] > 1){?>
          <dd>
						<span class="sale-type"><?php echo orderGoodsType($goods['goods_type']);?></span>
					</dd>
          <?php }?>
        </dl>
			</td>
      ...
      <td class="bdl"
				rowspan="<?php echo count($order['extend_order_goods']);?>">
          ...
          <p member_id="<?php echo $order['buyer_id'];?>">
            ...
            <a target="_blank"
							...
							border="0"
							src="http://amos.im.alisoft.com/online.aw?v=2&uid=<?php echo $order['extend_member']['store_ww'];?>&site=cntaobao&s=2&charset=<?php echo CHARSET;?>"
			...
			<td class="bdl"
				rowspan="<?php echo count($order['extend_order_goods']);?>"><p
        ...
        <p>
					<a href="javascript:void(0)" class="ncsc-btn-mini ncsc-btn-red mt5"
						nc_type="dialog"
        ...
        <p>
					<a href="javascript:void(0)"
						class="ncsc-btn-mini ncsc-btn-blue mt10"
						uri="index.php?act=store_order&op=change_state&state_type=goods_price&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>"
						dialog_width="480"
						dialog_title="<?php echo $lang['store_order_modify_price'];?>"
						nc_type="dialog" dialog_id="seller_order_adjust_fee"
						id="order<?php echo $order['order_id']; ?>_action_adjust_fee" /><i
        ...
        <p>
					<a class="ncsc-btn-mini ncsc-btn-green mt10"
