...
<div class="tabmenu">
  ...
  <a class="ncsc-btn ncsc-btn-green" style="right: 100px"
		href="<?php echo urlShop('store_promotion_mansong', 'mansong_add');?>"><i
		class="icon-plus-sign"></i><?php echo $lang['mansong_add'];?></a> <a
		class="ncsc-btn ncsc-btn-acidblue"
		href="<?php echo urlShop('store_promotion_mansong', 'mansong_quota_add');?>"
		title=""><i class="icon-money"></i>套餐续费</a>
  <?php } else { ?>
  <a class="ncsc-btn ncsc-btn-acidblue"
		href="<?php echo urlShop('store_promotion_mansong', 'mansong_quota_add');?>"
		title=""><i class="icon-money"></i>购买套餐</a>
  <?php } ?>
</div>
<div class="alert alert-block mt10">
  <?php if(!empty($output['current_mansong_quota'])) { ?>
  <strong>套餐过期时间<?php echo $lang['nc_colon'];?></strong><strong
		style="color: #F00;"><?php echo date('Y-m-d H:i:s', $output['current_mansong_quota']['end_time']);?></strong>
	</li>
  <?php } else { ?>
		...
		<tr>
          ...
          <?php } ?>
			...
			<td class="w160"><input type="text" class="text w150"
				name="mansong_name" value="<?php echo $_GET['mansong_name'];?>" /></td>
  ...
  <thead>
    ...
    <tr class="bd-line">
			...
			<td class="nscs-table-handle"><span><a
					href="index.php?act=store_promotion_mansong&op=mansong_detail&mansong_id=<?php echo $val['mansong_id'];?>"
					class="btn-blue"><i class="icon-th-list"></i>
						<p><?php echo $lang['nc_detail'];?></p> </a></span> <span><a
					nctype="btn_mansong_del"
					data-mansong-id="<?php echo $val['mansong_id'];?>"
					href="javascript:return void(0)" class="btn-red"><i
						class="icon-trash"></i>
