...
<div class="tabmenu">
  ...
  <a class="ncsc-btn ncsc-btn-green" style="right: 100px"
		href="<?php echo urlShop('store_promotion_xianshi', 'xianshi_add');?>"><i
		class="icon-plus-sign"></i><?php echo $lang['xianshi_add'];?></a> <a
		class="ncsc-btn ncsc-btn-acidblue"
		href="<?php echo urlShop('store_promotion_xianshi', 'xianshi_quota_add');?>"
		title=""><i class="icon-money"></i>套餐续费</a>
  <?php } else { ?>
  <a class="ncsc-btn ncsc-btn-acidblue"
		href="<?php echo urlShop('store_promotion_xianshi', 'xianshi_quota_add');?>"
		title=""><i class="icon-money"></i><?php echo $lang['xianshi_quota_add'];?></a>
  <?php } ?>
</div>
<div class="alert alert-block mt10">
  <?php if(!empty($output['current_xianshi_quota'])) { ?>
  <strong>套餐过期时间<?php echo $lang['nc_colon'];?></strong><strong
		style="color: #F00;"><?php echo date('Y-m-d H:i:s', $output['current_xianshi_quota']['end_time']);?></strong>
  <?php } else { ?>
  ...
  <ul>
		...
		<li>4、<strong style="color: red">相关费用会在店铺的账期结算中扣除</strong>。
		</li>
	</ul>
</div>
<form method="get">
	<table class="search-form">
		...
		<tr>
          ...
          <option value="<?php echo $key;?>"
						<?php if(intval($key) === intval($_GET['state'])) echo 'selected';?>><?php echo $val;?></option>
          <?php } ?>
          <?php } ?>
        </select></td>
			<th class="w110"><?php echo $lang['xianshi_name'];?></th>
			<td class="w160"><input type="text" class="text w150"
				name="xianshi_name" value="<?php echo $_GET['xianshi_name'];?>" /></td>
...
<table class="ncsc-table-style">
  ...
  <tbody id="xianshi_list">
		<tr class="bd-line">
          ...
          <?php if($val['editable']) { ?>
          <span> <a
					href="index.php?act=store_promotion_xianshi&op=xianshi_edit&xianshi_id=<?php echo $val['xianshi_id'];?>"
          ...
          <?php } ?>
          <span> <a
					href="index.php?act=store_promotion_xianshi&op=xianshi_manage&xianshi_id=<?php echo $val['xianshi_id'];?>"
					class="btn-green"> <i class="icon-cog"></i>
						<p><?php echo $lang['nc_manage'];?></p>
				</a>
			</span> <span> <a href="javascript:;" nctype="btn_del_xianshi"
					data-xianshi-id=<?php echo $val['xianshi_id'];?> class="btn-red"> <i
						class="icon-trash"></i>
  ...
  <tr id="xianshi_list_norecord">
			<td class="norecord" colspan="20"><div class="warning-option">
