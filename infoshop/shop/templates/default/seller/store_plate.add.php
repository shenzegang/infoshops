...
<div class="ncsc-form-default">
	<form method="post"
		action="<?php if (empty($output['plate_info'])) { echo urlShop('store_plate', 'plate_add');} else { echo urlShop('store_plate', 'plate_edit');}?>"
		id="plate_form">
		<input type="hidden" name="form_submit" value="ok" /> <input
			type="hidden" name="p_id"
			value="<?php echo $output['plate_info']['plate_id'];?>" />

		<dl>
			<dt>
				<i class="required">*</i>版式名称<?php echo $lang['nc_colon'];?></dt>
			<dd>
				<input type="text" class="text w200" name="p_name"
					value="<?php echo $output['plate_info']['plate_name']?>"
					id="p_name" />
		...
		<dl>
			<dt>
				<i class="required">*</i>版式位置<?php echo $lang['nc_colon'];?></dt>
			<dd id="gcategory">
				<ul class="ncsc-form-radio-list">
					<li><label><input type="radio" name="p_position" id="p_position"
							value="1" class="radio"
							<?php if (empty($output['plate_info']) || $output['plate_info']['plate_position'] == 1) {?>
							checked="checked" <?php }?> />顶部</label></li>
					<li><label><input type="radio" name="p_position" id="p_position"
							value="0" class="radio"
							<?php if (!empty($output['plate_info']) && $output['plate_info']['plate_position'] == 0) {?>
							checked="checked" <?php }?> />底部</label></li>
				</ul>
		...
		<dl>
			<dt>
				<i class="required">*</i>版式内容<?php echo $lang['nc_colon'];?></dt>
			<dd>
            ...
            <p class="hr8">
					<a class="ncsc-btn" nctype="show_desc"
						...
						class="icon-picture"></i>插入相册图片</a> <a href="javascript:void(0);"
						nctype="del_desc" class="ncsc-btn ml5" style="display: none;"><i
