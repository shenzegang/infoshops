...
<div class="wrap">
    ...
    <a
			href="<?php echo urlShop('store_navigation', 'navigation_add');?>"
			class="ncsc-btn ncsc-btn-green" title="添加导航">添加导航</a>
	</div>
	<table class="ncsc-table-style">
      ...
      <tr class="bd-line">
        ...
        <td class="tl"><dl class="goods-name">
						<dt>
							<a href="<?php echo $sn_href;?>" target="_blank"><?php echo $value['sn_title'];?></a>
						</dt>
					</dl></td>
				<td><?php if($value['sn_if_show']){echo $lang['nc_yes'];}else{echo $lang['nc_no'];}?></td>
				<td class="nscs-table-handle"><span><a
						href="<?php echo urlShop('store_navigation', 'navigation_edit', array('sn_id' => $value['sn_id']));?>"
						class="btn-blue"><i class="icon-edit"></i>
							<p> <?php echo $lang['nc_edit'];?></p> </a></span><span> <a
						href="javascript:;" nctype="btn_del"
						data-sn-id="<?php echo $value['sn_id'];?>" class="btn-red"><i
							class="icon-trash"></i>
...
<form id="del_form" method="post"
	action="<?php echo urlShop('store_navigation', 'navigation_del');?>">
