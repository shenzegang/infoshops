<div class="eject_con">
    ...
    <form id="post_form" method="post"
          action="index.php?act=store_refund&op=edit&refund_id=<?php echo $output['refund']['refund_id']; ?>">
        ...
        <dl>
            <dt>
                <i class="required">*</i><?php echo $lang['return_seller_confirm'] . $lang['nc_colon']; ?></dt>
            <dd>
                <label> <input type="radio" name="seller_state" value="2"/>
                    <?php echo $lang['nc_yes']; ?> </label> <label>

                    <?php if ($output['order_info']['order_state'] == "30"){ ?>
                    <input name="return_type" type="checkbox" value="2"/>要求买家寄回货物</label>
                <?php } ?>


                <br/>
                <label> <input type="radio" name="seller_state" value="3"/>
        ...
        <dl>
            <dt>
                <i class="required">*</i><?php echo $lang['refund_message'] . $lang['nc_colon']; ?></dt>
            <dd>
        ...
        <dl>
            ...
            <dd>
                <p class="hint">
                    只能提交一次，请认真选择。<br>同意并经过平台确认后会将金额以预存款的形式返还给买家。<br>不同意时买家可以向平台投诉或再次申请。
                </p>
            </dd>
        </dl>
        <div class="bottom">
            <label class="submit-border"><input type="submit" class="submit"
                                                id="confirm_button" value="<?php echo $lang['nc_ok']; ?>"/></label>
        </div>
    </form>
</div>
    ...
    $(function () {
        $('#post_form').validate({
            ...
            submitHandler: function (form) {
                if ($("input[type='radio'][name='seller_state']:checked").val() == '3') {
                    showDialog('拒绝退款退货可导致平台介入,确定拒绝吗？', 'confirm', '', function () {
