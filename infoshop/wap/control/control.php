    ...
    protected function checkLogin()
    {
        if ($_SESSION['is_login'] !== '1') {
            ...
            if ($_GET['inajax']) {
                ...
                @header("location: tmpl/member/login.html?ref_url=" . urlencode($ref_url));
            }
            exit();
        }
    }
 ...
 // 允许插入新记录的最大条数
    ...
    public function get_member_info()
    {
        ...
        if ($_cache = rcache($hash_key, 'sns_member')) {
            ...
            // 粉丝数
            $fan_count = $model->table('sns_friend')
                ->where(array(
            ...
            // 关注数
            $attention_count = $model->table('sns_friend')
                ->where(array(
            ...
            // 兴趣标签
            $mtag_list = $model->table('sns_membertag,sns_mtagmember')
                ->field('mtag_name')
                ->on('sns_membertag.mtag_id = sns_mtagmember.mtag_id')
                ->join('inner')
                ->where(array(
    ...
    protected function get_visitor()
    {
        $model = Model();
        // 查询谁来看过我
        $visitme_list = $model->table('sns_visitor')
            ->where(array(
        ...
        if ($this->relation == 3) { // 主人自己才有我访问过的人
                                  // 查询我访问过的人
            $visitother_list = $model->table('sns_visitor')
                ->where(array(
    ...
    private function check_relation()
    {
        ...
        if ($_SESSION['is_login'] == '1') {
            if ($this->master_id == $_SESSION['member_id']) { // 主人自己
                ...
                $this->relation = 1;
                // 查询好友表
                $friend_arr = $model->table('sns_friend')
                    ->where(array(
    ...
    private function add_visit()
    {
        ...
        if ($_SESSION['is_login'] == '1' && $this->relation != 3) {
            ...
            if (! empty($visitor_info)) {
                // 查询访客记录是否存在
                $existevisitor_info = $model->table('sns_visitor')
                    ->where(array(
                    'v_ownermid' => $this->master_id,
    ...
    private function my_attention()
    {
        if (intval($_SESSION['member_id']) > 0) {
            $my_attention = Model()->table('sns_friend')
                ->where(array(
                'friend_frommid' => $_SESSION['member_id']
            ))
                ->order('friend_addtime desc')
                ->limit(4)
                ->select();
