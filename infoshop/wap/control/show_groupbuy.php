...
class show_groupbuyControl extends BaseHomeControl
{
    ...
    private function _show_groupbuy_list($function_name)
    {
        ...
        if (intval($_GET['groupbuy_price']) > 0) {
            ...
            foreach ($price_range_list as $price_range) {
                if ($price_range['range_id'] == $_GET['groupbuy_price']) {
                    $condition['groupbuy_price'] = array(
                        'between',
                        array(
        ...
        Tpl::output('show_page', $model_groupbuy->showpage(1));
    ...
    public function groupbuy_detailOp()
    {
        ...
        // 浏览数加1
        $update_array = array();
        $update_array['views'] = array(
            'exp',
            'views+1'
        );
        $model_groupbuy->editGroupbuy($update_array, array(
        ...
        Model('seo')->type('group_content')
            ->param(array(
            'name' => $groupbuy_info['groupbuy_name']
        ))
            ->show();
