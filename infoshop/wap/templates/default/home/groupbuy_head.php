...
<link href="<?php echo SHOP_TEMPLATES_URL;?>/css/home_group.css"
...
<div id="headRelative" class="ncg-header">
  ...
  <div id="list" class="list" style="display: none;">
		<a id="button_close" class="close" href="javascript:void(0)">X</a>
		<ul>
			<li><a href="<?php echo dropParam(array('area_id'));?>"
				onClick="set_groupbuy_area('')"><?php echo $lang['text_country'];?></a></li>
      <?php if(is_array($output['area_list'])) { ?>
      <?php foreach($output['area_list'] as $groupbuy_area) { ?>
      <?php if(intval($groupbuy_area['deep']) === 0) { ?>
      <li><a
				href="<?php echo replaceParam(array('area_id' => $groupbuy_area['area_id']));?>"
				onClick="set_groupbuy_area('<?php echo $groupbuy_area['area_id'].','.$groupbuy_area['area_name'];?>')"><?php echo $groupbuy_area['area_name'];?></a></li>
      <?php } ?>
      <?php } ?>
      <?php } ?>
    </ul>
