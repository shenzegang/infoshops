$(function() {
	...
	var paypasswd = getcookie('paypasswd')
		...
		success:function(result){
			...
			$.each(data.store_cart_list,function(k,v){//循环店铺
				...
				htmldata+='<p class="buys-yt-tlt">店铺名称：'+v.store_name+'</p>';
						$.each(v.goods_list,function(k1,v1){//循环商品列表
                            ...
                            if(v1.goods_price > 0){
                                v1.goods_price = '<p>单价(元)：￥' + v1.goods_price + '</p>';
                            }else{
                                v1.goods_price = '';
                            }
                            
                            if(v1.gift_points > 0){
                                v1.gift_points = '<p>积分：'+ v1.gift_points + '</p>';
                            }else{
                                v1.gift_points = '';
                            }
							...
							htmldata+='<div class="clearfix">'
												...
												+'<div class="buys1-pdlcnt">'
													+'<p><a class="buys1-pdlc-name" href="'+WapSiteUrl+'/tmpl/product_detail.html?goods_id='+v1.goods_id+'">'+v1.goods_name+'</a></p>' + v1.goods_price + v1.gift_points + '<p>数量：'+v1.goods_num+'</p>'
												+'</div>'
											+'</div>'
										+'</div>';
						});
						...
						if(v.store_mansong_rule_list != null){
							var sp_total = v.store_goods_total-v.store_mansong_rule_list.discount;
						}else{
                        ...
                        if(v.store_gift_total > 0){
							htmldata+='<p class="clr-c07">积分：￥<span id="st'+k+'" store_price="'+v.store_gift_total+'" class="store_total">'+v.store_gift_total+'</span></p>';
						}
						htmldata+='</div>';
						htmldata+='</li>';
			...
			$.ajax({//保存地址
				...
				success:function(result){
					if(result.datas.state == 'success'){
						...
						var total_price = parseFloat($('input[name=total_price]').val())+sp_s_total;
						$('#total_price').html(total_price);						
						//$('input[name=total_price]').val(total_price);
			...
			$('select[name=voucher]').change(function(){//选择代金券				
				...
				if(varr == 0){
					...
					var store_price = parseFloat(varr.split('|')[2]);
				}
				var store_total_price = parseFloat($('#st'+store_id).attr('store_price'));
				var store_tran = parseFloat($('#store'+store_id).html());
				store_total = store_total_price - store_price + store_tran;
				$("#sv"+store_id).html(store_price);
				$("#st"+store_id).html(store_total);
				
				var total_price = '';
				$('.store_total').each(function(){
					total_price=parseFloat($(this).html())+total_price;
				});
				$('#total_price').html(total_price);
			});
		}
    ...
    $('#pguse').click(function(){//验证密码
		...
		var pay_passwd_again = $("input[name=pay_passwd_again]").val();
    	if(pay_passwd == ''){
    		$('.password_error_tip').show();
    		$('.password_error_tip').html('登录密码不能为空');
    		return false;
    	}else if(pay_passwd != pay_passwd_again){
			$('.password_error_tip').show();
    		$('.password_error_tip').html('两次密码输入不一致');
    		return false;
		}else if(pay_passwd.length < 6){
			$('.password_error_tip').show();
    		$('.password_error_tip').html('支付密码长度至少为6位');
    		return false;
		}
    	$.ajax({
    		type:'post',
    		url:ApiUrl+'/index.php?act=member_buy&op=paypasswd',
    		data:{key:key,paypasswd:pay_passwd},
    		dataType:'json',
    		success:function(result){
    			if(result.datas == 1){
    ...
    $('#buy_step2').click(function(){//提交订单step2
        ...
        if(!paypasswd){//使用预存款
			...
			if(passwd_verify != 1){//验证密码失败
				$('.password_error_tip').show();
    			$('.password_error_tip').html('你还没有设置支付密码，无法提交订单');       		
				return false;
			}
			//var pd_pay = 1;
			//data.pd_pay = pd_pay;
			//var passwd = $('input[name=paypasswd]').val();
