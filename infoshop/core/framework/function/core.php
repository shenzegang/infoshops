...
function showMessage($msg, $url = '', $show_type = 'html', $msg_type = 'succ', $is_show = 1, $time = 2000) {
	...
	switch($show_type) {
		...
		case 'exception' :
			...
			echo '<dd><p /><p /><p /><p /></dd>';
			echo '<dd><p /><p /><p /><p />Copyright 2003-2015 infolShop, All Rights Reserved ' . $lang['company_name'] . '</dd>';
			echo '</dl>';
			echo '</body>';
			echo '</html>';
			exit();
...
/*
 * 自定义memory_get_usage() @return 内存使用额度，如果该方法无效，返回0
 */
...
function getMemberIdcard($member_idcard) {
	if(empty($member_idcard)){
		return UPLOAD_SITE_URL . DS . ATTACH_COMMON . DS . C('default_user_portrait');
	}else{
		if(file_exists(BASE_UPLOAD_PATH . DS . ATTACH_IDCARD . DS . $member_idcard)){
			
			return UPLOAD_SITE_URL . DS . ATTACH_IDCARD . DS . $member_idcard;
		}else{
			return UPLOAD_SITE_URL . DS . ATTACH_COMMON . DS . C('default_user_portrait');
		}
	}
 ...
 * 获取合同电子档pdf
 */
function getStoreJoininAgreementUrl($file_name = '') {
	return UPLOAD_SITE_URL . DS . ATTACH_AGREEMENT . DS . $file_name;
}

/**
 * 获取合同模板word
 */
function getAgreementTemplateUrl($file_name = '') {
	return UPLOAD_SITE_URL . DS . ATTACH_AGREEMENT_TEMPLATE . DS . $file_name;
}
...
function write_file($filepath, $data, $mode = null) {
	...
	if($data === '') $data = '\'\'';
	$data = "<?php defined('CorShop') or exit('Access Invalid!'); return " . $data . "\n?>";
 ...
 *        	缓存内容
 * @param string $type
 *        	file,memcache,xcache,apc,eaccelerator，可以为空，默认为file缓存
...
function H($key, $value = '', $cache_type = '', $expire = null, $args = null) {
	...
	if($value !== ''){
		if(is_null($value)){ // 删除缓存
			...
			if($result) unset($cache[$cache_type . '_' . $key]);
			return $result;
		}else{ // 缓存数据
	...
	if(isset($cache[$cache_type . '_' . $key])) return $cache[$cache_type . '_' . $key];
