...
class UploadFile
{
     ...
     * 上传操作
     *
     * @param string $field $file_size(上传文件大小，默认为1M)
     *            上传表单名
     * @return bool
     */
    public function upfile($field,$file_size = 1)
    {
        ...
        if ($this->upload_file['size'] > $this->max_size* $file_size * 1024) {
        ...
        // 验证文件格式是否为系统允许
        if(!($tmp_ext == 'pdf'|| $tmp_ext == 'doc'|| $tmp_ext == 'docx')){
